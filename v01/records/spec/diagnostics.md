...
Tagged union model definition diagnostics は runtime MCP request / response payload validation を行わない。実際の JSON payload に discriminator field が存在するか、discriminator value が許可 tag に含まれるか、payload field が存在するか、unknown additional field を許可するかは V01-WORK-DATA-010 minimum の diagnostic 対象外である。
...
> 由来: V01-ADR-067 §6, V01-ADR-070 §7〜§8, V01-REQ-DATA-003 / V01-TASK-DATA-004-01
...
> 由来: V01-ADR-060 §6, §7; V01-ADR-061 §9; V01-ADR-062 §8; V01-ADR-063 §9
