...
| `state` | Application | FSMの状態ノード。State Diagramで使用。`store`（データ保持）とは別概念（V01-ADR-019参照） |
...
> `foreach` はV01-ADR-016にてnode typeから廃止。`flow:` セクションの制御構文として記述する。
...
`endpoint: true` を付与したtaskはバックエンドエンドポイントとして扱われ、`list_endpoints` MCPツールで出力される。class diagram viewには出力しない（V01-ADR-017）。
...
> endpointのfull pathはtask単体では持たず、API Table viewの `http_root_path` とmodule階層から構成される（V01-ADR-028）。
...
レイヤーをまたぐエッジは `write` / `read` の2種のみ。task nodeの `writes` / `reads` フィールドとして記述する（V01-ADR-020）。
...
`trigger` / `reflect` / `hydrate` はV01-ADR-018/019のevent・transition・`transition.action` で表現済みのため廃止。
...
| State Diagram | `state`（FSM）/ `event` / `store` | 何がどんな状態を持ち、どう遷移するか（V01-ADR-019） |
