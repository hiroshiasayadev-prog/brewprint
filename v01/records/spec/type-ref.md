...
この一覧は V01-ADR-021 §3 の primitive 予約語を網羅したものである。V01-ADR-060 §1 の primitive 表は代表例であり、完全な一覧は本節を正とする。
...
> 由来: V01-ADR-021 §3, V01-ADR-060 §1
...
> 由来: V01-ADR-060 §2, §3; V01-ADR-070 §6〜§8
...
> 由来: V01-ADR-060 §1, V01-ADR-069 §1〜§6, §10

---

## 6. named list/dict model の正規化

V01-ADR-021 の named list/dict model は v1.1 でも有効である。
...
> 由来: V01-ADR-060 §6, §7, M15 Phase A task, V01-ADR-069 §2, §10
...
`enum` と `str` の暗黙互換は導入しない。`any` は V01-ADR-060 の既存ルール通り、source / target のどちらに現れても互換とする。
...
`any` は V01-ADR-060 の既存ルール通り、source / target のどちらに現れても互換とする。
...
> 由来: V01-ADR-073 §6, §8; V01-TASK-DATA-015-01
