...
- **source**: V01-ADR-060 (TypeRef + flow wiring type compatibility) / V01-ADR-061 (foreach.returns collected asset) / V01-ADR-062 (task return source) / V01-ADR-063 (initialized source の wiring source 化) / V01-ADR-064 (returns.source の DAG render ルール) / V01-ADR-067 (enum model) / V01-ADR-069 (TypeRef container complexity lint) を起点とする v1.1 系の minimum-expressiveness release
...
M14a (V01-ADR-058 / V01-ADR-059) で v1.0 系の実装と spec の乖離を解消した時点で、
brewprint の型表現力には以下の制約が残っていた。

- flow wiring における型互換性ルールが存在しない（V01-ADR-060 §背景）
- `any` の代入互換性が未定義
...
V01-ADR-060 はこのうち **TypeRef 導入と flow wiring type compatibility** を v1.1 の基礎として確定した。
V01-ADR-061 は **foreach.returns collected asset source** を Phase B として確定した。
V01-ADR-062 は **task return source の明示化** を Phase B2 として確定した。
V01-ADR-063 は **initialized source を returns.source および flow 内部 wiring の bare token source として参照可能にする** ことを Phase B3 として確定した。
V01-ADR-064 は **`returns.source` / initialized source 参照の DAG render ルール** を Phase B4 で扱う（起票時点 proposed）。
M15 は V01-ADR-060 / V01-ADR-061 / V01-ADR-062 / V01-ADR-063 / V01-ADR-064 を実装に落とし込みつつ、関連する v1.1 表現力拡張をまとめて扱う milestone である。

当初 M14b として独立 milestone を切る案もあったが、V01-ADR-060 が v1.1 TypeRef 前提に拡張された結果、
M14b 独立の意義が薄れ、本 milestone（M15）に吸収する判断をした（V01-ADR-060 §M15 への影響）。
...
- **subtyping / structural typing / user-defined generics の導入**
  - V01-ADR-060 §8 で明示的に却下。M15 もこの方針を踏襲する
- **variance rules の導入**
  - 同上
- **v1.0.0-spec タグの再発行 / v1.0 系 ADR の遡及修正**
  - V01-ADR-050 §7 / V01-ADR-057 Non-goals 準拠
...
- M15 完了時点で **`v1.1.0-spec`** タグを発行する（V01-ADR-060 §タグ発行方針）
...
- Phase A: TypeRef 導入 (V01-ADR-060)
- Phase B: foreach.returns collected asset (V01-ADR-061)
- Phase B2: task return source 明示化 (V01-ADR-062)
- Phase B3: initialized source の wiring source 化 (V01-ADR-063)
- Phase B4: returns.source の DAG render ルール (V01-ADR-064, proposed)
- Phase C: enum / discriminated object など追加表現力

### Phase A: TypeRef + flow wiring type validation

V01-ADR-060 を実装と spec に落とし込む。
...
- [x] **`docs/spec/edges.md` §1（flow:セクション）末尾に「§型互換性ルール」節を追加**
  - V01-ADR-060 §3 のルールを spec として記述
  - V01-ADR-060 §4 の検証対象 wiring 一覧
  - V01-ADR-060 §5 の wiring source 型解決ルール（node ID / `$params.<name>` / `$item`）
  - V01-ADR-060 §7 の型解決失敗時の抑制ルール
  - 由来: V01-ADR-060 §3〜§7, §9

- [x] **`docs/spec/edges.md` §1-5 foreach.over の記述を V01-ADR-060 と整合させる**
  - 現状: 「`$params.field` を指定した場合、parser/validator は `main.params.<field>.model` が `kind: list` であることを検証する」
  - V01-ADR-060 後: 「`foreach.over` の解決結果が list を表す TypeRef でない場合 `invalid_foreach_over_type` を出す」
...
- [ ] **`internal/resolve/validation.go` に `validateFlowWiringTypes` を追加**
  - validate phase で実行
  - V01-ADR-060 §3 の互換ルール、§5 の source 型解決、§7 の抑制ルールを実装
  - 由来: V01-ADR-060 §影響「既存実装への影響」
...
- [ ] **UC-001 回帰テスト**
  ...
  - 由来: V01-ADR-060 §影響「UC-001」

---

### Phase B: foreach.returns collected asset 参照ルール

V01-ADR-061 は accepted 済み。
...
- V01-ADR-023 の制御フロースコープは維持し、`foreach.returns` は collect 結果だけを外部 source として公開する escape hatch とする
- `foreach.id` は導入しない
- `task.returns.source` および main task `returns.name` と `foreach.returns` の名前一致による暗黙接続は本 Phase では扱わない。V01-ADR-062 領域として未実装のままにする

#### 仕様反映

- [x] **V01-ADR-061 を accepted として確定**
...
- [x] **`docs/spec/diagnostics.md` に V01-ADR-061 diagnostics を反映**
...
- [ ] **V01-ADR-062 領域の task return source は未実装のままにする**
...
V01-ADR-062 は accepted 済み。
`task.returns.source` によって、task の外向き return signature と内部 flow / input の source を明示的に接続する。

#### 確定した仕様範囲

> V01-ADR-063 により initialized source も追加される。詳細は Phase B3 を参照する。下記は V01-ADR-063 反映後の現行仕様である。
...
- `returns.source` と `returns.model` は V01-ADR-060 の TypeRef compatibility で検証する
...
- [x] **V01-ADR-062 を accepted として確認**
...
- [x] **`docs/spec/diagnostics.md` に V01-ADR-062 diagnostics を反映**
...
- [x] **return source TypeRef compatibility を検証**
  - source TypeRef と `returns.model` TypeRef を V01-ADR-060 ルールで比較
...
- [x] **MCP / renderer / resolved index への露出方針を確認**
  ...
  - DAG renderer での可視化は V01-ADR-064 / `docs/spec/views/dag.md` の `returns.source` render ルールに委ねる
...
- [x] **V01-ADR-062 実装後に Evidence を更新**
...
- [ ] **UC-001 回帰更新**
  ...
  - 既存 render / validate が V01-ADR-062 後も通ることを確認

---

### Phase B3: initialized source の wiring source 化

V01-ADR-063 は accepted 済み。
...
- V01-ADR-014 の外部参照不可ルールは維持する。外部 file / module 跨ぎ参照は引き続き不可
- DAG render ルールは V01-ADR-064（proposed）で扱う。本 phase では暫定方針（V01-ADR-064 §暫定方針）に従う

#### 仕様反映

- [x] **V01-ADR-063 を accepted として確認**
...
- [x] **`docs/spec/nodes.md` の `init オブジェクト` 末尾に bare wiring source 化を追記**
  ...
  - 外部 file / module 跨ぎ参照は V01-ADR-014 通り引き続き不可
...
- [x] **TypeRef compatibility を initialized source にも適用**
  - `initializes[].model` を named model TypeRef として扱い V01-ADR-060 ルールで検証
...
- [ ] **DAG render は V01-ADR-064 §暫定方針に従う**
...
- [x] **V01-ADR-063 実装後に Evidence を更新**
...
- [x] **flow 内部 wiring から initialized source を bare token で参照できる**
  ...
  - TypeRef compatibility が V01-ADR-060 ルールで通る
...
- [ ] **V01-ADR-064 §論点 各案を実 render で比較**
  - 論点1（edge 表現）の各案
  - 論点2（ID 衝突回避）の各案
...
- [ ] **V01-ADR-064 を accepted に確定**
  - 比較結果から採用案を決め、V01-ADR-064 を accepted へ
...
- **`list<T>` / `dict<T>` の深さ制限または lint 方針** (V01-ADR-060 §1 後段)
...
- V01-ADR-060: v1.1 TypeRef と flow wiring の型互換性 (accepted)
- V01-ADR-061: foreach.returns collected asset 参照ルール (accepted)
- V01-ADR-062: task return source の明示化 (accepted)
- V01-ADR-063: task return source への initialized source 追加と initializes の wiring source 化 (accepted)
- V01-ADR-064: returns.source の DAG render ルール (proposed)
- V01-ADR-014: initializes フィールド設計（v1.1 で wiring source 対応に拡張、V01-ADR-063）
- V01-ADR-020: cross-edge management（v1.1 で flow wiring との分担を V01-ADR-063 §7 で整理）
- V01-ADR-021: primitive 予約語と list/dict model（v1.1 でも維持）
- V01-ADR-016: foreach 構文（変更なし。`$item` 型解決ルールは V01-ADR-060 で明文化）
- V01-ADR-058 / V01-ADR-059: M14a で確定した v1.0 系の実装バグ修正
- V01-ADR-050: spec-first documentation policy
- V01-ADR-057: self-hosting 思想 / Non-goals
...
Actual close boundary is defined by `V01-REQ-DATA-001` / `V01-WORK-DATA-001`, not by the stale phase checkboxes in this legacy document.

Closed scope:

- Phase A〜B3: V01-ADR-060〜V01-ADR-063 baseline implementation / tests / evidence confirmed.
- Phase B4: V01-ADR-064 accepted DAG render behavior implemented, UC-001 current renders regenerated, and regression verified.
- V01-ADR-069 minimum: parser safety limit, anonymous inline struct non-adoption, and `opaque_type_ref` warning reflected in spec / implementation / tests.
- V01-ADR-067 enum minimum: `mcp_object_type`, `mcp_diagnostic_severity`, `reference_tree_direction` introduced as enum models, and exactly the initial 5 UC-002 fields migrated to named enum TypeRef.
...
- `go test ./internal/resolve`: passed during V01-ADR-067 / V01-ADR-069 verification.
- `go test ./...`: passed.

Deferred outside this M15 close:

- V01-ADR-070 / V01-ADR-071 / V01-ADR-072 / V01-ADR-075 helper model / model render series.
- V01-ADR-073 tagged union.
- V01-ADR-074 DAG asset TypeRef hint.
- V01-ADR-078〜080 MCP semantic identity / state machine identity.
...
- commit: 01e7127
- impl commit: e7b8292
- close work item: V01-WORK-DATA-001
- close task: V01-TASK-DATA-001-05
