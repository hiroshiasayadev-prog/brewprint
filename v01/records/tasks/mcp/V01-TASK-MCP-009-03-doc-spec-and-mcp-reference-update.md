# V01-TASK-MCP-009-03: Doc, spec, and MCP reference update

- **id**: V01-TASK-MCP-009-03
- **status**: done
- **date**: 2026-06-02
...
V01-TASK-MCP-009-02 の cleanup policy decision に従い、必要な docs / spec / MCP reference updates を適用する。

この task は authoring guidance canonicalization と legacy cleanup の反映を扱う。V01-REQ-MCP-008 authoring transaction implementation には拡張しない。

## Work

- V01-TASK-MCP-009-02 の per-file classification を確認し、更新対象と非対象を明確にする。
...
- 更新対象が implementation / tests に及ぶ場合も、V01-REQ-MCP-008 の proposal / accept / authoring transaction tool implementation には踏み込まない。

## Done condition

- V01-TASK-MCP-009-02 の cleanup policy decision に従った docs / spec / MCP reference updates が完了している。
...
- V01-REQ-MCP-008 authoring transaction implementation に scope creep していない。

## Verification

- 更新した files と、V01-TASK-MCP-009-02 classification との対応を Evidence に記録する。
...
- Rerun `validate_records()` after the additional cleanup returned `ok: false` due to existing repository-wide diagnostics outside this cleanup scope: info-level investigation follow-up candidate diagnostics in `V01-INV-DATA-001` / `V01-INV-DATA-002`, and missing required metadata errors in existing `V01-TASK-MCP-005-01` through `V01-TASK-MCP-005-03`.
- Targeted rerun `validate_records(kind=task, id_range=V01-TASK-MCP-009-03..V01-TASK-MCP-009-03)` returned `ok: true`, `diagnostics: null`.
- Targeted rerun `validate_records(kind=investigation)` returned `ok: true` with the existing info-level `V01-INV-DATA-001` / `V01-INV-DATA-002` follow-up candidate diagnostics.
...
- `validate_records(kind=task, id_range=V01-TASK-MCP-009-03..V01-TASK-MCP-009-03)` returned `ok: true`, `diagnostics: null`.
...
- Did not reopen V01-REQ-MCP-008 or V01-WORK-MCP-006.
- Did not close V01-WORK-MCP-009 / V01-REQ-MCP-009.
