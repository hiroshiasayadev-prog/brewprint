# V01-TASK-MCP-007-02: Workflow artifact range filter contract decision

- **id**: V01-TASK-MCP-007-02
- **status**: done
- **date**: 2026-06-01
...
- `V01-REQ-MCP-007` が要求する workflow artifact navigation が満たせることを確認している。
- Cross-domain / mixed-kind comparison を silent reinterpretation しない拒否方針が決まっている。
- `V01-TASK-MCP-007-03` が spec update に進める粒度で decision result が残っている。

## Verification

- Decision result が `V01-REQ-MCP-007` の Acceptance expectation と矛盾しないことを確認する。
...
- Example valid range: `V01-TASK-MCP-007-01` .. `V01-TASK-MCP-007-05`.
- Example invalid range: `V01-TASK-MCP-006-01` .. `V01-TASK-MCP-007-05`.
...
- Example: `kind: work_item`, `id_range.from: V01-WORK-DATA-004` lists `WORK-DATA-*` records whose sequence is >= 004.
- Example: omitted `kind`, `id_range.from: V01-WORK-DATA-004` behaves as `kind: work_item` scoped to `DATA`.
...
- Mixed families are invalid: e.g. `V01-REQ-MCP-001` .. `V01-WORK-MCP-001`.
- Mixed domains are invalid: e.g. `V01-WORK-DATA-001` .. `V01-WORK-MCP-010`.
- Mixed task work sequences are invalid: e.g. `V01-TASK-MCP-006-01` .. `V01-TASK-MCP-007-05`.
...
- Existing error code `id_range_requires_decision_kind` should be replaced or generalized in spec / implementation, because the selected contract no longer means decision-only. Suggested direction: introduce `invalid_id_range` for malformed, mixed, or unsupported range endpoints. Exact naming can be finalized in `V01-TASK-MCP-007-03` spec update.

Examples expected after implementation:

Valid:

- `list_records(id_range: V01-ADR-067..V01-ADR-077)` -> decision records in numeric ADR range.
- `list_records(kind: requirement, id_range: V01-REQ-MCP-001..V01-REQ-MCP-007)` -> MCP requirements in numeric sequence range.
- `list_records(kind: work_item, id_range: V01-WORK-DATA-001..V01-WORK-DATA-004)` -> DATA work items in numeric sequence range.
- `list_records(kind: task, id_range: V01-TASK-MCP-007-01..V01-TASK-MCP-007-05)` -> tasks for `V01-WORK-MCP-007` in task sequence range.
- `list_records(id_range: V01-WORK-DATA-004..V01-WORK-DATA-004)` -> exact same-family range for `V01-WORK-DATA-004`.

Invalid:

- `list_records(kind: work_item, id_range: V01-WORK-DATA-001..V01-WORK-MCP-010)`.
- `list_records(kind: requirement, id_range: V01-REQ-DATA-001..V01-REQ-MCP-010)`.
- `list_records(kind: task, id_range: V01-TASK-MCP-006-01..V01-TASK-MCP-007-05)`.
- `list_records(id_range: V01-REQ-MCP-001..V01-TASK-MCP-001-01)`.
- `list_records(id_range: SPEC-design-records-mcp-tools..SPEC-design-records-mcp-schema)`.
- `list_records(id_range: V01-INV-DOCS-001..V01-INV-DOCS-010)`.

Verification:

- This decision satisfies `V01-REQ-MCP-007` because workflow artifacts can be listed by safe family/domain-scoped ID ranges without relying on filesystem listing.
- The decision preserves the existing ADR range behavior and narrows new workflow ordering to explicit same-family scopes.
- `V01-TASK-MCP-007-03` can now update the public tools spec from this contract.
