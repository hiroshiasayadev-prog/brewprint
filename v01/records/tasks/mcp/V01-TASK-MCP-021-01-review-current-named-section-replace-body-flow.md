# V01-TASK-MCP-021-01: review current named_section_replace body flow

- **id**: V01-TASK-MCP-021-01
- **status**: done
- **date**: 2026-06-05
...
- Check interactions with V01-REQ-MCP-021 heading canonicalization work and avoid making this task depend on tolerant selector matching.
...
- Any dependency or collision with V01-REQ-MCP-021 / V01-WORK-MCP-019 is explicitly noted.
...
- Important implementation risk: `prepareUpdate` currently treats any diagnostics returned from `replaceNamedSection` as proposal-blocking. V01-REQ-MCP-022 needs warning diagnostics to be retained and surfaced without blocking proposal creation. Error-severity diagnostics should still block proposal creation.
...
- V01-REQ-MCP-022 can be implemented independently from V01-REQ-MCP-021 / V01-WORK-MCP-019 because it operates after section selection has already succeeded.
...
Proceed to `V01-TASK-MCP-021-02` to update the MCP authoring contract and diagnostic documentation before implementation.
