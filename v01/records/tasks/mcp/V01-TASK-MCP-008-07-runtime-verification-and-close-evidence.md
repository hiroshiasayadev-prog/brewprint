# V01-TASK-MCP-008-07: Runtime verification and close evidence

- **id**: V01-TASK-MCP-008-07
- **status**: done
- **date**: 2026-06-02
...
- **outputs**:
  ...
  - V01-WORK-MCP-008 close evidence
  - V01-REQ-MCP-008 close evidence

## Goal

`V01-TASK-MCP-008-06` で実装された Design Records MCP authoring transaction MVP を runtime MCP stdio / JSON-RPC 経由で検証し、V01-WORK-MCP-008 / V01-REQ-MCP-008 を close できる evidence を揃える。
...
- Do not mark `V01-WORK-MCP-008` or `V01-REQ-MCP-008` done unless the user explicitly asks for close after reviewing evidence.
...
- validate_records still passes for `V01-ADR-093`, `V01-REQ-MCP-008`, `V01-WORK-MCP-008`, and this task.
...
- V01-WORK-MCP-008 and V01-REQ-MCP-008 are ready for close review.
...
- Untracked MCP authoring transaction artifacts included `V01-ADR-093`, `V01-REQ-MCP-008`, `V01-REQ-MCP-010`, `V01-TASK-MCP-008-01` through `V01-TASK-MCP-008-07`, `V01-WORK-MCP-008`, and implementation files `internal/designrecords/authoring.go` / `internal/designrecords/authoring_test.go`.
...
- Request: `kind: task`, `id: TASK-MCP-001-new`, `parent_id: V01-WORK-MCP-001`.
- Result: `proposal_created: true`.
- Proposal ID was returned.
- Target resolved to `V01-TASK-MCP-001-02`.
...
- `V01-ADR-093`: `ok: true`, diagnostics `null`.
- `V01-REQ-MCP-008`: `ok: true`, diagnostics `null`.
- `V01-WORK-MCP-008`: `ok: true`, diagnostics `null`.
- `V01-TASK-MCP-008-07`: `ok: true`, diagnostics `null`.
...
- Smoke coverage was representative, not exhaustive; deeper stale-target / collision / expiry behavior remains covered by targeted tests from `V01-TASK-MCP-008-06`.
...
- `V01-REQ-MCP-008` was not marked accepted / done.

### Close readiness

`V01-WORK-MCP-008` and `V01-REQ-MCP-008` are ready for close review based on accepted ADR coverage, spec reflection, implementation tests, runtime MCP smoke verification, and successful Design Records validation.
