# V01-TASK-MCP-002-06: Evidence / status と dogfooding 所感を反映する

- **id**: V01-TASK-MCP-002-06
- **status**: done
- **date**: 2026-05-26
...
- **depends_on**:
  - V01-TASK-MCP-002-05
- **outputs**:
  - V01-REQ-MCP-002 / V01-WORK-MCP-002 の完了状態反映
...
- Capability の採用結果と verification evidence を V01-REQ-MCP-002 / V01-WORK-MCP-002 に反映する。
...
- `V01-REQ-MCP-002` は `accepted` のまま維持する。Requirement status は採用判断を表し、implementation / runtime verification の完了は `V01-WORK-MCP-002` の close evidence で追跡する運用に従う。
- `V01-ADR-090`、`docs/spec/design-records-mcp/{overview,schema,tools}.md`、`internal/designrecords` / `internal/designrecordsmcp` の implementation と tests、`V01-TASK-MCP-002-05` の runtime evidence が揃ったため、`V01-WORK-MCP-002` は done へ進められる。
...
- 小粒度 task の分割は有効だった。利用価値確認（T1）、採用判断と ADR（T2）、spec contract 固定（T3）、implementation（T4）、runtime verification（T5）、close review（T6）が分かれたことで、V01-ADR-090 review で発見された schema の未確定点を実装前に解消できた。
- `decision_pending` / `design_spec_pending` / `implementation_pending` / `verification_pending` の blocker 表現は、次に何を確定すべきかを追いやすかった。
- Mermaid task flow は依存順と close 条件を共有するには有用だった。一方、この dogfooding 時点では workflow artifact が MCP 取得対象外であり、task ID から task record を取得できないため追加探索が発生した。これは physical path を relation として保持する理由ではなく、`V01-REQ-MCP-003` が扱う ID-as-ref query / resolve support の evidence とする。
...
- その後のレビューと明示判断により、task 粒度、status ownership、Mermaid flow、旧 milestone 役割を work item が引き取る境界、workflow relation の ID-as-ref / physical path 非対応は `V01-ADR-091` として accepted になった。`milestone` を新しい artifact layer、metadata field、または relation として導入しない。
- Implementation/test の別実行経路への委譲は V01-ADR-091 の確定範囲には含めず、引き続き運用上の候補として扱う。
