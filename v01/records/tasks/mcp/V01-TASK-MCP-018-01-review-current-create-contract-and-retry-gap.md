# V01-TASK-MCP-018-01: Review current create contract and retry gap

- **id**: V01-TASK-MCP-018-01
- **status**: done
- **date**: 2026-06-03
...
`propose_record_create` の現行 contract、legacy full-record create mode、`fields + body_cache_id` retry gap を確認し、V01-REQ-MCP-019 の実装前提を固める。

## Work

- `SPEC-design-records-mcp-tools` の `propose_record_create` input contract を確認する。
- `V01-REQ-MCP-014` / `V01-WORK-MCP-014` の close evidence と現行実装の前提を確認する。
- `V01-REQ-MCP-015` / `V01-WORK-MCP-012` の failed propose body_cache behavior を確認する。
...
- 不明点や仕様判断待ちがある場合は、V01-TASK-MCP-018-02 に渡せる形で明示する。
...
- The review output was carried forward into `V01-TASK-MCP-018-02` impact assessment and subsequent spec / implementation tasks.
