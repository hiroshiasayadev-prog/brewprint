...
- **outputs**:
  ...
  - implementation recommendations for V01-TASK-MCP-025-03

## Goal

Document the current behavior of `propose_record_create` validation paths that are in scope for V01-REQ-MCP-024 and V01-REQ-MCP-028, and identify what changes are needed.

## Work

- Trace `renderWorkflowMetadata` / `renderADRMetadata` to confirm first-missing-field-only behavior.
- Confirm `Diagnostic` struct fields currently available vs. fields required by V01-REQ-MCP-024 (`allowed_values`, `required_fields`, `target_kind`, `repair_suggestion`).
...
- Cross-check findings against V01-REQ-MCP-024 and V01-REQ-MCP-028 acceptance criteria.
