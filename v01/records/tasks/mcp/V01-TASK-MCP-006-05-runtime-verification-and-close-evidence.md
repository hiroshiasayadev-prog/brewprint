# V01-TASK-MCP-006-05: Runtime verification と close evidence を整理する

- **id**: V01-TASK-MCP-006-05
- **status**: done
- **date**: 2026-06-01
...
- **outputs**:
  ...
  - Close evidence for V01-REQ-MCP-006 / V01-WORK-MCP-006

## Goal

V01-WORK-MCP-006 の完了判断に必要な runtime verification と close evidence を整理し、V01-REQ-MCP-006 / V01-WORK-MCP-006 を close 可能な状態にする。
...
1. V01-TASK-MCP-006-03 / V01-TASK-MCP-006-04 の変更結果を確認する。
...
5. V01-REQ-MCP-006 / V01-WORK-MCP-006 の status 更新案と close evidence を作成する。
6. 必要なら follow-up candidate を分離し、V01-WORK-MCP-006 の scope に混ぜない。

## Expected output

- Runtime verification command / result。
- Close evidence summary。
- V01-REQ-MCP-006 / V01-WORK-MCP-006 の status 更新案。
- 残す follow-up がある場合は、その理由と scope boundary。

## Completion condition

V01-WORK-MCP-006 の completion condition を満たす evidence が揃い、close 判断に進める。

## Verification

- `go test -count=1 ./internal/designrecords ./internal/designrecordsmcp` passed during V01-TASK-MCP-006-04 implementation review.
...
- `diagnostics: null` remains an existing response-shape behavior and is explicitly out of scope for V01-WORK-MCP-006.

## Close review

V01-WORK-MCP-006 completion condition is satisfied.

Completed task chain:

- `V01-TASK-MCP-006-01`: current validator behavior and metadata field gap review completed.
- `V01-TASK-MCP-006-02`: workflow metadata strictness contract decision completed.
- `V01-TASK-MCP-006-03`: Design Records MCP schema / tools spec and authoring guidance updated and reviewed.
- `V01-TASK-MCP-006-04`: parser / validator / tests implemented and reviewed.
- `V01-TASK-MCP-006-05`: runtime verification and close evidence completed.
...
- README / legacy authoring guide cleanup, tracked separately by `V01-REQ-MCP-009`.
...
- `V01-TASK-MCP-006-05` can be marked `done`.
- `V01-WORK-MCP-006` can be marked `done`.
- `V01-REQ-MCP-006` remains `accepted`; close evidence is recorded on the requirement because requirement status has no `done` state.
