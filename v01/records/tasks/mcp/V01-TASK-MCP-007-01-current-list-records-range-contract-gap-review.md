# V01-TASK-MCP-007-01: Current list_records range contract gap review

- **id**: V01-TASK-MCP-007-01
- **status**: done
- **date**: 2026-06-01
...
- `V01-REQ-MCP-007` の要求に対して、現行 contract と implementation の gap を分類する。
...
- `list_records(id_range: V01-ADR-067..V01-ADR-077, order_by: id, order: asc, limit: 5)` returned ADR records starting at `V01-ADR-067`; runtime behavior matches the ADR-only range contract.
- `list_records(kind: work_item, id_range: V01-WORK-DATA-001..V01-WORK-DATA-004)` returned tool error `id_range_requires_decision_kind` with message `id_range requires kind decision`.
- `list_records(id_range: V01-WORK-DATA-001..V01-WORK-DATA-004)` returned tool error `id_range_requires_decision_kind` with message `id_range endpoints must use ADR-NNN decision IDs`.
- `list_records(kind: work_item, order_by: id, order: asc, limit: 10)` returned `V01-WORK-DATA-001` through `V01-WORK-DATA-010`, showing workflow artifact listing works without range, but range narrowing is unavailable.
...
- This is a public contract gap / usability gap captured by `V01-REQ-MCP-007`.
- `V01-WORK-DATA-004` occupancy can be discovered by exact ID lookup or broad `kind: work_item` listing, but cannot be expressed as a safe bounded workflow artifact range query.

Decision points for `V01-TASK-MCP-007-02`:
