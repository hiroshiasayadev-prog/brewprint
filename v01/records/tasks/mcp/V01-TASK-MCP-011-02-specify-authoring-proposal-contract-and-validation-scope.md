# V01-TASK-MCP-011-02: Specify authoring proposal contract and validation scope

- **id**: V01-TASK-MCP-011-02
- **status**: done
- **date**: 2026-06-02
...
- **outputs**:
  ...
  - Spec evidence that separates V01-REQ-MCP-011 and V01-REQ-MCP-012 behavior from implementation fixes

## Goal

Update the Design Records MCP public spec contract for authoring proposal behavior using the evidence from `V01-TASK-MCP-011-01`.
...
- Check whether `V01-ADR-093` remains consistent with the specified authoring transaction contract. If the transaction model itself must change, record the ADR follow-up instead of silently changing the spec beyond the ADR boundary.
...
- `SPEC-design-records-mcp-tools` and/or `SPEC-design-records-mcp-schema` define the public behavior for all four failures reproduced in `V01-TASK-MCP-011-01`:
...
- The spec text preserves the boundary between `V01-REQ-MCP-011` input normalization and `V01-REQ-MCP-012` validation scope isolation.
- Any ADR consistency concern is either ruled out with evidence or captured as a follow-up before implementation proceeds.
- The resulting spec contract is precise enough for `V01-TASK-MCP-011-03` to implement without guessing.
...
```powershell
validate_records(id_range={from: "V01-TASK-MCP-011-01", to: "V01-TASK-MCP-011-02"}, kind: task)
validate_records(id: "V01-WORK-MCP-011", kind: work_item)
```
...
- `V01-REQ-MCP-011` owns input normalization: body/fields, top-level `id` versus `fields.id`, and domain normalization.
- `V01-REQ-MCP-012` owns validation scope isolation and unrelated diagnostic separation.
- `V01-ADR-093` remains consistent. It already defines artifact identity as the primary write input, body/body_cache mutual exclusion, proposal validation result semantics, and tools-spec ownership for concrete schemas. No ADR follow-up is needed for this contract update.
...
- `validate_records(kind=task, id_range={from:"V01-TASK-MCP-011-01", to:"V01-TASK-MCP-011-02"})`: `ok:true`, no diagnostics.
- `validate_records(kind=work_item, id_range={from:"V01-WORK-MCP-011", to:"V01-WORK-MCP-011"})`: `ok:true`, no diagnostics.
- `validate_records(kind=requirement, id_range={from:"V01-REQ-MCP-011", to:"V01-REQ-MCP-012"})`: `ok:true`, no diagnostics.
