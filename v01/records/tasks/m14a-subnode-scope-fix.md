...
- **source**: M14 Phase A中に発覚した実装バグ。V01-ADR-058 / V01-ADR-059 で決定
...
V01-ADR-057でv1.0.0-spec凍結後、M14 Phase A（MCP公開contract のblueprint化）に着手したところ、
既存実装と既存specの間に2系統の乖離が判明した。

**B1: サブノードのファイル内private性が実装で保証されていない**

- V01-ADR-011 §3 / spec/nodes.md「ファイル構造」がサブノードはファイル内privateと規定
...
- V01-ADR-021 §3 / spec/nodes.md primitive予約語節が `any` 等7語の primitive を定義
...
このmilestoneは V01-ADR-058 / V01-ADR-059 の方針に従い、実装をspecに追従させる純粋なbug fix milestoneとして扱う。
...
- V01-ADR-011 / V01-ADR-021 / V01-ADR-027 / spec/nodes.md ファイル構造節の遡及修正
  - doc-policy.md §3 / V01-ADR-058 §理由 / V01-ADR-059 §理由
- v1.0.0-spec タグの再発行
...
- [ ] **`docs/spec/nodes.md` ファイル構造節を更新する** (V01-ADR-058 §4)
  - 「サブノードはファイル内private」を以下まで強化する
    - サブノードIDは同一ファイル内で一意
    - 別ファイルにある同名サブノードとは衝突しない
    - サブノードは QualifiedID で外部参照されない
  - 由来注記に V01-ADR-058 を追加

- [ ] **`docs/spec/nodes.md` task節 / `returns` オブジェクト節を更新する** (V01-ADR-059 §3)
  - `model` に primitive を指定可能であることを明記
  - 由来注記に V01-ADR-059 を追加

- [ ] **`docs/spec/diagnostics.md` を更新する** (V01-ADR-058 §4 / V01-ADR-059 §3)
  - `duplicate_node`: メインノード間衝突に限定する旨を明記、由来に V01-ADR-058 追加
  - `unresolved_model`: primitive は対象外である旨を明記、由来に V01-ADR-059 追加

- [ ] **`docs/spec/naming.md` を更新する** (V01-ADR-058 §4)
  - メインノード = 外部参照可能な QualifiedID 保持者
  - サブノード = ファイル内private、QualifiedIDによる外部参照対象外
  - §2 周辺に追記、由来に V01-ADR-058 追加
...
- [ ] **flow / reads / writes / param の解決ロジックを確認する**
  ...
  - 解決順序がV01-ADR-058 §3 と整合していることを再確認する
...
- [ ] **`docs/impl/go-m14a-summary.md` を起票する**
  - 修正内容、追加した回帰テスト、V01-ADR-058 / V01-ADR-059との対応関係を記録
...
- [ ] **`v1.0.1-spec` タグを発行する** (V01-ADR-058 §影響 / V01-ADR-059 §影響)
...
- B1: サブノードの file-private scope bug は `V01-REQ-RESOLVE-001` / `V01-WORK-RESOLVE-001` / `V01-TASK-RESOLVE-001-01..03` に移行して解消した。
  - V01-ADR-058 の spec alignment は `docs/spec/nodes.md` / `docs/spec/naming.md` / `docs/spec/diagnostics.md` / `docs/spec/edges.md` に反映済み。
...
- B2: task / join return primitive support は後続 v1.1 系 TypeRef / return handling に回収済み。
  ...
  - M15 / `v1.1.0-spec` は `V01-REQ-DATA-001` / `V01-WORK-DATA-001` により close 済み。
...
## V01-ADR-058 / V01-ADR-059 との対応

| ADR | 決定 | M14a task |
|---|---|---|
| V01-ADR-058 §1 | サブノードはQID一意性制約から除外 | impl修正 B1 — `addNode` 修正 |
| V01-ADR-058 §2 | サブノードの内部識別 | impl修正 B1 — 内部識別経路の整理 |
| V01-ADR-058 §3 | ファイル内bare ID解決のスコープ明示 | impl修正 B1 — flow解決確認 / 仕様反映 — naming.md |
| V01-ADR-058 §4 | spec更新 (nodes / diagnostics / naming) | 仕様反映セクション |
| V01-ADR-058 §5 | 実装方針（参考） | impl修正 B1 全体の指針 |
| V01-ADR-059 §1 | task / join return が primitive を許容 | impl修正 B2 — `validateReturn` 修正 |
| V01-ADR-059 §2 | validation の対称化 | impl修正 B2 — 同上 |
| V01-ADR-059 §3 | spec更新 (nodes / diagnostics) | 仕様反映セクション |
...
1. **M15: data layer expressiveness v1.1**
   - V01-ADR-060: model field enum type
   - V01-ADR-061: discriminated object model kind
   - V01-ADR-062: inline struct field type
   - free union (untagged) は導入しない方針 (V01-ADR-061で却下案として明記予定)
