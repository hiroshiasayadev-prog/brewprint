...
- **source**: V01-ADR-076 (Design Records MCP) / V01-ADR-077 (MVP boundary and tool prioritization) / docs/spec/design-records-mcp/**
...
V01-ADR-050 により、brewprint のドキュメント運用は spec-first に転換した。
現行仕様は `docs/spec/**` に置き、ADR は設計判断の根拠記録として扱う。

V01-ADR-076 では、ADR/spec 運用を machine-readable metadata と MCP query / validation で支援する Design Records MCP の導入方針を決めた。
V01-ADR-077 では、MVP の tool 優先度と read-only 境界を決めた。
...
- [x] V01-ADR-050 / V01-ADR-067〜V01-ADR-077 が decision record として index できる
...
- [x] V01-ADR-067〜V01-ADR-077 を `id_range` で取得できる
