...
- [x] **`get_reference_tree` または depth指定つきreference traversalを設計する**
  ...
  - V01-ADR-055で `get_references` direct only維持 + `get_reference_tree` 別toolを採用済み
  - V01-ADR-055で cycle detection / max depth / kind filter / direction の方針を確定済み
  - V01-ADR-049のdirect reference方針はsupersedeせず、V01-ADR-055で拡張する
  - `docs/spec/mcp/tools/get-reference-tree.md` へ正式仕様を反映済み
  - 実装タスクは次セッション以降で分割する

- [x] **`analyze_impact` を設計する**
  - 設計完了: V01-ADR-056 で全体方針を確定させ、 `docs/spec/mcp/tools/analyze-impact.md` に仕様反映済み
