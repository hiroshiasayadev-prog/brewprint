# V01-TASK-DATA-006-01: UC-002 helper-shape migration set selection

- **id**: V01-TASK-DATA-006-01
- **status**: done
- **date**: 2026-06-01
...
- **outputs**:
  ...
  - Boundary for V01-TASK-DATA-006-02 YAML migration

## Goal

Select the exact UC-002 helper-shape migration set for V01-WORK-DATA-006 before any UC-002 YAML, fixture, golden output, or Go implementation changes.

This task converts the V01-TASK-DATA-003-04 candidate inventory and the V01-WORK-DATA-004 / V01-REQ-DATA-003 task-file signature policy into a concrete migration boundary for the next task.

## Work

- Review the V01-TASK-DATA-003-04 model-file helper migration candidates.
- Use the V01-WORK-DATA-004 / V01-REQ-DATA-003 task-file private helper signature policy as a constraint.
...
These candidates are selected for V01-TASK-DATA-006-02.
...
The 8 UC-002 MCP task files keep the current `query_service.returns.model: any` to `build_response.params[].model: any` pattern for V01-WORK-DATA-006.
...
These candidates are not migrated to same-file task-file private helper models in V01-WORK-DATA-006. `returns.model` alone could use a task-file private helper, but using the same private helper from `build_response.params[].model` would violate the accepted DATA-004 / V01-REQ-DATA-003 policy, where task `params[].model` references to same-file private helper models are invalid.
...
The following categories remain outside this task and outside V01-TASK-DATA-006-02:
...
- V01-TASK-DATA-006-02 can start from this selected set without reopening V01-WORK-DATA-002, V01-WORK-DATA-003, V01-WORK-DATA-004, tagged union, MCP identity, or M15 scope.
...
- Confirm selected candidates match the model-file helper migration candidates recorded by V01-TASK-DATA-003-04.
- Confirm task-file `query_result:any` candidates are not selected for migration because of the DATA-004 / V01-REQ-DATA-003 `params[].model` private-helper policy.
- Confirm V01-WORK-DATA-006 is the parent work item.
