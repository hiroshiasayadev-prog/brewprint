# V01-TASK-DATA-010-02: V01-ADR-073 revision and spec / diagnostics alignment

- **id**: V01-TASK-DATA-010-02
- **status**: done
- **date**: 2026-06-01
...
- **outputs**:
  - Revised and accepted V01-ADR-073 or explicit blocker note
...
Revise V01-ADR-073 so that it can be accepted under the current V01-REQ-DATA-004 / V01-WORK-DATA-010 successor boundary, then align the active specs for the accepted tagged union / discriminator payload minimum.

This task exists because V01-TASK-DATA-010-01 concluded that V01-ADR-073 does not need a conceptual split, but should not be accepted as-is while it still contains stale M15 wording and while active specs do not yet define `kind: tagged_union`.

## Work

- Review V01-TASK-DATA-010-01 evidence before editing.
- Revise V01-ADR-073 to remove or update stale M15 / Phase C / close-scope wording.
- Make V01-REQ-DATA-004 / V01-WORK-DATA-010 the active acceptance boundary for V01-ADR-073.
- Move V01-ADR-073 from `proposed` to `accepted` only if the revised text cleanly preserves the minimum tagged union contract and excluded scope.
- Ensure V01-ADR-073 acceptance does not imply tagged union render support is already implemented or that all remaining V01-ADR-075 proposed text is accepted.
...
- Update `docs/spec/type-ref.md` for tagged union TypeRef behavior:
  ...
  - `any` compatibility follows existing V01-ADR-060 behavior
...
- V01-ADR-073 wording revision and status update if acceptance conditions are met.
...
- V01-ADR-073 is either accepted with updated successor-boundary wording or left proposed with an explicit blocker recorded.
...
- Confirm V01-ADR-073 no longer implies M15 / V01-WORK-DATA-003 reopening.
- Confirm V01-ADR-073 acceptance, if performed, does not accept V01-ADR-075 tagged-union render details as already implemented.
...
- `V01-TASK-DATA-010-01`: confirmed verdict `revise-before-acceptance`, no conceptual split required, and follow-up scope for ADR / spec / diagnostics alignment.
- `V01-ADR-073`: confirmed already accepted, with stale M15 / v1.1 wording that needed successor-boundary cleanup.
- `V01-REQ-DATA-004`: confirmed tagged union / discriminator payload support requirement and explicit excluded scope.
- `V01-WORK-DATA-010`: confirmed ownership of tagged union successor work and exclusion of DAG TypeRef hint, MCP identity, duplicate task QID repair, and broad notes-retreat cleanup.
- `docs/spec/nodes.md`, `docs/spec/type-ref.md`, `docs/spec/diagnostics.md`: active specs updated by this task.

### Changes made

- Updated `V01-ADR-073` wording so the accepted contract is framed as V01-REQ-DATA-004 / V01-WORK-DATA-010 successor scope rather than M15 Phase C / v1.1 initial adoption scope.
...
- V01-ADR-073 no longer frames the accepted minimum as M15 Phase C reopening.
- V01-ADR-073 remains accepted and explicitly leaves runtime MCP request / response payload validation out of scope.
...
- Tagged union render support is not marked as already implemented; it remains follow-up scope under V01-WORK-DATA-010 or later render-specific work.
...
- TypeRef compatibility confirmation that tagged union models remain nominal named models and `any` keeps V01-ADR-060 wildcard behavior.
