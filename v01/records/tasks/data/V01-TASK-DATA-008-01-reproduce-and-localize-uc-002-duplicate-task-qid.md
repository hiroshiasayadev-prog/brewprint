# V01-TASK-DATA-008-01: Reproduce and localize UC-002 duplicate task QID issue

- **id**: V01-TASK-DATA-008-01
- **status**: done
- **date**: 2026-06-01
- **work_item**: V01-WORK-DATA-008
...
- Read the repo instructions and the current V01-WORK-DATA-008 boundary before running commands.
...
- Confirm the reproduction does not require reopening M15, V01-WORK-DATA-001, V01-WORK-DATA-002, V01-WORK-DATA-003, or V01-WORK-DATA-004.
- Confirm V01-ADR-073 tagged union support, V01-ADR-074 DAG asset TypeRef hint, V01-ADR-078 / V01-ADR-079 / V01-ADR-080 MCP identity, broad UC-002 notes retreat cleanup, and helper model / model-file render redesign remain outside this task.
...
Not YAML identity problem: V01-ADR-058 and the current specs say file-private subnodes may share local IDs across files. The UC-002 YAML pattern is valid.
...
`V01-WORK-DATA-008` and this task were created from stale M15 close evidence that still treated the UC-002 duplicate task QID / unresolved flow task issue as unresolved.

Current evidence shows the issue was already resolved by the later `V01-WORK-RESOLVE-001` / ADR-058-aligned resolver behavior. Therefore no DATA YAML, resolver, renderer, validator, fixture, golden, MCP contract, V01-ADR-073, V01-ADR-074, V01-ADR-078, V01-ADR-079, or V01-ADR-080 work is needed for this blocker.
