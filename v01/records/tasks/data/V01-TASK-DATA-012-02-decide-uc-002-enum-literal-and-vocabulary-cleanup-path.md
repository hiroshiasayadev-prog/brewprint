# V01-TASK-DATA-012-02: Decide UC-002 enum, literal, and vocabulary cleanup path

- **id**: V01-TASK-DATA-012-02
- **status**: done
- **date**: 2026-06-05
...
- **outputs**:
  - Cleanup path decision table for V01-WORK-DATA-012 candidates
...
- Confirm selected outcomes do not route default/behavior scope into `V01-WORK-DATA-012`.
- Confirm selected outcomes do not route selector/support-matrix scope into `V01-WORK-DATA-012`.
- Confirm selected outcomes do not reopen M15, `V01-WORK-DATA-001`, `V01-WORK-DATA-006`, `V01-WORK-DATA-009`, or `V01-WORK-DATA-010`.
- Run Design Records MCP validation for this task and `V01-WORK-DATA-012` after creation.
...
- N-023 remains note-only. `language = yaml` is a literal constraint, not a finite selection enum in current V01-ADR-067 terms, and no separate literal-constraint requirement is needed now.
...
- Behavior / default / fallback rules remain owned by `V01-REQ-DATA-006` / `V01-WORK-DATA-013`.
- Selector / support matrix / object-dependent kind rules remain owned by `V01-REQ-DATA-007` / `V01-WORK-DATA-014`.
- Recursive / untagged-union representation remains owned by `V01-REQ-DATA-008` / `V01-WORK-DATA-015`.
- Tagged union / discriminator payload remains owned by `V01-REQ-DATA-004` / closed `V01-WORK-DATA-010`.
- MCP identity / semantic reference identity remains owned by `V01-REQ-MCP-004` / `V01-WORK-MCP-004`.
