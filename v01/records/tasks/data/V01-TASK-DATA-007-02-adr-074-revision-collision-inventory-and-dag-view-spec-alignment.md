# V01-TASK-DATA-007-02: V01-ADR-074 revision, collision inventory, and DAG view spec alignment

- **id**: V01-TASK-DATA-007-02
- **status**: done
- **date**: 2026-06-02
...
- **outputs**:
  ...
  - V01-ADR-074 revision / split decision for collision disambiguation
  - Accepted DAG TypeRef hint minimum contract
  - DAG view spec alignment for asset TypeRef hint labels

## Goal

Revise V01-ADR-074 toward the accepted V01-WORK-DATA-007 minimum, decide whether shortened QID fallback belongs in the minimum or should be split / deferred, and align the DAG view spec with the accepted TypeRef hint render contract.

This task exists because V01-TASK-DATA-007-01 concluded `revise-before-acceptance`, and the Opus review confirmed that the next task must close the shortened-QID fallback ambiguity before implementation or fixture work starts.

## Work

- Review V01-TASK-DATA-007-01 evidence and the Opus review findings as input.
- Inventory UC-001 / UC-002 current YAML for named model local-id collisions within the same DAG render scope.
- Use the collision inventory to decide whether V01-ADR-074 should be:
  - partially revised with collision behavior kept inside V01-ADR-074,
...
- If shortened QID fallback is deferred from the minimum, explicitly define the minimum collision-time behavior in V01-ADR-074 and the DAG spec. Candidate minimum behavior is to omit the ambiguous type hint rather than render misleading local-id hints or full QIDs in Mermaid labels.
- Revise V01-ADR-074 before acceptance so that:
  - the active boundary is V01-REQ-DATA-005 / V01-WORK-DATA-007,
  - M15 / V01-WORK-DATA-001 remain closed and are not reopened,
...
- Update V01-ADR-074 Evidence / close-boundary wording so it no longer reads as only an M15 deferred item.
- Update `docs/spec/views/dag.md` to define the accepted minimum label format and TypeRef hint calculation rule.
- Update stale DAG spec boundary wording that still points to `V01-WORK-DATA-003` as the future scope for V01-ADR-074, replacing it with V01-REQ-DATA-005 / V01-WORK-DATA-007.
...
2. If no collision is observed, is it sufficient to defer shortened QID fallback from the V01-WORK-DATA-007 minimum?
...
4. Should V01-ADR-074 be partially revised, or should collision disambiguation be extracted into a separate proposed ADR?
5. What exact collision-time behavior should the accepted DAG TypeRef hint minimum specify?
6. Which stale V01-ADR-074 / dag.md boundary lines must be updated so future readers do not treat V01-ADR-074 as a still-deferred M15 item?

## Included Scope

- Collision inventory for current UC-001 / UC-002 YAML and DAG render scopes.
- V01-ADR-074 revision or split decision before acceptance.
- V01-ADR-074 wording update for active V01-REQ-DATA-005 / V01-WORK-DATA-007 boundary.
- DAG view spec alignment for accepted asset TypeRef hint label behavior.
- Stale boundary reference cleanup directly tied to V01-ADR-074 / V01-WORK-DATA-007.
...
- V01-ADR-074 is either revised toward acceptance or collision disambiguation is explicitly split / deferred with a documented rationale.
...
- Stale `V01-WORK-DATA-003` / M15-only boundary wording directly related to V01-ADR-074 is updated or explicitly left with rationale.
- No renderer, fixture, golden, UC YAML, TypeRef compatibility, or diagnostic implementation changes are performed by this task.

## Verification

- Validate `V01-TASK-DATA-007-02` metadata with Design Records MCP.
- Validate `V01-WORK-DATA-007` metadata / relations with Design Records MCP.
- Confirm V01-ADR-074 status and wording reflect the chosen revise / split path.
...
This supports deferring `shortened QID fallback` from the V01-WORK-DATA-007 minimum, as long as the accepted minimum still defines deterministic collision-time behavior. The recommended minimum behavior remains: if a named model local id is ambiguous in a DAG render scope and shortened-QID fallback is not part of the minimum, omit the type hint for the ambiguous asset label rather than rendering a misleading local id or a full QID in Mermaid.
...
- Current UC-001 / UC-002 do not require shortened QID fallback in the V01-WORK-DATA-007 minimum.
...
- Current renderer does not implement V01-ADR-074 hints yet; the inventory checked candidate hint surfaces from resolved semantic structures.
...
This is sufficient evidence to proceed with V01-ADR-074 revision using the following V01-WORK-DATA-007 minimum:
...
The inventory does not prove future projects or future fixtures cannot create same-DAG named model local-id collisions. It only proves that current UC-001 / UC-002 do not require shortened QID fallback to validate the V01-WORK-DATA-007 minimum. Collision disambiguation can be split into follow-up behavior if a future fixture introduces a real same-DAG collision.

### V01-ADR-074 revision result

V01-ADR-074 was revised from `proposed` to `accepted` under the active V01-REQ-DATA-005 / V01-WORK-DATA-007 boundary.

Revision summary:

- The top note now states that V01-ADR-074 is accepted for V01-REQ-DATA-005 / V01-WORK-DATA-007.
- M15 / V01-WORK-DATA-001 are explicitly not reopened.
...
- V01-ADR-074 Evidence now records the V01-TASK-DATA-007-02 / Codex `PASS_NO_COLLISION` inventory and the V01-WORK-DATA-007 minimum collision behavior.

Decision: partial revise, not split.

Rationale: current UC-001 / UC-002 have no same-DAG collision, so a separate collision-disambiguation ADR is not required for the V01-WORK-DATA-007 minimum. The accepted ADR keeps the readability hint contract complete by defining hint omission for ambiguous local ids. A future fixture with real collision pressure can create a follow-up ADR / task for shortened QID disambiguation.

### DAG view spec alignment result

`docs/spec/views/dag.md` was updated to align with the accepted V01-ADR-074 minimum.
...
- The stale `V01-WORK-DATA-003` future-scope wording was replaced with V01-ADR-074 / V01-REQ-DATA-005 / V01-WORK-DATA-007 ownership.
...
2. If no collision is observed, is it sufficient to defer shortened QID fallback from the V01-WORK-DATA-007 minimum?
   - Yes, as long as deterministic collision-time behavior is still specified. V01-ADR-074 and dag.md now specify hint omission for ambiguous named model local ids.
...
4. Should V01-ADR-074 be partially revised, or should collision disambiguation be extracted into a separate proposed ADR?
   - Partially revise V01-ADR-074. Splitting is not needed for the current minimum because collision disambiguation is not accepted; the minimum only needs hint omission.
...
6. Which stale V01-ADR-074 / dag.md boundary lines must be updated so future readers do not treat V01-ADR-074 as a still-deferred M15 item?
   - V01-ADR-074 top note and Evidence close boundary were updated to name V01-REQ-DATA-005 / V01-WORK-DATA-007 and preserve M15 / V01-WORK-DATA-001 closure. dag.md stale `V01-WORK-DATA-003` wording was replaced with V01-ADR-074 / V01-REQ-DATA-005 / V01-WORK-DATA-007 ownership.

### Verification result

- `V01-TASK-DATA-007-02` performed collision inventory, ADR revision, and DAG view spec alignment only.
- V01-ADR-074 is now `accepted` under V01-REQ-DATA-005 / V01-WORK-DATA-007.
- `shortened QID fallback` is not part of the V01-WORK-DATA-007 minimum.
...
- Excluded scopes remain excluded: V01-ADR-073, V01-ADR-078 / V01-ADR-079 / V01-ADR-080, UC-002 duplicate task QID repair, remaining notes retreat cleanup, and reopening M15 / V01-WORK-DATA-001〜004.
