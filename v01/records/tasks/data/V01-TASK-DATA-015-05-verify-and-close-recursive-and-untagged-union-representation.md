# V01-TASK-DATA-015-05: Verify and close recursive and untagged-union representation

- **id**: V01-TASK-DATA-015-05
- **status**: done
- **date**: 2026-06-07
...
- **outputs**:
  - Verification summary for V01-WORK-DATA-015
  - V01-WORK-DATA-015 close evidence
  - V01-REQ-DATA-008 close / accepted update input if appropriate

## Goal

Verify the selected recursive named reference / unsupported untagged union boundary and close V01-WORK-DATA-015 when all selected follow-up work is complete.

## Work

- Review V01-TASK-DATA-015-01 through V01-TASK-DATA-015-04 results.
...
- Update V01-WORK-DATA-015 close evidence when ready.
- Identify whether V01-REQ-DATA-008 can move from `captured` to `accepted` or should remain open.

## Included Scope

- Verification and close synchronization.
- V01-WORK-DATA-015 evidence update.
...
- V01-WORK-DATA-015 is closed or explicitly left open with remaining blocker rationale.
...
- `V01-TASK-DATA-015-01`: contract boundary decision.
- `V01-TASK-DATA-015-02`: TypeRef spec update.
- `V01-TASK-DATA-015-03`: recursive named model reference runtime investigation.
- `V01-TASK-DATA-015-04`: UC-002 recursive / untagged-like YAML cleanup.
- `V01-TASK-DATA-015-06`: independent review of spec and task split.
...
- `V01-TASK-DATA-015-03` classified current runtime behavior as already-supported.
...
- V01-ADR-073 remains limited to tagged / discriminated union.
- No untagged union, general oneOf, anyOf, scalar union, or V01-ADR-073 broadening was introduced.
...
- MCP `validate_records` for `V01-TASK-DATA-015-04`: PASS.
- MCP `validate_records` for `V01-WORK-DATA-015`: PASS.

### Close decision

`V01-WORK-DATA-015` can be closed as done.

`V01-REQ-DATA-008` can move from `captured` to `accepted` because the representation requirement was decided, specified, verified, and applied to the selected UC-002 surfaces without introducing untagged union support.

### Workspace note

The only dirty file reported before this close update was `tmp.py`, which is unrelated to V01-WORK-DATA-015 and should not be included in this close commit.
