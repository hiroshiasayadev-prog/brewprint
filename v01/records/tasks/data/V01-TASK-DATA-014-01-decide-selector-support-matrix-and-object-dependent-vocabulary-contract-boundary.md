# V01-TASK-DATA-014-01: Decide selector support matrix and object-dependent vocabulary contract boundary

- **id**: V01-TASK-DATA-014-01
- **status**: done
- **date**: 2026-06-07
...
Decide the contract boundary for `V01-REQ-DATA-007` / `V01-WORK-DATA-014` before any spec, YAML, fixture, or implementation change is introduced.

This task determines where selector support matrices and object-dependent `kind` vocabulary are represented and validated.

## Work

- Review the V01-WORK-DATA-014 source bucket: N-020, N-031, N-037, N-040, and N-042.
...
- Confirm the decision aligns with `V01-REQ-DATA-007` and `V01-WORK-DATA-014`.
- Confirm the decision does not reopen `V01-WORK-DATA-009`, `V01-WORK-DATA-010`, `V01-WORK-DATA-013`, MCP identity, tagged union, recursive/untagged-union, or DAG TypeRef hint scope.
...
- V01-WORK-DATA-014 will not introduce a DATA DSL extension for selector support matrices, object-dependent `kind` vocabulary, dependent enums, or selector-field combination validation at this stage.
...
- Compared against `V01-REQ-DATA-007` / `V01-WORK-DATA-014` scope: the decision covers selector support matrices and object-dependent vocabulary without mixing in excluded tagged union, recursive/untagged union, numeric/default behavior, DAG TypeRef hint, or MCP identity work.
