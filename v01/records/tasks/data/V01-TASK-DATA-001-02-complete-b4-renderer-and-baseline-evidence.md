# V01-TASK-DATA-001-02: Phase A〜B4 の evidence と V01-ADR-064 renderer を完了する

- **id**: V01-TASK-DATA-001-02
- **status**: done
- **date**: 2026-05-29
...
- **outputs**:
  - V01-ADR-060〜V01-ADR-063 の implementation / test evidence 確認結果
  - V01-ADR-064 DAG renderer 実装と current fixture / golden 更新
  - Phase A〜B4 regression verification evidence

## Goal

M15 baseline である Phase A〜B4 を閉じるため、既に完了扱いとなっている Phase A〜B3 の証跡を独立に確認し、未完の V01-ADR-064 DAG render 実装・fixture・regression verification を完了する。

## Work

- V01-ADR-060〜V01-ADR-063、関連 spec、implementation、tests を照合し、legacy M15 checkbox に依存せず Phase A〜B3 の完了 evidence を整理する。
- V01-ADR-064 の accepted decision と `docs/spec/views/dag.md` の反映内容を照合し、renderer implementation の不足を確定する。
...
- V01-ADR-074 の DAG asset TypeRef hint は本 task に混入させない。

## Done condition

- V01-ADR-060〜V01-ADR-063 の implementation / tests / evidence の存在確認結果が記録されている。
- V01-ADR-064 の accepted render boundary に必要な実装と golden / fixture 更新が完了している。
- Phase A〜B4 に関する対象 tests / regression verification が passing であり、結果が evidence として記録されている。
- V01-ADR-074 の TypeRef hint 表示を追加していない。

## Verification

- V01-ADR-060〜V01-ADR-064 と対応 spec / implementation / tests / golden の差分を照合する。
...
- Codex implementation gap review returned `Blocking gaps` for V01-TASK-DATA-001-02.
- V01-ADR-060〜V01-ADR-063 resolver baseline was reported as implemented and tested by existing tests (`flow_wiring_type_test.go`, `foreach_returns_test.go`, `return_source_test.go`).
- Blocking gaps were concentrated in V01-ADR-064 DAG renderer / UC-001 current golden:
  - `subgraph returns` still emitted even though V01-ADR-064 abolished it.
...
- Implemented draft V01-ADR-064 renderer changes:
...
- Observed mismatches align with V01-ADR-064 renderer changes: old goldens still expect `subgraph returns`; actual output emits no returns boundary, emits `subgraph initializes` where applicable, and adds Returns table `source` column.
...
- `go test ./...`: passed.
- UC-001 current DAG goldens now represent v1.1 V01-ADR-064 behavior; `v1.0.0-spec` remains preserved by tag and was not retroactively modified.
- V01-ADR-074 TypeRef hint rendering was not introduced in this task.
