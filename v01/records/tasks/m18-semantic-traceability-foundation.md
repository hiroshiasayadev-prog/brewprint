...
V01-ADR-088 により、MVP は semantic realization mapping ではなく **canonical reference resolution foundation** に縮小された。M18 は、`spec:` semantic ref、record ID-as-ref、および investigation canonical reference validation の責務境界を spec / policy / authoring guidance / M19 handoff に同期する。
...
- `docs/spec/concepts/traceability/` の V01-ADR-088 追従
...
- V01-ADR-087 / V01-ADR-088 に基づく Design Records MCP implementation handoff の M19 への同期
...
- V01-ADR-082: golden fixture と self-hosting requirement の責務境界
...
- [x] V01-ADR-087 の investigation canonical reference boundary を反映する
...
- [x] `V01-INV-DOCS-002` により external coverage artifact の MVP 必要性を評価する
- [x] `V01-INV-DOCS-003` により `internal-design:` endpoint / semantic realization relation 自体の MVP 必要性を評価する
- [x] V01-ADR-088 を corrected decision として accepted にし、MVP を canonical reference resolution foundation に縮小する
...
- [x] `docs/doc-policy.md` の MVP active prefix / coverage 説明を V01-ADR-088 に同期する
...
- [x] `V01-REQ-MCP-001` / `V01-WORK-MCP-001` の V01-ADR-088 および acceptance boundary 追従を反映する
...
- [x] V01-ADR-088 に基づく spec / policy / README / task / example cleanup 一式を独立レビューする
...
- `V01-INV-DOCS-002` は external coverage artifact を MVP に残す実需が確認できないことを示した。
- `V01-INV-DOCS-003` は `internal-design:` endpoint と realization relation 自体についても MVP 必須性が確認できないことを示した。
- Corrected V01-ADR-088 は、MVP を `spec:` semantic ref、record ID-as-ref、investigation canonical reference validation からなる foundation に縮小した。
...
- M19 で、`V01-ADR-086` の `depends_on: V01-INV-DOCS-001` が investigation record integration 後に resolve でき、現行の `missing_depends_on_target` error が解消されることを test する。
- M19 contract refinement で、ADR / spec の `depends_on` が `INV-*` を参照できる boundary と implementation 移行前の既知 unresolved の扱いを確定する。
- `migrated_to_spec` の付与基準は convention が未確定であるため、V01-ADR-088 では未指定へ戻し、必要なら別の小さな doc-policy / authoring-guide 作業で定める。
...
- [x] V01-ADR-088 の corrected MVP boundary が traceability leaf spec に反映されている
- [x] project artifact model / doc-policy / internal-design README が V01-ADR-088 と整合し、external artifact 用 directory を MVP layout として予約していない
...
- [x] V01-REQ-MCP-001 / V01-WORK-MCP-001 と Design Records MCP spec が V01-ADR-088 の M19 boundary と整合する
