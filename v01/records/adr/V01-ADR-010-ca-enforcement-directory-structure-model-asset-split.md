# V01-ADR-010: CA強制・ディレクトリ構造・model/asset分離

- **status**: accepted
- **date**: 2026-04-19
- **supersedes**: V01-ADR-002, V01-ADR-007

## 背景

YAMLの階層構造とディレクトリ構造の1:1対応設計を検討する中で、以下の問題が明確になった。

1. V01-ADR-002が示した `dag.yaml` / `er.yaml` というビュー別ファイル分けは、型別サブディレクトリ設計と軸が異なる
2. V01-ADR-007で定義した「asset = 型定義」は、DagsterのSoftware-Defined Assets（データ成果物の実体）を参照根拠としていたが、両者は別概念だった
...
> **V01-ADR-011により改訂**：「1ファイル=1ノード」は「1ファイル=1メインノード（+ 複数のサブノード可）」に上書きされた。
...
| Class | `endpoint: true` なtaskを自動収集（V01-ADR-005） |
| State | `state.yaml` に明示宣言（自動導出不可） |
| Sequence | `sequence.yaml` に明示宣言（自動導出不可） |

V01-ADR-002が示したビュー別ファイル（`dag.yaml` / `er.yaml`）は廃止。
名前空間ルール（フォルダ = モジュール）はV01-ADR-002から継続して有効。
...
- V01-ADR-002の `dag.yaml` / `er.yaml` 例示は本ADRにより廃止。名前空間ルール（フォルダ=モジュール）は継続有効
- V01-ADR-007の「asset = 型定義」は `model` に移管。`store` の定義（kind: db / session / collection / context）は継続有効
- V01-ADR-009の `params` / `returns` は `model` ID を参照する形に更新（旧: asset ID）
- `spec/nodes.md`（未作成）にて `model` / `task` のフィールド定義を詳細化する
- `spec/overview.md` のノード種別テーブルを更新する必要がある ✅
- **V01-ADR-011により「1ファイル=1ノード」は「1ファイル=1メインノード」に改訂。その他の決定は継続有効**
