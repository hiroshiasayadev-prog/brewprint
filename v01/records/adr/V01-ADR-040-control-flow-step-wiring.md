# V01-ADR-040: 制御フロー内step wiringの明示化
...
V01-ADR-015で `flow:` セクションを導入し、通常のtask実行は以下の `step` エントリでparams wiringを明示できるようにした。
...
case内に `steps:` listを導入すると、V01-ADR-023の「branchはエントリポイントのみを持ち、以降はDAG構造から導出する」という設計を崩すため採用しない。

## 影響

- V01-ADR-015のfork branches記法を本ADRで更新する
- V01-ADR-023のbranch cases記法に `cases[].params` を追加する
