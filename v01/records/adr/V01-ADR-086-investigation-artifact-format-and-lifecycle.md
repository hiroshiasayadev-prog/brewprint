...
- **status**: accepted
- **date**: 2026-05-19
- **depends_on**: V01-ADR-050, V01-ADR-068, V01-ADR-081, V01-ADR-083, V01-ADR-084, V01-ADR-085, V01-INV-DOCS-001
- **supersedes**:
- **migrated_to_spec**:
...
V01-ADR-085 により、複雑な変更における調査結果、根拠、影響範囲、未確定点、選択肢、後続 artifact 候補を保存する artifact layer として `docs/investigations/` が導入された。

V01-ADR-085 は、investigation が決定、現行仕様、要求そのもの、横断進捗、完了状態、具体的な作業手順を所有しないことを定めた。
...
一方で、V01-ADR-085 は `docs/investigations/` の詳細 format、status 語彙、ID 規則、MCP index 方針、lifecycle を確定しなかった。
これらは初回 investigation `V01-INV-DOCS-001` で調査することになった。

`V01-INV-DOCS-001` では、directory 運用、ID 規則、file name、metadata、status、scope / non-scope、起点 artifact と後続 artifact の記録方法、別 investigation の扱い、README / doc-policy / authoring guide の責務分担、MCP index 対象化について調査した。
...
初回 investigation である `V01-INV-DOCS-001` は `docs/investigations/docs/` に置く。
...
V01-ADR-081 / V01-ADR-084 では、requirement / work item / coverage mapping の ID 候補として domain-scoped sequence が示されている。
...
V01-ADR-085 の責務境界に従い、作業状態や完了状態は work item / task が所有する。
...
- 欠点: Design Records MCP の責務拡張と contract 変更を伴う。V01-ADR-084 の semantic trace MVP scope も不用意に広がる
...
`V01-INV-DOCS-001` は、以下の path に置く。
...
`V01-INV-DOCS-001` の status は `concluded` とする。
...
V01-ADR-083 の artifact placement decision rule は、investigation layer を含む形で後続更新または後続ADRによる refinement が必要になる。

### M18 task への影響

`docs/tasks/m18-semantic-traceability-foundation.md` は、V01-ADR-085 / V01-ADR-086 / `docs/investigations/` の導入を踏まえて、references / scope / done criteria を更新する必要がある。
...
- commit: 5688941
- impl commit: tbd
- 参考: V01-ADR-085 investigation artifact boundary, V01-INV-DOCS-001 investigation artifact format and lifecycle
