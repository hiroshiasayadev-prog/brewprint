...
V01-ADR-013にて「foreachの結果をstoreの特定フィールドへ蓄積するパターン」をV01-ADR-014に委譲した。
...
`initializes` で宣言されたstoreは**ファイル内にprivate**。同ファイル内のsub taskおよびforeachから参照可能。外部からの参照不可（V01-ADR-011のスコープ規則と同じ）。
...
- storeへの参照・更新のedge表現はV01-ADR-015（ファイル内edge記述構造）に委譲
