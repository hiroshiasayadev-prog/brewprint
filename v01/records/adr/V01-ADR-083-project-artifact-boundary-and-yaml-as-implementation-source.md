...
- **status**: accepted
- **date**: 2026-05-16
- **depends_on**: V01-ADR-050, V01-ADR-068, V01-ADR-082
- **supersedes**:
- **migrated_to_spec**:
...
V01-ADR-081 では `docs/requirements/` layer と spec traceability を導入する案を起票した。
V01-ADR-082 では UC / fixture を golden fixture に限定し、self-hosting を requirements 側へ移す案を起票した。
...
本ADRは、V01-ADR-081 / V01-ADR-082 の用語と責務境界を再検証するため、project artifact の関係と YAML の位置付けを明確化する。
...
fixture / golden の責務境界は V01-ADR-082 で扱う。
...
V01-ADR-081 で導入する `docs/requirements/` は、決定や現行仕様本文を所有しない。
...
V01-WORK-DAG-001:
  ...
  source_requirement: V01-REQ-DAG-001
  status:
    decision: proposed
    design_spec: pending
...
> V01-ADR-085 / V01-ADR-086 により `docs/investigations/` が導入されたため、本 rule は investigation layer を含む形に更新された。

### 12. V01-ADR-081 / V01-ADR-082 は本ADRを踏まえて再レビューする

V01-ADR-081 / V01-ADR-082 は、本ADRの用語整理を踏まえて再レビューする。

特に V01-ADR-081 は以下の点を修正対象とする可能性がある。
...
V01-ADR-081 は、本ADRを踏まえて accepted 前に revision する必要がある。
...
V01-ADR-081 の `requirements` を system obligation として扱う表現、`requirements → implementation / yaml / fixture` に見える関係、requirements 内の implementation-facing document、`confirmed requirement` などの用語は、本ADRにより refine / replace される。

### V01-ADR-082 への影響

V01-ADR-082 の方向性（self-hosting を fixture から requirement 側へ移す、editor / viewer notes を分離する）は維持される。
ただし V01-ADR-082 §7 の「現在の内容 → 移動先」テーブルは、本ADRの placement rule に従って再生成する必要がある。
...
fixture / golden は一般 project の source-of-truth chain には含めないが、work item は fixture / golden impact を追跡してよい。fixture / golden の責務境界は V01-ADR-082 が所有する。
...
- commit: a80ec7b
- impl commit: tbd
- 参考: V01-ADR-050 spec-first documentation policy, V01-ADR-068 ADR authoring guide, V01-ADR-081 project requirement layer と spec traceability, V01-ADR-082 golden fixture と self-hosting requirement の責務境界
