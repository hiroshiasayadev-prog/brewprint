# V01-ADR-032: sequence diagramシナリオファイルのYAML schema

- **status**: accepted
- **date**: 2026-04-22

> ※ V01-ADR-035 で拡張：stepに `guard:` フィールドを追加し、guard分岐を持つtransitionの一意特定に対応。`(from_state, via)` による一意特定は `(from_state, via, guard?)` に変更される。

## 背景

V01-ADR-004でsequence diagramのparticipant種別・矢印ラベル・DB participantの粒度を確定した。
...
| 情報 | 解決元 |
|------|--------|
| 矢印の送信元participant | event の `source` / `actor` フィールド（V01-ADR-018） |
...
| UI participantの生成 | `source=ui` のeventが存在する場合に暗黙生成（V01-ADR-004） |
...
- **案C（state.yamlに `scenarios:` セクション追加）**: 1ファイルがFSM定義とview定義の二役を担うことになり、V01-ADR-030の原則と相性が悪い。また1シナリオ1ファイルの方がdiff・レビュー・参照が明確。却下。
...
- V01-ADR-030の `as:` 値一覧に `sequence_diagram` を追加する
