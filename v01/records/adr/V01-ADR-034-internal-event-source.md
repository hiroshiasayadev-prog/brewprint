# V01-ADR-034: eventのsourceに `internal` を追加
...
V01-ADR-018でeventの `source` を `ui` / `external` / `er` の3種で定義した。しかしUC-001 EC Checkout Flow の `auth/state.yaml` 設計中に、以下のイベントがいずれのsourceにも綺麗に当てはまらないことが判明した。
...
- **`ui` に入れる案**: V01-ADR-018で「ユーザー操作（クリック・フォーム送信等）」と定義されている。task完了はユーザー操作ではない。`ui` に入れるとカテゴリが膨張し、LLMが `source: ui` を見て「ユーザーが何かした」と推論できなくなる。
- **`external` に入れる案**: V01-ADR-018で「webhook・MQ・WebSocket等の外部システムからの入力」と定義されている。task完了通知はアプリ内部で起きる事象であり、物理的に外部システムから来るわけではない。`spec/views/state-diagram.md` のYAML入力例が `login_succeeded` を `source: external, actor: auth_server` と書いていたのは、V01-ADR-018策定時にFSM駆動まで設計が届いていなかった仕様の穴と見るべき。
...
### sourceカテゴリの意味（V01-ADR-018からの拡張）
...
- **V01-ADR-018をsupersedeして書き直す案**: 既存3カテゴリの定義は変わらないため、supersedeは過剰。拡張ADRとして独立させるほうが履歴が辿りやすい。
...
- V01-ADR-018は拡張対象。supersedeはしない（既存3カテゴリの定義は変更なし）
