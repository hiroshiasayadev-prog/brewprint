# V01-ADR-074: DAG asset node label の TypeRef hint 表示

- **status**: accepted
- **date**: 2026-05-11

> このADRは、V01-REQ-DATA-005 / V01-WORK-DATA-007 の active boundary で accepted とする。
> M15 / V01-WORK-DATA-001 は reopen しない。実装・fixture 更新は後続 task で扱う。
> 現在の仕様は spec を参照すること。

## 背景

V01-ADR-060 で TypeRef と flow wiring type compatibility を導入し、task params / returns / model fields などに primitive / named model / inline `list<T>` / `dict<T>` を指定できるようにした。
...
V01-WORK-DATA-007 minimum では、曖昧な named model local id に対して shortened QID fallback は行わない。
...
shortened QID fallback / suffix-qualified id の算出は、core readability hint とは別の name disambiguation concern であり、V01-WORK-DATA-007 minimum には含めない。
...
V01-ADR-064 により、DAG render の `subgraph returns` は廃止されている。
...
- `docs/spec/views/dag.md`
  ...
  - shortened QID fallback / suffix-qualified id 算出は V01-WORK-DATA-007 minimum から外し、将来 follow-up として扱うことを明記する
...
shortened QID fallback / suffix-qualified id 算出は V01-WORK-DATA-007 minimum の renderer 実装対象に含めない。
...
### V01-ADR-071 / V01-ADR-075 との関係

V01-ADR-071 / V01-ADR-075 は file-private helper model の Markdown render exposure を扱う。
...
- commit: 5ae7769
- impl commit: tbd
- close boundary: M15 / `v1.1.0-spec` では follow-up scope として deferred。現在の active boundary は V01-REQ-DATA-005 / V01-WORK-DATA-007。M15 / V01-WORK-DATA-001 は reopen しない。実装は後続 task で扱う。
- collision inventory: V01-TASK-DATA-007-02 / Codex verification で UC-001 / UC-002 は `PASS_NO_COLLISION`。V01-WORK-DATA-007 minimum では曖昧な named model local id の TypeRef hint を omit する。
- 参考: V01-ADR-060 TypeRef、V01-ADR-064 DAG returns.source render、V01-ADR-071 task file helper model render exposure、V01-ADR-075 model file render
