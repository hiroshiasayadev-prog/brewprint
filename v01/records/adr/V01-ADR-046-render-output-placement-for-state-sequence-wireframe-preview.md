# V01-ADR-046: State / Sequence / Wireframe / Preview のrender出力配置
...
V01-ADR-043で `renders/` の基本構造を定義し、V01-ADR-045で `render_index.yaml` によるgroupingを定義した。
UC-001の移行レビューを通じて、V01-ADR-043 / V01-ADR-045だけでは以下の配置規則が不足していることが分かった。

1. **State Diagramのファイル名粒度が曖昧**
   - V01-ADR-043のディレクトリ図では `state-{state-id}.md` と書かれている。
...
2. **Sequence scenarioの所属groupが未定義**
   ...
   - ただしV01-ADR-032により、sequence scenarioは必須フィールド `state_file` を持つ。
   - `state_file` が指すFSMファイルのmoduleから所属groupを推論できるが、このルールはまだV01-ADR-043 / V01-ADR-045に明記されていない。

3. **Wireframe renderの配置が未定義**
   - V01-ADR-029 / V01-ADR-042 / `docs/spec/views/wireframe.md` により、wireframe DSLとHTML fragment renderは定義済みである。
   - しかしV01-ADR-043の `renders/` 構造には `wireframe-*` の出力先がない。
...
明示group / 暗黙groupの解決はV01-ADR-045と同じ規則に従う。

V01-ADR-043の `state-{state-id}.md` は、実運用上は `state-{fsm-id}.md` を意図していた表記として補正する。
...
`state_file` のtop-level moduleが明示groupに含まれない場合は、V01-ADR-045のuncovered moduleルールに従い、暗黙group（group id = module名）に出力する。

`state_file` はV01-ADR-032で必須フィールドであるため、省略されているsequence scenarioはparser errorとする。
...
group idに `_preview` を含むアンダースコア始まりのIDを使うことは、V01-ADR-043 / V01-ADR-045の方針と同じくvalidation errorとする。
...
- V01-ADR-043の `state-{state-id}.md` 表記は、本ADRに基づき `state-{fsm-id}.md` として補正する。
...
- V01-ADR-043およびdoc-policyのUC構造例は、本ADRの命名規則と `_preview/` 配置に追随して更新する必要がある。
