...
V01-ADR-069 では anonymous inline struct TypeRef を導入しないことを決めた。
V01-ADR-070 では、その補完策として public model / file-private helper model の visibility 概念を導入した。
V01-ADR-071 では、task file 内 file-private helper model を DAG Markdown detail section に表示する方針を決めた。
...
V01-ADR-070 によって helper model を導入すると、public model と file-private helper model の関係も増える。
...
ただし、本ADRは opt-in の cross-cutting catalog view を定めるものであり、V01-ADR-070 §10 の不可視禁止制約を単体で履行するものではない。
model file 内 helper model の file-local な自動 render exposure は、後続 V01-ADR-075 で扱う。
...
本ADRの model catalog は、V01-ADR-070 §10 を単体で履行するものではなく、user が module / contract 単位で schema 群を読むための opt-in view である。
file-private helper model の自動表示は V01-ADR-071（task file）および後続 V01-ADR-075（model file）で担う。
...
表記形式は V01-ADR-071 の `## Private models` と揃える。
...
V01-ADR-069 で anonymous inline struct を導入しないと決めたため、意味を持つ shape は named model として定義する方針になる。
V01-ADR-070 で file-private helper model を認めたことで、public model と private helper model の関係も増える。
...
V01-ADR-070 の visibility 方針に従い、render 上も public / private を明確に分ける。
...
本ADRの catalog は opt-in の curated view であり、V01-ADR-070 §10 の不可視禁止制約を単体で履行するものではない。
file-private helper model の自動表示は、task file については V01-ADR-071、model file については後続 V01-ADR-075 が担う。
...
### V01-ADR-070 / V01-ADR-071 / V01-ADR-075 との関係

V01-ADR-070 の「file-private helper model を人間向け render から不可視にしてはならない」制約は、複数の render ADR に分担して履行する。

| 対象 | 履行責任 | render 経路 |
|---|---|---|
| task file 内 helper model | V01-ADR-071 | DAG Markdown `## Private models` 自動表示 |
| model file 内 helper model | V01-ADR-075（後続） | model file 単体 render の自動表示 |
...
model file 内 helper model の file-local な自動 render exposure は、本ADRではなく後続 V01-ADR-075 で扱う。
...
- 参考: V01-ADR-069 anonymous inline struct 不採用、V01-ADR-070 model visibility、V01-ADR-071 task file helper model render exposure、UC-002 MCP contract model 群
