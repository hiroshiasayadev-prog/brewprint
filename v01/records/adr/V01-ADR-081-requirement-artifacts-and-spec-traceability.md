# V01-ADR-081: project requirements layer と semantic traceability

- **status**: accepted
- **date**: 2026-05-16
- **depends_on**: V01-ADR-050, V01-ADR-068, V01-ADR-083
- **supersedes**:
- **migrated_to_spec**:
...
brewprint は V01-ADR-050 により spec-first のドキュメント運用へ移行した。
現行仕様は `docs/spec/**` に置き、ADR は設計判断の背景・理由・却下案を記録する。
V01-ADR-068 では ADR / spec / task / UC docs の責務境界を整理した。
...
V01-ADR-083 に従い、work item は source requirement を必ず持つ。
...
これは V01-ADR-083 の trace layer common principle に従う。
...
fixture の責務境界は V01-ADR-082 が所有する。
...
また、requirements から YAML / implementation へ直接流れるように見えると、V01-ADR-083 の artifact boundary と衝突する。
...
V01-ADR-083 の trace layer common principle に従い、requirements も semantic ref を primary key として関連 artifact を参照する。
...
- 利点: 要求から成果物を直接辿れる
- 欠点: requirements が source of truth に見える。V01-ADR-083 の YAML as primary DSL source / coverage / work item の責務境界と衝突する
...
work item、internal design、coverage、tasks、YAML との責務境界は V01-ADR-083 に従う。

### V01-ADR-082 への影響

fixture / golden は requirements を所有しない。
requirements は fixture semantic ref を参照してよいが、fixture の expected output や test harness 責務は V01-ADR-082 側で扱う。
...
- commit: a80ec7b
- impl commit: tbd
- 参考: V01-ADR-050 spec-first documentation policy, V01-ADR-068 ADR authoring guide, V01-ADR-083 project artifact boundary と YAML as primary implementation source
