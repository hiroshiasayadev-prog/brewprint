...
- **status**: accepted
- **date**: 2026-05-17
- **depends_on**: V01-ADR-050, V01-ADR-068, V01-ADR-081, V01-ADR-082, V01-ADR-083
- **supersedes**:
- **migrated_to_spec**:
...
V01-ADR-081 により `docs/requirements/` layer と semantic traceability の方針が導入された。
V01-ADR-083 により、brewprint project の artifact boundary と YAML as primary implementation source が整理され、trace layer は physical path ではなく semantic ref を primary key とする方針が定まった。
...
V01-ADR-083 §7 が coverage に課した問いのうち、本ADRの MVP coverage が扱うのは YAML を含まない問い、すなわち spec と internal design の対応関係に限る。
...
`spec:` ref の解決は、V01-ADR-081 §7〜§8 の spec front matter `sections` mapping と anchor append-only 規則を前提とする。
...
V01-ADR-083 が primary DSL source と呼ぶ YAML はこの brewprint DSL YAML である。
...
新規 fixture では V01-ADR-082 に従い、期待 render 出力を `render_expected/` に置ける。
...
V01-ADR-082 §8 の fixture-local coverage は本ADR MVP の project-level trace graph には含めない。
...
V01-ADR-083 の trace layer common principle に従い、semantic trace MVP でも physical path を primary key にしない。
...
requirement ID は V01-ADR-081 §5 で示された domain-scoped sequence に従い、ADR 番号と結合しない。
work item は V01-ADR-083 に従い、source requirement を必ず持つ。
...
不足が見つかった場合は V01-ADR-081 の requirements layer に捕捉し、work item によって拡張できる。
...
- 利点: 実装が簡単
- 欠点: file rename、split、merge、section move で trace が壊れる。V01-ADR-083 の trace layer common principle と衝突する
...
`docs/doc-policy.md` は V01-ADR-081〜083 と本ADRを踏まえて更新する必要がある。
...
また、UC docs / fixture が gap discovery log や migration state を所有するように読める既存記述は、V01-ADR-082 / V01-ADR-083 / 本ADRと整合するように更新する必要がある。
...
V01-ADR-082 に従い、新規 fixture では `render_expected/` を使える。
...
- commit: 17d6910
- impl commit: tbd
- 参考: V01-ADR-050 spec-first documentation policy, V01-ADR-068 ADR authoring guide, V01-ADR-081 project requirements layer と semantic traceability, V01-ADR-082 golden fixture と self-hosting requirement の責務境界, V01-ADR-083 project artifact boundary と YAML as primary implementation source
