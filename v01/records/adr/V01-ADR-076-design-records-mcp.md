...
brewprint は V01-ADR-050 により spec-first のドキュメント運用へ移行している。

この運用では、現行仕様は `docs/spec/**` に置き、ADR は設計判断の背景・却下案・トレードオフを記録する。
また、既存ADRの仕様記述は一括移行せず、触れたタイミングで漸進的にspecへ移す方針を採っている。

V01-ADR-068 では、ADR / spec / task / UC docs の責務境界を「所有」「参照」「影響範囲」という観点で整理した。
...
task file / UC docs / impl notes は、V01-ADR-068で責務境界上のartifactとして扱われる。
...
- V01-ADR-067〜V01-ADR-076
- V01-ADR-050: spec-first documentation policy
- `docs/spec/design-records-mcp/**`（本ADRと同時または直後に新設）

それ以前のADR（V01-ADR-001〜V01-ADR-049、V01-ADR-051〜V01-ADR-066）は一括移行しない。
...
Design Records MCP は、V01-ADR-050のspec-first方針を置き換えるものではない。
...
brewprint MCP は、V01-ADR-054で設計対話を支援するquery layerとして位置づけられている。
...
- commit: f92c228
- impl commit: tbd
- 参考: V01-ADR-050のspec-first運用、V01-ADR-068のADR authoring guide、V01-ADR-054のMCP設計対話coverage
