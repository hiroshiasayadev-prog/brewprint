# V01-ADR-041: Sequence Diagram message rendering
...
V01-ADR-036でevent source別のSequence Diagram矢印ルールを定義し、V01-ADR-037でactionなしtransitionをUI self-messageとして描画することを決めた。
また、V01-ADR-038でtransition.actionが指すtaskファイル内のmain task + sub taskを辿り、DB操作tableに `step` / `task` / `sub_task` / `store` / `操作` を出力することを決めた。
