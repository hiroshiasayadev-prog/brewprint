# V01-ADR-036: sequence diagram矢印ルールのevent source別拡張
...
V01-ADR-004でsequence diagramのparticipant種別・矢印ラベルを定義した時点では、`source: ui` を起点とするフロー（UI → API → DB → UI）が主な想定だった。

その後の拡張でeventの `source` は4種に増えた:

- `source: ui`（V01-ADR-018）
- `source: external`（V01-ADR-018）
- `source: er`（V01-ADR-018）
- `source: internal`（V01-ADR-034）
...
| participant | 生成条件 |
|---|---|
| Actor | `source=external` のeventを参照するstepが1つでもある場合。対象actorは `event.actor` で指定されたglobal actor（V01-ADR-031） |
...
`store.kind=db` のstoreは既にDB participantとして列に存在する（V01-ADR-004）ため、`DB→API` として自然に描画できる。
`kind=session` / `kind=collection` / `kind=context` のstoreはparticipant列に出ない（V01-ADR-004）ため、矢印の起点として表現できない。したがって `source: internal` と同様に自己ループで描画する。

## 理由

### 4 sourcesの網羅的定義

V01-ADR-018で3種、V01-ADR-034で4種目のsourceが追加されたが、sequence diagramの描画ルールは V01-ADR-004 当時の想定（主にui起点）のまま取り残されていた。本ADRでこの不整合を解消する。
...
- **`source: er` を一律 `DB→API` にする**: `kind=session` 等はDB participantに出ない設計（V01-ADR-004）と矛盾する。
- **`source: er` を一律自己ループ**: `kind=db` のケースで「DB観測駆動」という本質情報を矢印で表現できなくなる。
- **V01-ADR-004をsupersedeして書き直す案**: V01-ADR-004のparticipant種別・DB粒度・happy path方針は変更不要。矢印ルールのみの拡張であり、独立したADRとして履歴を残すほうが辿りやすい（V01-ADR-018 → V01-ADR-034 と同じパターン）。

## 影響

- V01-ADR-004 はparticipant種別・DB粒度・happy path方針は変更なし。矢印ラベル表のみ本ADRで拡張（supersedeはしない）
...
- V01-ADR-034 で追加された `source: internal` の可視化方針が本ADRで補完される
