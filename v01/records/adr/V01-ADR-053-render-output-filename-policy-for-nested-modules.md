# V01-ADR-053: nested module の render output filename は local ID + collision error とする

- **status**: accepted
- **date**: 2026-04-30
- **depends on**: V01-ADR-043, V01-ADR-045, V01-ADR-046, V01-ADR-050
...
V01-ADR-043 / V01-ADR-046 により、brewprint の render 出力は `renders/{group-id}/...`、`renders/_cross/...`、`renders/_preview/...` に配置されることが決まっている。
また、V01-ADR-045 により、`render_index.yaml` の group 定義では最上位 module を指定し、nested module は親 module の group に含める方針が決まっている。
