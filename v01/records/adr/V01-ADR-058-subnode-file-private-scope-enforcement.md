# V01-ADR-058: サブノードのファイル内private性を実装で正しく保証する
...
これは V01-ADR-011 §3 / spec/nodes.md「ファイル構造」が定める**サブノードはファイル内private**という規定と矛盾する。
サブノードがファイル内privateであるならば、別ファイルにある同名サブノードは衝突しないはず。

### 仕様側の規定（再掲）

- V01-ADR-011 §3 サブノードの可視性スコープ: 「サブノードはファイル内にprivate。外部モジュールからの参照不可」
...
V01-ADR-057でUC-001を v1.0.0-spec のcanonical fixtureと固定したが、UC-001だけでは仕様の一部側面が未カバーだったことがUC-002 Phase Aで露呈した形である。
...
V01-ADR-050 のspec-first方針上、実装詳細はADRの拘束事項ではない。以下は方針メモ:
...
V01-ADR-011がそもそも「外部参照不可」と定義したサブノードが、
...
V01-ADR-011 / spec/nodes.md の現状記述「サブノードはファイル内private」は意図としては正しいが、
「同名サブノードが別ファイルにあっても衝突しない」までは読み切れない曖昧さが残っていた。
spec-first運用（V01-ADR-050）に従い、現行仕様の唯一の正であるspecを強化する。

V01-ADR-011本体は遡及修正しない（doc-policy.md §3 「ADRの記述は遡及修正しない」）。
本ADRが V01-ADR-011 の意図を引き継ぎつつ実装側の整合を取るための補強ADRとなる。
...
- 仕様上認められた書き方を回避する形になり、V01-ADR-011のLLMエルゴ目的に反する
...
`v1.0.0-spec` 凍結条件（V01-ADR-057 §2）の判定基準のうち「`go test ./...` パス」は
M14a で本修正を入れる際に**バグ修正テストを追加した状態**で再度満たす必要がある。

ただしV01-ADR-057の凍結基準の意図は「凍結時点での品質ベースライン」であり、
...
V01-ADR-057 §4 のRelease snapshots運用に従い、本ADR受理時点では新タグは切らない。
