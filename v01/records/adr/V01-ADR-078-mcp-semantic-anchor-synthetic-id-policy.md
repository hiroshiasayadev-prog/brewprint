# V01-ADR-078: MCP synthetic ID を semantic anchor based に見直す

- **status**: accepted
- **date**: 2026-05-13
- **depends_on**: V01-ADR-047, V01-ADR-048, V01-ADR-049, V01-ADR-054, V01-ADR-058, V01-ADR-070
...
これは、V01-ADR-058 で private sub node を project-wide QualifiedID から外し、file-private object として扱う方針を補強した流れから生じた表現である。
V01-ADR-058 の主眼は、private sub node を外部参照可能な QualifiedID 一意性制約から外し、同一 file 内だけで解決できるようにすることだった。
...
V01-ADR-070 では file-private helper model を導入し、MCP response 内では private sub node と同じく synthetic ID を使う方針を記録した。
...
Codex review では、V01-ADR-078 の大方針は妥当だが、transition ID は現行 spec / implementation / tests に深く file path based ID として組み込まれており、V01-ADR-078 に含めて acceptance するには未確定要素が大きすぎると指摘された。
...
V01-ADR-070 の file-private helper model 方針は維持する。
...
2. Opus review
   ...
   - transition ID policy を別ADRに分割する前提で、V01-ADR-078 が総論として十分か確認する
...
これは、MCP が Raw YAML AST ではなく semantic object query layer であるという V01-ADR-047 / MCP spec の基本方針と相性が悪い。
...
したがって、transition ID を V01-ADR-078 に含めて確定しようとすると、MCP synthetic ID の総論が transition / state 設計全体に引きずられる。
本ADRでは transition を unresolved scope として切り出し、後続ADRで扱う。

### なぜ V01-ADR-058 を直接修正しないか

V01-ADR-058 は起票時点で、private sub node を project-wide QualifiedID から外し、file-private scope を実装で保証するための判断を記録したものである。

V01-ADR-058 の本文はスナップショットとして残す。
本ADRは、V01-ADR-058 の private object 方針を否定するものではなく、MCP public contract に出す synthetic ID の anchor を file path から semantic object へ寄せる補強・再整理である。
...
- 欠点: render / inspect / implementation review で private sub node や helper model の詳細を取得できない。V01-ADR-054 の設計対話 coverage と V01-ADR-070 の helper model query 需要に合わない
...
- 利点: ID体系は単純になる
- 欠点: private object が外部参照可能に見える。V01-ADR-011 / V01-ADR-058 / V01-ADR-070 の file-private 方針に反する。public surface が過剰に広がる

→ 却下。private object は public QualifiedID を持たない。

### 代替案E: transition ID も V01-ADR-078 で確定する

- 利点: MCP synthetic ID policy を1本のADRで完結できる
- 欠点: state file / state machine / guard exact match / transition uniqueness まで同時に決める必要があり、V01-ADR-078 のスコープを超える。現行実装・テストへの破壊範囲も大きい
...
V01-ADR-058 の private sub node scope 方針は維持する。

ただし、V01-ADR-058 由来で MCP spec に導入された `<file-id>#<local-id>` 形式は、本ADRにより見直し対象となる。
V01-ADR-058 本文は遡及修正しない。
必要であれば、V01-ADR-058 の後続補強ADRとして本ADRを参照し、spec 側で現行仕様を更新する。
...
8. transition ID policy を後続ADRへ分割する前提で、V01-ADR-078 が総論ADRとして十分に閉じているか確認する
...
- 参考: V01-ADR-047 MCP query layer boundary、V01-ADR-048 ResolvedProject index、V01-ADR-049 MCP reference vocabulary、V01-ADR-054 design conversation coverage、V01-ADR-058 private sub node scope、V01-ADR-070 file-private helper model、Codex review of V01-ADR-078
