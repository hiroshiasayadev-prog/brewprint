# V01-ADR-098: 既存 artifact ID の v2 grammar への一括移行

- **status**: superseded
- **date**: 2026-06-08
- **depends_on**: V01-ADR-096, V01-ADR-097
- **supersedes**: V01-ADR-096（「既存 artifact ID 変更なし」の決定のみ）
- **superseded_by**: V01-ADR-099
- **migrated_to_spec**: 

## 背景

V01-ADR-096 は「既存 artifact ID は変更しない」と決定した。しかしその後、V01-ADR-097 により app namespace-first 物理レイアウトへの移行が確定した。

物理レイアウト移行と ID 移行が不一致のまま進むと以下の問題が生じる。

- **UI（V01-REQ-MCP-013）**: artifact を app namespace 単位で表示する際、`V01-REQ-MCP-001`（旧形式）と `DRMCP-REQ-MCP-033`（新形式）が混在し、表示ロジックに「旧形式か否か」の分岐が必要になる。
- **MCP 挙動**: namespace-aware な browse / discovery API を実装する際にも、同様の形式分岐が必要になる。

物理レイアウト移行（V01-WORK-PRODUCT-003）のタイミングで ID も揃えることで、分岐コストを永続させずに済む。現時点では artifact 数が有限であり、script による一括変換が現実的である。
...
**移行コストの前倒し**: 物理レイアウト移行（V01-WORK-PRODUCT-003）と同一作業ウィンドウで実施することで、二度手間を避ける。artifact 数は現時点で有限（docs 634 ファイル）であり、script による自動変換が現実的。

**PRODUCT domain の明示化**: `V01-REQ-PRODUCT-003` の `PRODUCT` が app namespace なのか domain なのか曖昧だった問題が解消される。
...
- Go コードに artifact ID 形式の validator（正規表現等）が存在する場合、新形式に対応した更新が必要（V01-WORK-PRODUCT-003 内で確認・対応）
- V01-ADR-096 の「既存 artifact ID 変更なし」の決定を本 ADR が supersede する。V01-ADR-096 の他の決定（所有権・物理 migration タイミング）は本 ADR のスコープ外
- namespace-model spec の mapping rule セクション（論理マッピング規則の参照用途として記載）は migration 完了後に削除または「移行済み」として更新する

## Evidence

- V01-ADR-096: 既存 artifact の PRODUCT namespace 所有と per-app migration 非実施（本 ADR が ID 変更なし決定を supersede）
- V01-ADR-097: app namespace-first repository directory layout の採用
- V01-REQ-MCP-013: Design Records MCP record browser UI（UI 実装時の形式分岐問題の根拠）
...
- V01-INV-PRODUCT-001: migration impact inventory（変換対象スケールの根拠）
