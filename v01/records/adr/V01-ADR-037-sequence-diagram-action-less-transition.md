# V01-ADR-037: sequence diagram における action なし transition の描画ルール
...
V01-ADR-019 で `transition.action` は任意フィールドと定義されている。
...
V01-ADR-004 / V01-ADR-032 / V01-ADR-036 はいずれも「transition.action → task → endpoint」という経路で
...
ラベル選択の原則（V01-ADR-036）を踏襲して event ID を採用する。
...
A（パーサーエラー）は「V01-ADR-004 の粗い粒度＝レイヤー越えのみ」という読み過ぎに基づく案。
V01-ADR-004 は粒度の方針を定めるものであり、UI 内遷移の描画を禁止していない。
...
D（Note）は V01-ADR-004 で「例外・エラーフローの表現」として位置づけられており、
happy path のフロー記述に note を使うのは誤用になる。

### `uiChange` のような新 source 種別を設けなかった理由

`source: ui` かつ action なし、という条件は既存フィールドの組み合わせで表現できる。
新フィールドを増やすと V01-ADR-018 の source 定義・V01-ADR-036 の矢印ルール表を拡張する必要が生じ、
既存概念との整合コストが高い。最小変更で解決できるため新 source は設けない。

## 影響

- `spec/views/sequence-diagram.md` の矢印ラベル表に action なしのケースを追記する
- V01-ADR-036 のルール表は `event.source` ベースであり action の有無は扱っていないため、
  本 ADR は V01-ADR-036 の **補完** として位置づける（supersede はしない）
