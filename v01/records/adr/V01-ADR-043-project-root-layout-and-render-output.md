# V01-ADR-043: プロジェクトルートレイアウトとrender出力構造
...
3. **render_index.yaml によるグルーピング制御**: スキーマ詳細はV01-ADR-045（仕様詳細: [project-layout.md](../spec/project-layout.md) §4）
...
State / Sequence / Wireframe / Preview の詳細な配置規則はV01-ADR-046で規定する。
`render_index.yaml` のスキーマ詳細はV01-ADR-045で規定する。
...
### modules フィールドとV01-ADR-010 / V01-ADR-027との整合

`groups[].modules` はV01-ADR-010が定義する「モジュール = ディレクトリ」と同義のmodule名を指定する。
V01-ADR-027のmodule nestingとの整合（ネストしたmoduleを指定したとき子moduleを含むかどうか等）はV01-ADR-045で規定する。
