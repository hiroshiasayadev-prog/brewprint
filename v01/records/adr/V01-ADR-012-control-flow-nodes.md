# V01-ADR-012: 制御フローノード（branch / fork / join）
...
V01-ADR-011にて `foreach` / `cond` / `initializes` の設計をV01-ADR-012に委譲していた。
議論の過程でこれらは概念が異なることが明確になったため、本ADRでは制御フローノード（条件分岐・並列実行・合流）のみを扱う。`foreach` はV01-ADR-013、`initializes` はV01-ADR-014に委譲。

また、V01-ADR-011では `cond` という仮称を使っていたが、本ADRで正式名称を確定する。
...
`cond` はLisp/関数型言語の慣習。DAGツールの文脈では Airflow の `BranchPythonOperator` に由来する `branch` が定着しており、V01-ADR-006の方針（業界標準の語彙から採る）と一致する。
...
- V01-ADR-011で使われていた仮称 `cond` は本ADRにより `branch` に確定
- `foreach` の設計は V01-ADR-013 へ
- `initializes` の設計は V01-ADR-014 へ
- ファイル内edgeの記述構造（`fork`-`join` の紐付け方法を含む）は V01-ADR-015 へ委譲
