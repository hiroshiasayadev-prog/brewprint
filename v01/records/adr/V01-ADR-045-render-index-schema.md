# V01-ADR-045: render_index.yaml スキーマ
...
V01-ADR-043でbrewprintプロジェクトのルートに `render_index.yaml` を置き、`renders/` のgroup構成を制御することを決定した。
...
- V01-ADR-027のmodule nestingとの整合（ネストしたmoduleを指定したときの子moduleの扱い）
...
V01-ADR-043で「group idにアンダースコア始まりはvalidation error」と定めている。
V01-ADR-027はmodule名の命名規則を規定していないが、アンダースコア始まりのmodule名を使うと暗黙group idが同じくアンダースコア始まりになり、validation errorと衝突する。
...
V01-ADR-027のmodule nestingは名前解決のためのルールであり、groupingはそれに依存する形で動く。
