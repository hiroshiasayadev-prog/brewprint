...
- **status**: accepted
- **date**: 2026-06-01
- **depends_on**: V01-ADR-076, V01-ADR-077, V01-ADR-092
- **supersedes**:
...
That split makes authoring path-oriented even when the user intent is artifact-oriented, such as `REQ-MCP-new`, `V01-WORK-MCP-008`, or `V01-TASK-MCP-008-01`.

V01-REQ-MCP-008 captures the need to add authoring transaction support. V01-TASK-MCP-008-01 confirmed that the absence of write capability is not a bug in the current MCP; it is a deliberate public contract gap in the read-only MVP.
...
- source requirement: V01-REQ-MCP-008
- work item: V01-WORK-MCP-008
- evidence task: V01-TASK-MCP-008-01
- ADR drafting task: V01-TASK-MCP-008-02
- ADR review task: V01-TASK-MCP-008-03
- related guidance capability: V01-REQ-MCP-005 / V01-WORK-MCP-005
- related workflow navigation capability: V01-REQ-MCP-007 / V01-WORK-MCP-007
- current tools spec: SPEC-design-records-mcp-tools
- decision ID suggestion: `suggest_next_record` returned `V01-ADR-093` for this title.
...
- spec reflection review: review reported `Needs revision before implementation`; spec skeleton creation / `SPEC-new` was excluded from MVP and spec placement discovery was captured as V01-REQ-MCP-010.
- impl commit: tbd
