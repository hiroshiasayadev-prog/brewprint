# V01-ADR-049: MCP / QueryService の reference 語彙統一

- **status**: accepted
- **date**: 2026-04-27
- **supersedes**: V01-ADR-047, V01-ADR-048
- **depends on**: V01-ADR-047, V01-ADR-048

## 背景

V01-ADR-047では、QueryService の責務として `GetSignature` / `GetDeps` / `Inspect` を仮置きした。
V01-ADR-048でも、MCP tool が `get_deps` を返却材料として使う前提の記述が残っている。
...
V01-ADR-047で仮置きされた `GetDeps` は廃止し、Go実装上のQueryService method名も `GetReferences` とする。
...
V01-ADR-048の方針通り、v1ではtransitive dependency graphを事前構築しない。
...
### V01-ADR-047 / V01-ADR-048を全面supersedeしない理由

V01-ADR-047のQueryService境界、V01-ADR-048のResolvedProject index strategyは継続有効。
変更対象は `GetDeps` / `get_deps` という命名部分のみである。

そのため、本ADRはV01-ADR-047/048全体をsupersedeせず、命名部分だけを上書きする。
...
- V01-ADR-047の `GetDeps` 記述は `GetReferences` と読み替える。
- V01-ADR-048の `get_deps` / `GetDeps` 記述は `get_references` / `GetReferences` と読み替える。
