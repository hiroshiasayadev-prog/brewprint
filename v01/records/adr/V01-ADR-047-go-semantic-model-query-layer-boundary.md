...
V01-ADR-001〜046でbrewprintのYAML仕様、名前解決、各viewのrender規則、render出力配置はおおむね確定した。
...
- node registryに加え、reverse lookup index等の派生indexを構築する（詳細はV01-ADR-048）
...
reverse lookup index の具体戦略はV01-ADR-048で別途決める。
...
現時点では具体的な型を決める材料が不足しているため、V01-ADR-047では「RendererはRaw YAMLを読まない」という境界だけを固定し、view modelの必要性と型はDAG vertical slice中に判断する。
...
- V01-ADR-048で `ResolvedProject` のindex strategyを続けて決める必要がある。
