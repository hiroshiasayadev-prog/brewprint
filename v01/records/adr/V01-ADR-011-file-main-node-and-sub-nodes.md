# V01-ADR-011: 1ファイル=1メインノード + サブノード構造

- **status**: accepted
- **date**: 2026-04-19
- **supersedes**: V01-ADR-010

## 背景

V01-ADR-010は「1ファイル=1ノード」を原則とした。しかし以下のケースで実態と合わなくなることが明確になった。
...
**privateスコープ強制**：sub taskを外部参照可能にすると「どこからでも呼べる内部ヘルパー」が生まれ、モジュール境界が崩れる。CAの層構造強制というV01-ADR-010の思想と矛盾する。
...
**1ファイル=1subgraph**：mermaidのsubgraphはファイル単位の責務の境界を視覚化する。V01-ADR-010が目指した「設計と実装の乖離を構造的に防ぐ」仕組みをsubgraphレベルで担保する。

## 影響

- V01-ADR-010の「1ファイル=1ノード」は本ADRにより「1ファイル=1メインノード」に改訂
- V01-ADR-010のその他の決定（CA強制・ビュー自動導出・model/asset分離・ディレクトリ構造）は継続有効
- `spec/nodes.md`（未作成）にてメインノード・サブノードのフィールド定義を詳細化する
- `branch`（旧仮称 `cond`）/ `fork` / `join` の設計はV01-ADR-012に委譲
- `foreach` の設計はV01-ADR-013にて確定
- `initializes` の設計はV01-ADR-014にて確定
- ファイル内edgeの記述構造（control/data section）はV01-ADR-015に委譲
