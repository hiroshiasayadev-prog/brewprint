# V01-ADR-069: TypeRef container complexity と anonymous inline struct 不採用
...
V01-ADR-060 / `docs/spec/type-ref.md` では、`list<T>` / `dict<T>` を built-in container TypeRef として導入した。
...
v1.1 の TypeRef variant は V01-ADR-060 の最小構成を維持する。
