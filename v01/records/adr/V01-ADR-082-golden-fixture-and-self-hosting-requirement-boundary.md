...
- **status**: accepted
- **date**: 2026-05-16
- **depends_on**: V01-ADR-043, V01-ADR-050, V01-ADR-057, V01-ADR-068, V01-ADR-081, V01-ADR-083
- **supersedes**:
- **migrated_to_spec**:
...
V01-ADR-081 により `docs/requirements/` は要求・不足・要望を捕捉する layer として定義される。
V01-ADR-083 により、requirements / work-items / tasks / internal design / coverage / YAML の責務境界が整理された。
...
UC-001 は V01-ADR-057 により v1.0.0-spec の canonical fixture として固定されている。
...
### 4. V01-ADR-043 / project-layout の fixture 運用を refine する

V01-ADR-043 および `docs/spec/project-layout.md` は、通常 project の render output として `renders/` を定義している。
...
V01-ADR-083 の artifact placement decision rule に従い、以下のように分解する。
...
これは V01-ADR-083 の `docs/coverage/` が定義する project-level trace coverage とは別概念である。
...
V01-ADR-083 の責務境界に従い、self-hosting の内容は artifact ごとに分解する。
...
- 欠点: requirements が内部設計・coverage・work item・task を抱え、V01-ADR-083 の artifact boundary と衝突する
...
本ADRは V01-ADR-083 の artifact boundary を前提とする。
...
V01-ADR-043 / project-layout の `renders/` 方針は通常 project の render output として維持し、fixture expected output は refinement として追加する。
...
V01-ADR-081 の requirement traceability と V01-ADR-083 の semantic ref 方針と連携し、fixture-local coverage と project-level coverage を区別する必要がある。
...
- commit: a80ec7b
- impl commit: tbd
- 参考: V01-ADR-043 project root layout and render output, V01-ADR-050 spec-first documentation policy, V01-ADR-057 brewprint v1 snapshot, V01-ADR-068 ADR authoring guide, V01-ADR-081 project requirements layer と semantic traceability, V01-ADR-083 project artifact boundary と YAML as primary implementation source
