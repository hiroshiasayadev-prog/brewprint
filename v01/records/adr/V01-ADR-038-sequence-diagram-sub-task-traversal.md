# V01-ADR-038: sequence diagram における sub task reads/writes の辿り方
...
V01-ADR-011 で「1ファイル = 1 main task + 複数の sub task」構造を確定した。
V01-ADR-020 で `reads` / `writes` は task node のフィールドとして定義された。
V01-ADR-032 / V01-ADR-036 / V01-ADR-037 で sequence diagram の描画ルールを確定したが、
...
- 辿る範囲は**ファイル境界まで**（V01-ADR-011 の「1ファイル = 1 subgraph」と一致）
...
| `sub_task` | sub task の short ID（ファイル内private、V01-ADR-011）。main task 自身の `reads` / `writes` の行は `-` |
...
C は V01-ADR-011 の「1ファイル = 1 subgraph」境界と自然に一致し、sub task が main task の「inlined helper」である性質と整合する。採用。
...
V01-ADR-011 で sub task はファイル内privateと定義されており、外部参照可能なIDを持たない。`task` 列が qualified ID を持つため、`(task, sub_task)` のペアでファイル内一意に特定できる。`sub_task` 列は short ID で十分。
