...
ドキュメント運用は **spec-first** で行う（V01-ADR-050）。
...
- `SPEC-new` / spec skeleton creation は V01-REQ-MCP-008 MVP 外であり、spec placement discovery は V01-REQ-MCP-010 の後続対象とする。
...
V01-ADR-057 で brewprint v1.0.0-spec を凍結した際に、以下の運用が確定した。
...
詳細は V01-ADR-057 を参照する。

### DISCLAIMER.md

- プロジェクト root に `DISCLAIMER.md` を新設する方針が確定（V01-ADR-057）。
