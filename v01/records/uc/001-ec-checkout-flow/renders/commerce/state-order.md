...
| payment_webhook_received | external | stripe | Stripe webhook受信（POST /api/stripe）。actor: stripe はプロジェクトglobalなactors.yamlに定義（V01-ADR-031）。payload.status で成否を判定し、guardで分岐する。 |
