...
| `fork` / `branches` / `join` | `yaml/order/task/checkout.yaml` | V01-ADR-040 の `branches[].steps[].step` + `steps[].params` 形式と `join: finalize_checkout` をカバーする。 |
...
| `branch.params` / `cases[].params` | `yaml/order/task/process_order.yaml` | `branch.params` はbranch node自身の判定入力、`cases[].params` はcase先taskへのwiringとして分離するV01-ADR-040形式をカバーする。 |
