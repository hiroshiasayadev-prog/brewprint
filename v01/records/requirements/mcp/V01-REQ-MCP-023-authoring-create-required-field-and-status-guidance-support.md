# V01-REQ-MCP-023: authoring create required field and status guidance support
...
- **Criterion 3** (task allowed values `todo / doing / blocked / done`): stale. V01-ADR-094 changed canonical values to `not_started / in_progress / blocked / done`.
- **Criterion 4** (synonym repair, e.g., `in_progress → doing`): resolved. `in_progress` is now canonical per V01-ADR-094; no synonym mapping needed.
- **Criterion 2** (`allowed_values` in status diagnostics): transferred to V01-REQ-MCP-024.
- **Criteria 1, 5, 6** (batch field validation, reciprocal follow-up surfacing, `include_required` mode clarity): transferred to V01-REQ-MCP-028.
