# V01-REQ-MCP-025: atomic multi-operation record update proposal support
...
This requirement assumes the field-level metadata update capability from `V01-REQ-MCP-020` for metadata-side operations. The intended metadata operation is `metadata_field_replace` / `metadata_fields_replace`, not requiring callers to resubmit the complete metadata block.

## Evidence

Closing workflow artifacts commonly requires writing Evidence and then changing status to `done`.

For `V01-TASK-DATA-011-03`, attempting to update `status` first hit metadata replacement friction and then the done-state Evidence gate:

```text
V01-TASK-DATA-011-03 を done に更新します。まずメタデータ更新。
```
...
- Metadata-side status update uses the field-level metadata update semantics from `V01-REQ-MCP-020`.
- Section-side update can use `named_section_replace`, including the heading-safe normalization behavior from `V01-REQ-MCP-022` when applicable.
