# V01-REQ-MCP-006: Workflow artifact metadata validation strictness が必要
...
`V01-WORK-SELFHOST-001` の `status` に `fixture_pending` が入り、Design Records MCP の `validate_records(kind="work_item")` は `ok: true` を返した。
...
- 本 requirement は `V01-WORK-SELFHOST-001` または `V01-TASK-SELFHOST-001-02` を修正しない。
- `fixture_pending` を work item status から削除する要求ではない。
- V01-REQ-MCP-003 / V01-WORK-MCP-003 の完了済み workflow artifact MCP support を再オープンしない。
...
- 2026-06-01: `accepted`。V01-WORK-MCP-006 で扱い、current validation gap review から開始する。

本件は V01-REQ-MCP-003 の target chain 完了を否定する blocker ではない。V01-REQ-MCP-003 は workflow artifact record / resolver / declared relation validation の最小 public contractを完了済みである。

ただし、required workflow metadata と field value strictness は別の validation completeness issue として追跡する必要があるため、V01-REQ-MCP-006 として分離し、V01-WORK-MCP-006 で strictness 範囲の判断と実現を進める。

## Close evidence

- 2026-06-01: `V01-WORK-MCP-006` で requirement を解消した。
- Public contract:
...
- Scope boundaries preserved:
  ...
  - README / legacy authoring guide cleanup is tracked separately by `V01-REQ-MCP-009`.
