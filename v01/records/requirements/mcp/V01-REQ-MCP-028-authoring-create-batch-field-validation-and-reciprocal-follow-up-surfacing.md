# V01-REQ-MCP-028: authoring create batch field validation and reciprocal follow-up surfacing
...
This requirement covers three criteria carried forward from V01-REQ-MCP-023 that were not resolved by the vocabulary alignment work (V01-ADR-094) and are not covered by V01-REQ-MCP-024:
...
The full failure sequence is documented in V01-REQ-MCP-023. This requirement carries forward only the criteria not resolved by vocabulary alignment (V01-ADR-094) or covered by structured repair guidance format (V01-REQ-MCP-024).
...
- Diagnostic shapes for missing fields are consistent with the structured diagnostic format defined by V01-REQ-MCP-024.

## Explicitly Excluded Scope

- Status vocabulary or enum definition (addressed by V01-ADR-094 and V01-REQ-MCP-026).
- Synonym repair suggestion mapping (resolved by V01-ADR-094; canonical vocabulary is now `not_started` / `in_progress`).
- `allowed_values` structured format for invalid status diagnostics (covered by V01-REQ-MCP-024).
- Removing `accept_proposed_write` guard checks.
- Automatically accepting proposals after repair guidance is produced.

## Boundary

This requirement belongs to the Design Records MCP authoring create contract. It is the narrowed successor to V01-REQ-MCP-023, carrying forward only the criteria not resolved by V01-ADR-094 (criterion 4: synonym repair; criterion 3: stale token list) or transferred to V01-REQ-MCP-024 (criterion 2: `allowed_values` diagnostic format). It complements V01-REQ-MCP-024 by addressing field-level batch validation and reciprocal follow-up surfacing.
