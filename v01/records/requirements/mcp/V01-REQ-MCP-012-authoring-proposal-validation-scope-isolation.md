# V01-REQ-MCP-012: Authoring proposal validation scope isolation
...
- `V01-WORK-DATA-010` has an invalid work item status and references absent `V01-TASK-DATA-010-03`;
- `V01-TASK-MCP-005-01` through `V01-TASK-MCP-005-03` are missing metadata required by the current task validation contract.
...
It does not own authoring input normalization issues such as `body` versus `fields`, duplicate `id` placement, or domain case normalization. Those are captured separately by `V01-REQ-MCP-011`.
