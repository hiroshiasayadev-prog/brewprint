# V01-REQ-MCP-026: status vocabulary alignment across workflow artifact kinds
...
During TASK-DATA-011 authoring and the investigation that produced V01-REQ-MCP-023, the following failures occurred in sequence:
...
V01-REQ-MCP-023 proposes synonym repair as a band-aid. This requirement addresses the underlying vocabulary design so that the band-aid is not the permanent solution.
...
- After alignment, the synonym repair behavior introduced by V01-REQ-MCP-023 may be narrowed or removed if the underlying confusion source is eliminated.
...
This requirement belongs to the Design Records MCP schema and vocabulary contract. It depends on V01-REQ-MCP-023 for the evidence of confusion and the immediate diagnostic improvement. It supersedes the need for a permanent synonym mapping table once vocabulary alignment is complete.
