# V01-REQ-MCP-020: propose_record_update metadata field replace support
...
Runtime authoring attempt for `V01-TASK-DATA-011-01` failed when trying to update only `status` via `metadata_block_replace`:
...
This requirement belongs to the MCP authoring update contract. The observed target record was `V01-TASK-DATA-011-01`, but the missing capability is not DATA-domain behavior. It is a Design Records MCP write/update ergonomics and correctness gap.
