# V01-WORK-MCP-011: Authoring proposal contract and validation fixes
...
This work item primarily closes `V01-REQ-MCP-011` and also includes the closely coupled validation-scope fix captured by `V01-REQ-MCP-012`.
...
- This work item does not reopen `V01-REQ-MCP-008` / `V01-WORK-MCP-008`; it treats authoring transaction support as existing functionality that now needs bug-fix hardening.
...
|---|---|
| requirements | `V01-REQ-MCP-011` is the source requirement; `V01-REQ-MCP-012` is included as coupled scope and should be related by task evidence or a follow-up work item if split becomes necessary |
| ADR | `V01-ADR-093` is checked for authoring transaction model consistency; supersession is not expected unless the transaction model itself changes |
...
```mermaid
flowchart TD
  T1["V01-TASK-MCP-011-01: Reproduce authoring proposal dogfooding failures"]
  T2["V01-TASK-MCP-011-02: Specify input normalization and validation scope"]
  T3["V01-TASK-MCP-011-03: Implement authoring proposal fixes"]
  T4["V01-TASK-MCP-011-04: Regression tests and runtime smoke"]
  T1 --> T2 --> T3 --> T4
```

## Task Candidates

- `V01-TASK-MCP-011-01`: Reproduce and classify the observed `propose_record_create` failures: `body`/`fields` precedence, `fields.id` duplication, domain case mismatch, and unrelated diagnostics during proposal validation.
- `V01-TASK-MCP-011-02`: Update the MCP tools/schema spec to define the accepted behavior for authoring create inputs and proposal validation scope.
- `V01-TASK-MCP-011-03`: Patch Design Records MCP implementation for input normalization and validation scope isolation.
- `V01-TASK-MCP-011-04`: Add regression tests, run targeted Go tests, perform no-write runtime smoke, and update close evidence.

## Completion Condition

The work item can be closed when:

- `V01-REQ-MCP-011` input normalization behavior is specified and covered by tests.
- `V01-REQ-MCP-012` proposal validation scope behavior is specified and covered by tests or explicitly split into a follow-up work item with preserved traceability.
...
- 2026-06-02: Created from dogfooding failures in Design Records MCP authoring transactions. Initial scope intentionally covers both `V01-REQ-MCP-011` and coupled `V01-REQ-MCP-012` because the bugs appeared in the same authoring proposal path.
- 2026-06-02: Completed `V01-TASK-MCP-011-02` spec contract update for create input normalization and proposal-local validation scope; implementation remains pending for `V01-TASK-MCP-011-03`.
- 2026-06-02: Completed `V01-TASK-MCP-011-03` implementation for create input normalization, case-insensitive domain comparison, proposal-local validation filtering, and semantic ref source preservation; targeted and package Go tests passed.
- 2026-06-02: Completed `V01-TASK-MCP-011-04` regression and runtime smoke verification. Targeted and package Go tests passed; JSON-RPC smoke confirmed create input normalization, lowercase domain handling, proposal-local diagnostic isolation, and accept-time pre-write validation. No unrelated repository validation errors were fixed.
