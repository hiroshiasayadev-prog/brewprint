# V01-WORK-MCP-018: Normalize propose_record_create fields-required retry contract
...
`V01-REQ-MCP-019` を解消するため、`propose_record_create` の正規 create contract を fields-required / section-only body source 前提へ整理する。
...
- `fields + body` の section-only create 方針は `V01-WORK-MCP-014` の決定を前提とする。
- failed propose response の body_cache return behavior は `V01-WORK-MCP-012` の close 済み成果を前提とし、再オープンしない。
...
1. `V01-TASK-MCP-018-01` で current contract、legacy mode、retry gap をレビューする。
2. `V01-TASK-MCP-018-02` で strict fields-required contract の test / runtime 影響範囲を調査する。
3. `V01-TASK-MCP-018-03` で spec / guidance を strict contract に更新する。
4. `V01-TASK-MCP-018-04` で実装と regression tests を行う。
5. `V01-TASK-MCP-018-05` で runtime smoke、validation、REQ / WORK close synchronization を行う。

## Task Candidates

- `V01-TASK-MCP-018-01`: current contract / legacy mode / retry gap review
- `V01-TASK-MCP-018-02`: strict fields-required create contract impact assessment
- `V01-TASK-MCP-018-03`: spec and guidance update for strict fields-required create contract
- `V01-TASK-MCP-018-04`: implementation and regression tests
- `V01-TASK-MCP-018-05`: runtime smoke and close synchronization
...
- `V01-REQ-MCP-019` の status / work_items / evidence が同期されている。

## Evidence

- `V01-TASK-MCP-018-01` reviewed the current create contract and retry gap.
- `V01-TASK-MCP-018-02` completed strict contract impact assessment and confirmed all breaking tests were scoped to implementation.
- `V01-TASK-MCP-018-03` updated `docs/spec/design-records-mcp/tools.md` and `docs/spec/design-records-mcp/schema.md` for strict fields-required create contract.
- `V01-TASK-MCP-018-04` implemented strict `propose_record_create` behavior and regression tests.
- `V01-TASK-MCP-018-05` completed runtime smoke through actual Design Records MCP stdio JSON-RPC path.
