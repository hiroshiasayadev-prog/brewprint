# V01-WORK-MCP-020: Add metadata field replacement support to propose_record_update
...
- `V01-ADR-093`: check whether the authoring transaction model needs a short compatibility note or no change.
...
V01-TASK-MCP-020-01 -> V01-TASK-MCP-020-02 -> V01-TASK-MCP-020-03 -> V01-TASK-MCP-020-04

## Task Candidates

- `V01-TASK-MCP-020-01`: Review current metadata update contract and implementation boundary.
- `V01-TASK-MCP-020-02`: Update the MCP tools spec for metadata field replacement.
...
- `V01-REQ-MCP-020`, `V01-WORK-MCP-020`, and all related tasks are synchronized to their final statuses.
...
- Runtime smoke confirmed `metadata_fields_replace` can update only `V01-TASK-MCP-020-04.status` to `doing` via proposal / accept flow while preserving `work_item`, `source_requirement`, `depends_on`, and `outputs`.
