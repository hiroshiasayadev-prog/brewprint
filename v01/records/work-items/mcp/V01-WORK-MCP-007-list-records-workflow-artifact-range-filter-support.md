# V01-WORK-MCP-007: list_records workflow artifact range filter support を判断・実現する
...
V01-REQ-MCP-007 を解消するため、Design Records MCP の `list_records` で workflow artifact を ID range または同等の domain-scoped sequence filter により安全に探索できるようにする。
...
- V01-WORK-DATA-004 の仕様判断自体は再オープンしない。本 work item では、workflow artifact navigation の利用例として扱う。

## Impact scope

| layer | expected handling |
|---|---|
| requirement | V01-REQ-MCP-007 の gap を解消する |
...
```mermaid
flowchart TD
  T1["V01-TASK-MCP-007-01: Current list_records range contract gap review"]
  T2["V01-TASK-MCP-007-02: Workflow artifact range filter contract decision"]
  T3["V01-TASK-MCP-007-03: MCP tools spec update"]
  T4["V01-TASK-MCP-007-04: Implementation and tests"]
  T5["V01-TASK-MCP-007-05: Runtime verification and close evidence"]
  T1 --> T2 --> T3 --> T4 --> T5
```

## Task Candidates

- `V01-TASK-MCP-007-01`: Current `list_records.id_range` contract / implementation / tests の evidence を整理する。
- `V01-TASK-MCP-007-02`: `id_range` 拡張か、workflow artifact 専用 filter 追加かを判断する。
- `V01-TASK-MCP-007-03`: 採用 contract を Design Records MCP tools spec に反映する。
- `V01-TASK-MCP-007-04`: Parser / index / MCP tool schema / tests を実装する。
- `V01-TASK-MCP-007-05`: Runtime verification と close review を実施する。

`V01-TASK-MCP-007-01` through `V01-TASK-MCP-007-05` are materialized.
...
- V01-REQ-MCP-007 への relation と close evidence が整合している。
...
- 2026-06-01: V01-REQ-MCP-007 から起票。現行 spec では `list_records.id_range` は `ADR-NNN` decision record 専用であり、workflow artifact の range navigation は後続 refinement に委ねられていた。
...
- `V01-TASK-MCP-007-01` through `V01-TASK-MCP-007-05` are done.
- `validate_records(kind: requirement, id_range: V01-REQ-MCP-007..V01-REQ-MCP-007)` returned `ok: true`.
- `validate_records(kind: work_item, id_range: V01-WORK-MCP-007..V01-WORK-MCP-007)` returned `ok: true`.
- `validate_records(kind: task, id_range: V01-TASK-MCP-007-01..V01-TASK-MCP-007-05)` returned `ok: true`.
- `list_records(kind: work_item, id_range: V01-WORK-MCP-007..V01-WORK-MCP-007)` returned `V01-WORK-MCP-007`.
- `list_records(kind: task, id_range: V01-TASK-MCP-007-01..V01-TASK-MCP-007-05)` returned `V01-TASK-MCP-007-01` through `V01-TASK-MCP-007-05`.
