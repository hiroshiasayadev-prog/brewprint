# V01-WORK-PRODUCT-004: Repository layout migration execution — v01/ restructure and V01- ID rename

- **id**: V01-WORK-PRODUCT-004
- **status**: not_started
- **date**: 2026-06-08
...
V01-ADR-097（app namespace-first layout）と V01-ADR-099（V01- epoch prefix）で確定した方針に基づき、repository の物理移行を実行する。
...
- parser の namespace prefix 導出ルール実装（ディレクトリ名を大文字化して `-` を付与: `v01/` → `V01-`）
- CLAUDE.md 等のパス参照更新

このWORKが所有しないもの:

- MCP の multi-root 対応（v01/ + 各 app records/ を横断インデックスする機能）
- 各 app namespace の `records/` を scan root にする移行（`drmcp/records/` 等）は v02 スコープとして別 REQ/WORK を起票する。本 WORK の parser 変更は `v01/records/` 向けの namespace 導出ルール実装にとどめる
...
- TASK-02: `drmcp/records/spec/`・`bpdsl/records/spec/`・`product/records/spec/` を作成し、V01-INV-PRODUCT-001 の分類に基づき対応する spec ファイルを v01/ からコピーする。CLAUDE.md 等のパス参照を更新する
...
- V01-REQ-PRODUCT-003 の migration target 宣言が物理的に実現されている
