...
import (
	...
	"github.com/hiroshiasayadev-prog/brewprint/drmcp/src/internal/designrecordsmcp"
)
...
func run(args []string, stdin io.Reader, stdout io.Writer, stderr io.Writer) error {
	...
	recordsRoot := flags.String("records-root", "", "records directory relative to root (default: v01/records)")
	summary := flags.Bool("summary", false, "print a human-readable index summary and exit")
	if err := flags.Parse(args); err != nil {
		return err
	}

	cfg, err := designrecords.NewConfig(*root, *recordsRoot)
	if err != nil {
		return err
	}
	if !*summary {
