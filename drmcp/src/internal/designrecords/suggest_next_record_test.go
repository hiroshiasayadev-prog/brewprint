...
func TestSuggestNextRecordBasicDecisionResponse(t *testing.T) {
	...
	wantPath := "records/adr/ADR-078-design-records-mcp-implementation-package-layout.md"
...
func TestSuggestNextRecordNumberingRules(t *testing.T) {
	root := t.TempDir()
	writeTestFile(t, root, "records/adr/ADR-001-one.md", "# ADR-001: One\n- **status**: accepted\n")
	writeTestFile(t, root, "records/adr/077-seventy-seven.md", "# ADR-077: Seventy seven\n- **status**: accepted\n")
	writeTestFile(t, root, "records/adr/099-ninety-nine.md", "# ADR-099: Ninety nine\n- **status**: accepted\n")
	writeTestFile(t, root, "records/adr/100-one-hundred.md", "# ADR-100: One hundred\n- **status**: accepted\n")
	writeTestFile(t, root, "records/adr/999-invalid-h1.md", "# 999: Invalid H1\n- **status**: accepted\n")
	...
	if resp.SuggestedPath != "records/adr/ADR-101-next.md" {
...
func TestSuggestNextRecordNoExistingDecisionsStartsAtADR001(t *testing.T) {
	...
	if resp.SuggestedPath != "records/adr/ADR-001-first-decision.md" {
		t.Fatalf("SuggestedPath = %q", resp.SuggestedPath)
	}
...
func TestSuggestNextRecordSlugGeneration(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name:  "ascii title",
			title: "Design Records MCP implementation package layout",
			want:  "records/adr/ADR-001-design-records-mcp-implementation-package-layout.md",
		},
		{
			name:  "mixed punctuation",
			title: "ADR: Foo / Bar",
			want:  "records/adr/ADR-001-adr-foo-bar.md",
		},
		{
			name:  "repeated separators",
			title: "Hello --- World___Again",
			want:  "records/adr/ADR-001-hello-world-again.md",
		},
		{
			name:  "leading and trailing separators",
			title: " / Hello, World! / ",
			want:  "records/adr/ADR-001-hello-world.md",
		},
		{
			name:  "uppercase lowercasing",
			title: "UPPER Case 123",
			want:  "records/adr/ADR-001-upper-case-123.md",
		},
		{
			name:  "non ascii only",
			title: "日本語タイトル",
			want:  "records/adr/ADR-001.md",
		},
		{
...
func TestSuggestNextRecordRepositoryBootstrap(t *testing.T) {
	...
	wantNextID := ns + formatDecisionRecordID(maxNum+1)
	if resp.NextNumber != maxNum+1 || resp.NextID != wantNextID || resp.ExistingMaxID != maxID {
		t.Fatalf("response = %#v, want max %s plus one", resp, maxID)
	}
	wantPath := "v01/records/adr/" + ns + formatDecisionRecordID(maxNum+1) + "-repository-bootstrap-check.md"
	if resp.SuggestedPath != wantPath {
		t.Fatalf("SuggestedPath = %q, want %q", resp.SuggestedPath, wantPath)
	}
}
