...
func TestGetRecordBasicDecisionAndSpecResponseShape(t *testing.T) {
	root := t.TempDir()
	adrRaw := "# ADR-076: Design Records MCP\n\n" +
		"- **status**: accepted\n" +
		...
		"```\n\n" +
		"## Decision\n"
	writeTestFile(t, root, "records/adr/076-design-records-mcp.md", adrRaw)
	specRaw := "---\n" +
		"status: draft\n" +
		"design_record:\n" +
		...
		"## get_record\n"
	writeTestFile(t, root, "records/spec/design-records-mcp/tools.md", specRaw)
	investigationRaw := "# INV-DOCS-001: Test investigation\n\n" +
		...
		"## Result\n"
	writeTestFile(t, root, "records/investigations/docs/INV-DOCS-001-test.md", investigationRaw)
	...
	adr := adrResp.Record
	if adr.ID != "ADR-076" || adr.Kind != RecordKindDecision || adr.Title != "Design Records MCP" || adr.Status != RecordStatusAccepted || adr.Path != "records/adr/076-design-records-mcp.md" {
	...
	assertHeadings(t, adr.Headings, []Heading{
		{Level: 1, Text: "ADR-076: Design Records MCP"},
	...
	if spec.ID != "SPEC-design-records-mcp-tools" || spec.Kind != RecordKindSpec || spec.Title != "Design Records MCP tools" || spec.Status != RecordStatusDraft || spec.Path != "records/spec/design-records-mcp/tools.md" {
...
func TestGetRecordIncludeBodyReturnsRawMarkdown(t *testing.T) {
	...
	writeTestFile(t, root, "records/spec/raw-body.md", specRaw)
	idx := buildTestIndex(t, root)
...
func TestGetRecordDuplicateIDReturnsFirstIndexRecord(t *testing.T) {
	idx := &Index{Records: []Record{
		{ID: "ADR-001", Kind: RecordKindDecision, Title: "First", Path: "records/adr/001-first.md", RawBody: "# 001: First\n"},
		{ID: "ADR-001", Kind: RecordKindDecision, Title: "Second", Path: "records/adr/001-second.md", RawBody: "# 001: Second\n"},
	}}
	...
	if resp.Record.Title != "First" || resp.Record.Path != "records/adr/001-first.md" {
...
func TestGetRecordRepositoryBootstrapRecords(t *testing.T) {
	...
	adrResp, err := GetRecord(context.Background(), idx, GetRecordRequest{ID: "V01-ADR-076", IncludeBody: true})
	if err != nil {
		t.Fatalf("GetRecord V01-ADR-076: %v", err)
	}
	if adrResp.Record.Kind != RecordKindDecision || adrResp.Record.Title != "Design Records MCP" || adrResp.Record.Path != "v01/records/adr/V01-ADR-076-design-records-mcp.md" {
		t.Fatalf("V01-ADR-076 = %#v", adrResp.Record)
	}
	indexedADR := findRecord(idx.Records, "V01-ADR-076")
	if indexedADR == nil {
		t.Fatal("V01-ADR-076 missing from index")
	}
	if adrResp.Record.Body == nil || *adrResp.Record.Body == "" || *adrResp.Record.Body != indexedADR.RawBody {
		t.Fatalf("V01-ADR-076 raw body not returned from index")
	}
	if !hasHeading(adrResp.Record.Headings, Heading{Level: 1, Text: "V01-ADR-076: Design Records MCP"}) || !hasHeadingLevel(adrResp.Record.Headings, 2) {
		t.Fatalf("V01-ADR-076 headings = %#v", adrResp.Record.Headings)
	}
	assertGetRecordJSONShape(t, adrResp, true)

	withoutBody, err := GetRecord(context.Background(), idx, GetRecordRequest{ID: "V01-ADR-076"})
	if err != nil {
		t.Fatalf("GetRecord V01-ADR-076 without body: %v", err)
	}
	if withoutBody.Record.Body != nil {
		t.Fatalf("V01-ADR-076 body present when include_body=false")
	}

	specResp, err := GetRecord(context.Background(), idx, GetRecordRequest{ID: "V01-SPEC-design-records-mcp-tools"})
	if err != nil {
		t.Fatalf("GetRecord spec: %v", err)
	}
...
func TestGetRecordEmptyHeadingsJSONShape(t *testing.T) {
	idx := &Index{Records: []Record{{
		...
		Path:     "records/adr/001-no-headings.md",
		Headings: nil,
	}}}
