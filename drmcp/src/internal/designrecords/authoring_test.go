...
func TestAuthoringProposalAcceptAndLifecycle(t *testing.T) {
	...
	targetPath := filepath.Join(fx.root, "records", "tasks", "mcp", "TASK-MCP-001-01-first-task.md")
...
func TestAuthoringAcceptGuardsStaleTargetAndCreateCollision(t *testing.T) {
	...
	taskPath := filepath.Join(fx.root, "records", "tasks", "mcp", "TASK-MCP-001-01-first-task.md")
	...
	writeTestFile(t, fx.root, "records/requirements/mcp/"+create.Target.ResolvedID+"-other.md", "# "+create.Target.ResolvedID+": Other\n\n- **id**: "+create.Target.ResolvedID+"\n- **status**: captured\n- **date**: 2026-06-02\n- **source_refs**:\n- **work_items**:\n")
...
func TestAuthoringAcceptPartialMultiFileWriteReportsWrittenFiles(t *testing.T) {
	...
	parentPath := filepath.Join(fx.root, "records", "work-items", "mcp", "WORK-MCP-001-first-work.md")
...
func TestAuthoringCreateInputContractNormalization(t *testing.T) {
	...
	if !withoutFieldsID.ProposalCreated || withoutFieldsID.Target.Domain != "MCP" || !strings.Contains(withoutFieldsID.Target.Path, "records/requirements/mcp/REQ-MCP-052") || !strings.Contains(withoutFieldsID.Diff.Text, "- **id**: REQ-MCP-052") {
...
func TestAuthoringNamedSectionSelectors(t *testing.T) {
	...
	multiPath := filepath.Join(fx.root, "records", "tasks", "mcp", "TASK-MCP-001-01-first-task.md")
...
func TestProposeRecordUpdateRequiredHeadingCaseFallback(t *testing.T) {
	...
	t.Run("cross-kind required headings are not canonicalized", func(t *testing.T) {
		fx := newAuthoringFixture(t)
		reqPath := filepath.Join(fx.root, "records", "requirements", "mcp", "REQ-MCP-001-first-req.md")
...
func TestAuthoringProposalValidationIsAffectedRecordSetOnly(t *testing.T) {
	fx := newAuthoringFixture(t)
	writeTestFile(t, fx.root, "records/requirements/mcp/REQ-MCP-999-bad.md", "# REQ-MCP-999: Bad\n\n- **id**: REQ-MCP-999\n- **status**: captured\n- **date**:\n- **source_refs**:\n- **work_items**:\n")
	fx.idx = mustBuildIndex(t, fx.cfg)
...
func TestAuthoringHypotheticalIndexPreservesUnchangedSemanticRefSources(t *testing.T) {
	...
	hyp := buildHypotheticalIndex(fx.idx, []ProposedFile{{
		Path:       "records/tasks/mcp/TASK-MCP-001-01-first-task.md",
		Change:     "modify",
		RecordID:   "TASK-MCP-001-01",
		RecordKind: RecordKindTask,
		Content:    readTestFile(t, filepath.Join(fx.root, "records", "tasks", "mcp", "TASK-MCP-001-01-first-task.md")),
	}})
...
func TestExactIDSequenceGapWarning(t *testing.T) {
	...
	t.Run("WORK_fills_existing_gap", func(t *testing.T) {
		...
		writeTestFile(t, fx.root, "records/work-items/mcp/WORK-MCP-003-third.md",
...
func newAuthoringFixture(t *testing.T) authoringFixture {
	...
	writeTestFile(t, root, "records/requirements/mcp/REQ-MCP-001-first-req.md", "# REQ-MCP-001: First req\n\n- **id**: REQ-MCP-001\n- **status**: captured\n- **date**: 2026-06-01\n- **source_refs**:\n- **work_items**:\n  - WORK-MCP-001\n\n## Requirement\n")
	writeTestFile(t, root, "records/work-items/mcp/WORK-MCP-001-first-work.md", "# WORK-MCP-001: First work\n\n- **id**: WORK-MCP-001\n- **status**: in_progress\n- **date**: 2026-06-01\n- **source_requirement**: REQ-MCP-001\n- **impact_refs**:\n- **tasks**:\n  - TASK-MCP-001-01\n\n## Goal\n")
	writeTestFile(t, root, "records/tasks/mcp/TASK-MCP-001-01-first-task.md", "# TASK-MCP-001-01: First task\n\n- **id**: TASK-MCP-001-01\n- **status**: not_started\n- **date**: 2026-06-01\n- **work_item**: WORK-MCP-001\n- **source_requirement**: REQ-MCP-001\n- **estimate**: 0.5d\n- **depends_on**:\n- **outputs**:\n  - initial\n\n## Goal\n\n## Verification\n\n## Evidence\nold evidence\n")
	writeTestFile(t, root, "records/spec/test.md", "---\nscope: docs/spec/test.md\nstatus: draft\nauxiliary: keep-me\nsummary: '# Ignored'\ndesign_record:\n  id: SPEC-test\n  kind: spec\n  status: draft\n  depends_on:\n    - ADR-001\n---\n\n# Test spec\n\n```md\n## Ignored\n```\n\n## Real\n")
...
func writeAuthoringTaskRecord(t *testing.T, root string, status RecordStatus, sections string) {
	...
	content := "# TASK-MCP-001-01: First task\n\n" +
		...
		"  - initial\n\n" +
		sections
	path := filepath.Join(root, "records", "tasks", "mcp", "TASK-MCP-001-01-first-task.md")
