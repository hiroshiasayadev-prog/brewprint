...
> 由来: V01-ADR-077 §P0: MVP必須tool, V01-ADR-077 §P1: MVPに含めてもよい補助tool, V01-ADR-090 §決定, V01-ADR-093 §決定
...
> 由来: V01-ADR-077 §list_records の責務, V01-ADR-077 §get_record の責務
...
> 由来: V01-ADR-077 §list_records の責務
...
Task range はさらに同一 work sequence に限定し、`V01-TASK-MCP-006-01` .. `V01-TASK-MCP-007-05` のような work item をまたぐ task range は request error とする。

One-sided workflow range は、指定された endpoint の family / domain / work sequence scope 内で評価する。
たとえば `kind: work_item` と `id_range.from: V01-WORK-DATA-004` は `WORK-DATA-*` の sequence `004` 以上を対象にする。
...
`ids[]` は record index に対する exact lookup key としてのみ評価する。前後 whitespace の trim、case normalization、canonical reference resolution、input kind classification は行わない。したがって `spec:trace`、physical path、`adr-077`、` V01-ADR-077 `、または grammar に合わない workflow ID のような string input が indexed record ID と一致しない場合、tool error や `unsupported` ではなく item-level `not_found` とする。`V01-REQ-MCP-003` / `V01-WORK-MCP-003` / `V01-TASK-MCP-003-01` は index 済みの場合に found record を返す exact lookup key である。
...
V01-ADR-088 / V01-ADR-092 により、`internal-design:` / `coverage:` / `COV-*`、semantic realization relation、coverage mapping query、orphan workflow artifact diagnostics、progress projection、workflow traversal query は MVP required scope として扱わない。Investigation metadata の canonical workflow reference 拡張は `REQ-*` / `WORK-*` に限定し、同 field 内の `TASK-*` は unsupported reference として扱う。
...
`depends_on` は `ADR-*` / `SPEC-*` / `INV-*` の canonical record ID-as-ref を参照できる。したがって `V01-ADR-086` の `depends_on: V01-INV-DOCS-001` は contract 上 valid であり、investigation integration 実装前に返る `missing_depends_on_target` は既知 implementation gap である。M19 implementation 後は `V01-INV-DOCS-001` の index integration によりこの diagnostic が解消されなければならない。
...
> 由来: V01-ADR-077 §suggest_next_record の責務
...
> 由来: V01-REQ-MCP-012, V01-TASK-MCP-011-01
...
The canonical ID domain remains the uppercase domain segment in IDs such as `V01-REQ-MCP-011`.
...
Therefore `domain: "mcp"` and `id: "V01-REQ-MCP-011"` are consistent, while `domain: "data"` and `id: "V01-REQ-MCP-011"` are rejected as `invalid_request`.
...
`SPEC-new` and spec skeleton create are rejected in the MVP because spec record placement cannot be derived safely from ID alone. Spec placement discovery / domain tree support is tracked as V01-REQ-MCP-010.
...
| `id_range_requires_decision_kind` | legacy error code。V01-REQ-MCP-007 以前の decision-only `id_range` boundary を示す。新規 implementation では `invalid_id_range` を用いる |
...
> 由来: V01-ADR-077 §MVP外, V01-ADR-077 §理由, V01-ADR-093 §決定
