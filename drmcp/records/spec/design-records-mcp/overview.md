...
V01-ADR-088 / V01-ADR-092 により、MVP で必須とする resolver input は active `spec:` semantic ref、Design Records MCP が扱う record ID-as-ref (`ADR-*` / `SPEC-*` / `INV-*` / `REQ-*` / `WORK-*` / `TASK-*`)、investigation canonical reference validation、および workflow relation integrity validation とする。
