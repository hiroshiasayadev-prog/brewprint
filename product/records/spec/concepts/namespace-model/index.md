...
既存 V01-REQ-MCP-001〜032 は V01-ADR-096 上 PRODUCT-owned のままだが、namespace mapping 上の実質的帰属 / logical projection として DRMCP MCP domain に対応し、以下の subdomain が識別される。
...
V01-ADR-096 の決定に従い、既存 artifact の ID 変更は実施しない。この規則は UI / MCP でのグルーピング・表示・帰属解決のための参照規則として使う。

| 既存 domain prefix | app namespace | 変換規則 | 例 |
|---|---|---|---|
| `MCP` | `DRMCP` | `DRMCP-` を先頭に付加 | `V01-REQ-MCP-013` → `DRMCP-REQ-MCP-013` |
| `DATA` | `BPDSL` | `BPDSL-` を先頭に付加 | `V01-WORK-DATA-009` → `BPDSL-WORK-DATA-009` |
| `RESOLVE` | `BPDSL` | `BPDSL-` を先頭に付加 | `V01-REQ-RESOLVE-001` → `BPDSL-REQ-RESOLVE-001` |
...
| 現行形式 | v2 完全形 | 備考 |
|---|---|---|
| `V01-REQ-PRODUCT-001` | `PRODUCT-REQ-NAMESPACE-001` | namespace model 要件 |
| `V01-WORK-PRODUCT-001` | `PRODUCT-WORK-NAMESPACE-001` | namespace model 作業 |
| `V01-TASK-PRODUCT-001-01` | `PRODUCT-TASK-NAMESPACE-001-01` | namespace model タスク |
...
V01-ADR-096 の決定に従い、既存の全 artifact は `PRODUCT` namespace が所有するものとして扱う。per-app namespace への振り分け migration は実施しない。
...
新規 artifact は帰属 app namespace が確定している場合は `<APP_NAMESPACE>-...` 形式、cross-app または帰属不明な場合は `PRODUCT` namespace を使う。v2 ID grammar の詳細は V01-TASK-PRODUCT-001-03 で追加する。
