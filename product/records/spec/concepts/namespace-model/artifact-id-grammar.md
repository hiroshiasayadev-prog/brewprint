...
- **id**: `spec:product.concepts.namespace_model.artifact_id_grammar`
- **status**: draft
- **date**: 2026-06-24
...
Defines the canonical artifact ID grammar for new sequential records — ADR, investigation, requirement, work item, and task: public ID format, sequence format, allocation scopes, subdomain exclusion rule, and canonical-ref policy. Spec identity is governed by `spec:product.concepts.spec_format.spec_id_as_ref`.
...
> Subdomains are not segments in the artifact ID. They are expressed as a `subdomain` field in record metadata. See `spec:product.concepts.namespace_model.subdomain_model`.

> New ADRs use the grammar above. Existing issued ADR IDs are governed by `spec:product.concepts.namespace_model.legacy_id_compatibility`.
...
| `spec:product.concepts.namespace_model.legacy_id_compatibility` | Compatibility policy for issued V01-* legacy IDs. |
| `spec:product.concepts.namespace_model.subdomain_model` | Subdomain grouping; subdomains are not ID segments. |
| `spec:product.concepts.namespace_model.existing_artifacts` | Attribution policy for existing artifacts. |
| `spec:product.concepts.traceability.artifact_refs` | How canonical IDs function as semantic refs. |
