...
| ID-as-ref | Design Records MCP resolves complete public record IDs. Existing issued records retain legacy public IDs; new sequential records use canonical app-aware artifact IDs, while new and migrated specs use path-derived `spec:` refs. DRMCP also validates declared workflow relation integrity, but excludes physical paths, orphan diagnostics, progress projection, and workflow traversal. `COV-*` is outside MVP. |
...
| investigation trace | Resolve validation of `source_refs` and recorded `follow_up_results`. `follow_up_candidates` checks canonical form but does not require unresolved candidates to exist. Additional workflow ID-as-ref support is limited to complete requirement and work item public IDs; task public IDs are excluded from investigation metadata canonical references. |
