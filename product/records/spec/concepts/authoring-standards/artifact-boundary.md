...
- **id**: `spec:product.concepts.authoring_standards.artifact_boundary`
- **status**: draft
- **date**: 2026-06-23
...
Use this spec to select the correct artifact kind before writing. Canonical artifact ownership is defined by `spec:product.concepts.project_artifact_model.artifact_responsibility_matrix`. When wording conflicts between this spec and the canonical source, the canonical source wins.
...
Metadata, file shape, status lifecycle, and authoring interface rules for each kind are defined in the corresponding per-artifact authoring guide under `spec:product.concepts.authoring_standards`.
