...
- **id**: `spec:product.design_records.spec_format.overview`
- **status**: accepted
- **date**: 2026-06-11
...
This overview owns the entry-level framing for the spec-format area. The detailed rules are split into focused child specs under `spec:product.design_records.spec_format`.
...
| record identity | H1-adjacent `- **id**:` marker. For new or migrated specs, the value is the path-derived canonical spec ref, for example `spec:product.design_records.spec_format`. | Keeps identity visible and directly referenceable while preserving one-to-one path/ref mapping. |
...
| topic | ref |
|---|---|
| Document shape | `spec:product.design_records.spec_format.document_shape` |
| Topics table | `spec:product.design_records.spec_format.topics_table` |
| Spec ID-as-ref | `spec:product.design_records.spec_format.spec_id_as_ref` |
| Validation policy | `spec:product.design_records.spec_format.validation_policy` |
| Follow-up boundary | `spec:product.design_records.spec_format.follow_up_boundary` |

## Related specs

| ref | relation |
|---|---|
| `spec:product.design_records.spec_format` | Parent Index and authoritative topic declaration for this child spec. |
| `spec:product.design_records.namespace_model` | Defines app and domain namespace model. |
| `spec:product.design_records.repository_layout` | Defines namespace-first repository layout. |
