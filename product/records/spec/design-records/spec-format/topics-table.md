...
- **id**: `spec:product.design_records.spec_format.topics_table`
- **status**: accepted
- **date**: 2026-06-11
...
Each row targets the child spec by canonical `spec:` ref. Tooling resolves `ref` to a path through the path-derived canonical mapping defined by `spec:product.design_records.spec_format.spec_id_as_ref`; the table does not carry canonical physical file paths.
...
| `spec:product.design_records.spec_format.document_shape` | Defines `Index` and `Overview` kind expectations. |
| `spec:product.design_records.spec_format.spec_id_as_ref` | Defines parent reference grammar used by this table. |
