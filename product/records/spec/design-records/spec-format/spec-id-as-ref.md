...
- **id**: `spec:product.design_records.spec_format.spec_id_as_ref`
- **status**: accepted
- **date**: 2026-06-14
...
| Legacy `semantic_refs` / `sections` in front matter | Handled by `spec:product.design_records.spec_format.validation_policy` severity rules: warning for existing specs, error for new or migrated specs. No parsing shim introduced. |
...
| `spec:product.design_records.spec_format.topics_table` | Uses canonical refs in authoritative topic rows. |
| `spec:product.design_records.spec_format.validation_policy` | Severity rules for existing unmigrated specs with hyphen-form IDs or legacy front matter. |
