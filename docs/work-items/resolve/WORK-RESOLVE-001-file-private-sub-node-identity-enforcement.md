...
`REQ-RESOLVE-001` が捕捉した resolver / symbol table の乖離を解消し、file-private sub node を project-wide public QualifiedID 一意性制約から分離する。

本 work item は、ADR-058 の spec 反映、resolver internal index の修正、regression tests、および UC-002 validate / render verification までを一つの解消フローとして所有する。

## Boundary

### Included in this work item

- ADR-058 の accepted 判断に沿った spec alignment
  - `docs/spec/nodes.md`
  - `docs/spec/naming.md`
...
|---|---|---|
| source requirement | `REQ-RESOLVE-001` captured | 本 work item の source requirement として扱う |
| decision | `ADR-058` accepted, `ADR-078` accepted | ADR-058 を primary contract、ADR-078 を MCP synthetic ID 方向性として参照する |
| spec | sub node file-private の記述が ADR-058 の具体化に追いついていない | TASK-RESOLVE-001-01 で alignment する |
| implementation | sub task が project-wide QID uniqueness に混入している | TASK-RESOLVE-001-02 で resolver / index を修正する |
| tests / fixture | UC-002 full validate が duplicate_node / unresolved_flow_task で fail | TASK-RESOLVE-001-03 で regression と verification を行う |
...
```mermaid
flowchart TD
  T1["TASK-RESOLVE-001-01: ADR-058 spec alignment"]
  T2["TASK-RESOLVE-001-02: resolver symbol table / index split"]
  T3["TASK-RESOLVE-001-03: regression tests and UC-002 verification"]
  T1 --> T2 --> T3
```
...
|---|---|---|
| TASK-RESOLVE-001-01 | immediately | 実装前に spec の読解余地を潰す |
| TASK-RESOLVE-001-02 | TASK-RESOLVE-001-01 完了後 | MCP public contract migration を混ぜない |
| TASK-RESOLVE-001-03 | TASK-RESOLVE-001-02 完了後 | UC-002 validate / render verification で close 判定する |

## Completion condition

以下をすべて満たしたとき、本 work item を `done` にできる。

1. ADR-058 の判断が `docs/spec/nodes.md` / `docs/spec/naming.md` / `docs/spec/diagnostics.md` に反映されている。
