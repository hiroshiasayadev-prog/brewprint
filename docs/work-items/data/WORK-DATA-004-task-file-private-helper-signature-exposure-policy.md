# WORK-DATA-004: Implement task-file private helper signature exposure policy
...
Implement the accepted REQ-DATA-003 asymmetric policy for task-file private helper models in task signatures without reopening WORK-DATA-002 or mixing in WORK-DATA-003 model-file render scope.
...
- Preserve the REQ-DATA-003 asymmetric signature exposure policy.
...
|---|---|---|
| source requirement | REQ-DATA-003 accepted | Owns policy execution and close evidence |
| previous work | WORK-DATA-002 done | Do not reopen; treat task-file helper minimum as baseline |
| model-file render | WORK-DATA-003 in progress elsewhere | Explicitly out of scope |
| decision | ADR-070 / ADR-071 accepted | Use task-file helper semantics and render exposure as context only |
...
```mermaid
flowchart TD
  T1["TASK-DATA-004-01: Diagnostic boundary"]
  T2["TASK-DATA-004-02: Spec and diagnostics alignment"]
  T3["TASK-DATA-004-03: Validation implementation"]
  T1 --> T2 --> T3
```

## Tasks

- `TASK-DATA-004-01`: Finalize diagnostic boundary for task-file private helper signature exposure.
- `TASK-DATA-004-02`: Align specs and diagnostics for params rejection / returns allowance.
- `TASK-DATA-004-03`: Implement validation behavior and regression tests.
...
- WORK-DATA-003, ADR-075, ADR-073, ADR-074, MCP identity, UC-002 migration, WORK-DATA-002 reopening, and M15 reopening remain outside scope.

## Close Outcome

WORK-DATA-004 is closed as `done` for the task-file private helper signature exposure policy.

Closed scope:

- TASK-DATA-004-01 finalized the diagnostic boundary:
...
- TASK-DATA-004-02 reflected the policy in:
  - `docs/spec/diagnostics.md`
  - `docs/spec/nodes.md`
  - `docs/spec/type-ref.md`
- TASK-DATA-004-03 implemented and verified validation behavior:
...
- WORK-DATA-003 remains closed separately.
- WORK-DATA-002 remains closed.
- Tagged union / ADR-073 remains out of scope.
