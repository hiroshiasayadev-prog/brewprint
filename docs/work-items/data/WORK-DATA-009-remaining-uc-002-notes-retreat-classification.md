...
| inventory task | TASK-DATA-009-01 done | Reconcile remaining candidates before bucket classification |
| bucket classification task | TASK-DATA-009-02 done | Classify remaining still-unowned candidates into successor buckets before action decisions |
| successor outcome decision task | TASK-DATA-009-03 done | Decide covered / obsolete / new-artifact outcomes for each bucket before follow-up split or close |
| follow-up split / close task | TASK-DATA-009-04 done | Create selected follow-up planning artifacts and record explicit no-action close outcomes |
...
```mermaid
flowchart TD
  T1["TASK-DATA-009-01 Reconcile remaining notes retreat candidates"]
  T2["TASK-DATA-009-02 Classify successor buckets"]
  T3["TASK-DATA-009-03 Decide obsolete / covered / new-artifact outcomes"]
  T4["TASK-DATA-009-04 Create follow-up split or close"]
  T1 --> T2 --> T3 --> T4
```

`TASK-DATA-009-01` owns only `T1`. `TASK-DATA-009-02` owns only `T2`. `TASK-DATA-009-03` owns only `T3`. `TASK-DATA-009-04` owns only `T4`.
...
WORK-DATA-009 is closed as planning and classification work only:

- `TASK-DATA-009-01` reconciled the remaining UC-002 notes-retreat candidates.
- `TASK-DATA-009-02` classified the remaining candidates into successor buckets.
- `TASK-DATA-009-03` selected successor outcomes for each bucket.
- `TASK-DATA-009-04` created the selected follow-up planning artifacts and recorded explicit no-action outcomes.

Created follow-up artifacts:

- `WORK-DATA-011`: request-side / generic container cleanup planning under `REQ-DATA-002`.
- `WORK-DATA-012`: enum / literal / usage-site vocabulary cleanup planning under `REQ-DATA-002`.
- `REQ-DATA-006` / `WORK-DATA-013`: request option and response behavior constraints.
- `REQ-DATA-007` / `WORK-DATA-014`: selector support matrix and object-dependent vocabulary.
- `REQ-DATA-008` / `WORK-DATA-015`: recursive and untagged-union representation.

Explicit no-action / obsolete outcomes:

- Primary MCP identity / semantic-reference candidates are covered by `REQ-MCP-004` / `WORK-MCP-004`; no duplicate DATA artifact was created.
...
- No bucket was kept deferred by `TASK-DATA-009-04`.
- Residual N-048 has only conditional reopen evidence, preserved in `TASK-DATA-009-03` and `TASK-DATA-009-04`, if `WORK-MCP-004` later needs a public map-shape contract.

No direct cleanup implementation was performed in `WORK-DATA-009`. The close did not change UC-002 YAML, fixtures, golden outputs, parser, renderer, validator, MCP implementation, ADR-073 tagged union implementation, ADR-074 DAG TypeRef hint implementation, ADR-078 / ADR-079 / ADR-080 MCP identity implementation, or the UC-002 duplicate task QID / unresolved flow task fix.
