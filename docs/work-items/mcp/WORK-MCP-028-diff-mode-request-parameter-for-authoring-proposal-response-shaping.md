# WORK-MCP-028: diff_mode request parameter for authoring proposal response shaping
...
```
TASK-MCP-028-01 → [spec review gate] → TASK-MCP-028-02 → TASK-MCP-028-03
```
...
- TASK-MCP-028-01: spec update — tools.md に `diff_mode` パラメータと各モード応答形状を追記
- TASK-MCP-028-02: types + implementation — types.go / authoring.go / designrecordsmcp/tools.go
- TASK-MCP-028-03: tests + verification — authoring_test.go + go test
...
- spec / tests が合格し、REQ-MCP-031 の Required Outcome を全項目満たしている。

## Evidence

2026-06-07: 全タスク完了。

- TASK-MCP-028-01: `docs/spec/design-records-mcp/tools.md` 更新。外部 LLM レビュー 3 回・承認済み。
- TASK-MCP-028-02: `authoring.go` / `tools.go` 実装完了。`go test` 全件 PASS。
- TASK-MCP-028-03: `authoring_test.go` に `TestDiffModeRequestParameter`（8 サブテスト）追加。`go test ./internal/designrecords/... ./internal/designrecordsmcp/...` 全件 PASS。
