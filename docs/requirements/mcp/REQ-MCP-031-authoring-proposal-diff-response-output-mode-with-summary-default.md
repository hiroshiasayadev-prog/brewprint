# REQ-MCP-031: authoring proposal diff response output mode with summary default
...
Observed `propose_record_create` responses for `TASK-MCP-026-01..05` included full create diffs. The diffs were semantically correct, but the JSON response was large because every new task body appeared inside `diff.text`.
