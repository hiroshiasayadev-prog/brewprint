...
`ADR-090` により、既存の単一取得 contract を壊さない追加 public read-only tool `get_records` を採用する。
...
`WORK-MCP-002` において、`ADR-090` に基づく public contract / spec 更新、implementation、tests、runtime verification を追跡する。
