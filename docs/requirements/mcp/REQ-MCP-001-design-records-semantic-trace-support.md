# REQ-MCP-001: Design Records MCP に semantic trace support を実装する
...
Design Records MCP は、ADR-087 / ADR-088 と traceability spec に従い、canonical reference resolution foundation と investigation record integration を実装として提供する必要がある。
...
- ADR-087 は semantic/artifact ref resolve と investigation integration を Design Records MCP の責務として accepted にした。
- ADR-088 は MVP resolve / validation を `spec:` semantic ref、record ID-as-ref、investigation canonical references に限定した。
