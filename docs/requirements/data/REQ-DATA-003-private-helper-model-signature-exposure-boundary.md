# REQ-DATA-003: Private helper model signature exposure boundary
...
During TASK-DATA-002-03 discussion and the follow-up boundary review, the following direction emerged:
...
This requirement is resolved through `WORK-DATA-004` without reopening the already-completed WORK-DATA-002 Option A task-file helper minimum.
...
- Model-file helper render boundary owned by WORK-DATA-003.
- ADR-073 tagged union model.
- ADR-074 DAG asset TypeRef hint.
- ADR-078 MCP helper exposure / semantic identity.
...
This requirement captures the signature exposure policy for task-file helper models and is resolved by `WORK-DATA-004`. It does not reopen `WORK-DATA-002`, decide model-file helper render, or block the already-completed task-file helper minimum.

The accepted policy is asymmetric. `WORK-DATA-004` finalizes the concrete spec wording, diagnostic behavior, implementation, and tests:
