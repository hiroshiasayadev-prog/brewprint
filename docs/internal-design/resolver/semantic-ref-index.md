...
ADR-088 により、本 document 自体は semantic trace MVP の active endpoint または required acceptance target ではない。
...
| known gap closure | `ADR-086` の `depends_on: INV-DOCS-001` が investigation integration 後に解決する |
...
Investigation source は ADR-086 / schema に従う H1 直下の bullet metadata である。`parseInvestigationRecord` は ADR parser と同じ metadata block boundary（H1 後から最初の H2 または blockquote 直前まで）を使い、以下を読む。
...
investigation record が index に入ると、既存 `recordsByID` に `INV-*` が現れる。これにより、既存 dependency validation route は `ADR-086 -> INV-DOCS-001` を record ID-as-ref として解決でき、現行 `missing_depends_on_target` baseline error を自然に解消する。
...
- `INV-DOCS-001` を investigation record として取得できる。
- `ADR-086 -> INV-DOCS-001` の既知 `missing_depends_on_target` error が消える。
...
> 由来: ADR-083, ADR-087, ADR-088; INV-DOCS-002; INV-DOCS-003; M19 Phase A contract result
