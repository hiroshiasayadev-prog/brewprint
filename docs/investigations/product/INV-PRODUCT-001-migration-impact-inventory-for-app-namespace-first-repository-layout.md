# INV-PRODUCT-001: app namespace-first repository layout への migration impact inventory

- **status**: concluded
- **date**: 2026-06-08
- **trigger**: WORK-PRODUCT-003 TASK-02。ADR-097 で確定した app namespace-first layout model への migration を計画するにあたり、`docs/` と `internal/` の移動対象・規模・エッジケースを事前に把握する必要があった
...
- **follow_up_candidates**:
  - WORK-PRODUCT-003 TASK-03: compatibility / legacy path 方針決定
  - WORK-PRODUCT-003 TASK-04: migration plan or explicit defer
...
**ADR**:
- ADR-076〜090（Design Records MCP MVP から semantic trace, workflow artifacts, authoring transaction まで）
- ADR-093（authoring transaction model）
...
**ADR**:
- ADR-001〜075（node type 定義・name resolution・DAG/ER/sequence/state/wireframe・task IO・type system 等、BPDSL コア設計判断群）
- ADR-083（project artifact boundary — YAML as implementation source）
...
- ADR-081, 082, 084, 085, 086, 087, 088, 089（workflow artifact / investigation / semantic trace 設計判断）
- ADR-091, 092, 094（workflow artifact status vocabulary）
- ADR-095（YAML DSL と DRMCP の結合境界）← エッジケース判断結果参照
- ADR-097（app namespace-first repository layout）
...
### 3. ADR-095（YAML DSL と Design Records MCP の結合境界）
