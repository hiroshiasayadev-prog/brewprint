...
- **status**: accepted
- **date**: 2026-04-24
- **depends on**: ADR-017, ADR-026, ADR-028
...
また ADR-028 で API Table は明示的な view YAML を持つと定めており、
...
- `modules[].module`: モジュールパス（ADR-027 の名前解決ルールに従う）
...
- API Table（ADR-028）と同じ「明示的 view YAML」パターンを採用することで、view 定義の設計を一貫させる
