# 089: LLM-first 設計原則と intent transfer 境界

- **status**: accepted
- **date**: 2026-05-24
- **depends_on**: ADR-081, ADR-083, ADR-087, ADR-088
- **supersedes**:
- **migrated_to_spec**:
...
ADR-087 / ADR-088 および M19 が扱う canonical read / resolve / validation foundation は、以下を確認するための意図的な first step と位置付ける。
...
未確認の不足を前提に MVP scope を広げると、ADR-088 の scope reduction と矛盾し、過剰設計になる。まず dogfooding で concrete requirement を確認するため採用しない。
...
`spec:trace.artifact-refs` と `spec:trace.coverage-mapping` が所有する canonical identity および deferred realization coverage の境界は維持する。External tool interop や LLM-mediated evaluation のために spec への追記が必要かは、INV-DOCS-004 / INV-DOCS-005 の後続評価を含めて判断する。
...
INV-DOCS-004 が扱う OFT 評価、および INV-DOCS-005 が扱う Doorstop 評価では、機能適合性・ownership 適合性に加えて、agent read I/O、diagnostic usability、identity mapping、testability、connector maintenance cost を判断材料に含める候補が生じる。
...
- commit: tbd
- impl commit: 該当なし
- investigation: INV-DOCS-006
- decision context: ADR-081, ADR-083, ADR-087, ADR-088
