...
ADR-017にてState DiagramはApplicationレイヤーの図として位置づけられた。
`store`（データ保持、ADR-007）とは別概念として、FSMの状態を表現する`state`ノード種別を設計する必要があった。

合わせて以下を設計する：
- transition記法（ADR-015の`flow:`セクションとの関係）
- eventノード（ADR-018）との接合
...
eventノードはstate diagram YAMLの`nodes:`に`state`と同列で定義する（ADR-018のnode typeとしての扱いは変わらない）。
...
Sequence Diagramから同じeventを参照する場合はcross-file参照になる。cross-file参照の解決ルールはADR-003に従う。

### 5. `store`との区別

| ノード | 種別 | 概念 |
|--------|------|------|
| `store` | データ保持（ADR-007） | 実行時に状態を保持するインスタンス。DAGのflow内でreads/writesされる |
...
State Diagramが扱うeventはそのFSM専用のものが多く、別ファイルに分離するとcross-file参照が増えて見通しが悪くなる。Sequence DiagramなどからもアクセスされるeventはADR-003のcross-file参照で解決できるため、定義場所の制約は不要。
