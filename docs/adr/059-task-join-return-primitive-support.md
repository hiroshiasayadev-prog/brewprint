# 059: task / join return が primitive を許容するよう validation を整える
...
これは ADR-021 §3 / spec/nodes.md「primitive予約語」が `any` を含む7語の primitive 予約語を定義していることと矛盾する。

### 仕様側の規定（再掲）

ADR-021 §3 / spec/nodes.md primitive予約語節:
...
- `str` / `int` / `float` / `bool` / `bytes` / `datetime` / `any` (ADR-021 §3 のprimitive予約語7語)
...
`any` の使用は最小限であるべき（ADR-021 §3 の but）だが、現状 v1 model の表現力には限界があり、
...
ADR-021 §3 は primitive の存在を定義しているが、
「primitive を return.model に書けるか」までは明示していない。
spec-first運用（ADR-050）に従い、specで明示する。
...
- ADR-021が定めたprimitive予約語の中で一部だけ扱いを変える根拠がない
...
**型互換性ルールは ADR-060 で別途規定する。**

### v1.0.0-spec タグへの影響

ADR-058 と同様、`v1.0.0-spec` 凍結条件のうち「`go test ./...` パス」は
...
`v1.0.1-spec` タグは ADR-058 / 本ADR の修正を含むpatch releaseとして発行する。

### M14への影響

- M14a で ADR-058 (B1: subnode scope) と本ADR (B2: return primitive) を一括対応する
- M14（self-hosting）Phase A は M14a + M15 完了後に再構築する

## Evidence
- commit: tbd
- impl commit: tbd
- 参考: ADR-021 §3 primitive予約語、`internal/resolve/validation.go` `validateParams` の実装
