...
ADR-025では `actor` ノードは独立ファイルを持たず、参照元ファイル（`state.yaml` 等）のサブノードとして同居すると定めた。

しかし `stripe` や `scheduler` のような外部システム・外部actorは、複数のstate diagram / sequence diagramから参照されうる。ADR-025の設計では同一actorが複数ファイルに重複定義される問題が生じる。

また、`source=external` のeventには発火元actorを明示する必要があり（ADR-018改訂）、actorがモジュールスコープに縛られていると参照解決が複雑になる。
...
ADR-027のsentinel方式による名前解決は適用しない。
...
却下した代替案：
- actorを参照元ファイルに同居（ADR-025）→ 複数ファイルへの重複定義問題が生じる
- actorをモジュールスコープで管理 → 外部システムの性質と合わない

## 影響

- ADR-025は本ADRにsupersededされる
- ADR-027のsentinel予約語から `actor` を削除する（適用外のため）
