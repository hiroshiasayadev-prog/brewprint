...
たとえば ADR-061 で `foreach.returns` が collected asset source として定義された結果、以下のような flow source を作れるようになった。
...
しかし、main task `validate_cart` がこの `validated_items` を返すことをどう明示するかは ADR-061 の責務ではない。
...
`returns.source` の source 解決規則は、ADR-060 §5（wiring source 解決）および ADR-061 §3（collected asset source 参照）を継承する。
...
`returns.source` を指定した場合、source TypeRef と `returns.model` の TypeRef は ADR-060 の TypeRef compatibility ルールで検証する。
...
この方針は ADR-060 の flow wiring type compatibility における二重診断抑制と同じである。
...
なお、ADR-060 §4 で定義された `join.params` の `returns.name` 一致による暗黙接続は v1.0 由来の既存仕様として維持する。
...
foreach 全体として返す場合は、ADR-061 の `foreach.returns` で collect した source を `returns.source` に指定する。
...
- `docs/spec/edges.md` に「task return wiring」節を新設し、`returns.source` の source 解決ルールおよび ADR-060 §1-7 と同じ TypeRef compatibility ルールを記述する
  - source 解決規則は ADR-060 §5 / ADR-061 §3 を継承する
...
- ADR-009 の task I/O contract に、`returns.source` による return wiring を追補する
- ADR-015 の flow wiring とは別に、task return wiring を定義する。ただし source 解決体系は flow wiring source と揃える
- ADR-060 の TypeRef compatibility を task return source 検証にも適用する
- ADR-061 の `foreach.returns` は、`returns.source` から参照可能な collected asset source になる
...
ADR-060 / ADR-061 を前提にした forward 拡張であり、v1.0.0-spec の遡及修正ではない。
