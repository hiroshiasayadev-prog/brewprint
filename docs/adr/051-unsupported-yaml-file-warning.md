# 051: Unsupported YAMLファイルへのwarning出力

- **status**: accepted
- **date**: 2026-04-29
- **depends on**: ADR-030, ADR-050
...
`as:` を必須としたADR-030の方針（view定義は明示的にファイル種別を宣言する）を実効性あるものにするには、宣言が欠落している事象を検出して通知する仕組みが必要。
...
- spec-first 方針（ADR-050）として、loader の振る舞いは外部仕様に明示されるべき。silent skip は仕様化しても利用者にとって価値がない
