...
ADR-003では名前解決ルールを `モジュール名.ノード種別.ID`（3階層固定）と定めた。
...
クロスモジュール参照はフルパス必須。ADR-003の同モジュール内ID直書きルールは継続有効。
...
- ADR-003の `モジュール名.ノード種別.ID`（3階層固定）は本ADRに supersede される
