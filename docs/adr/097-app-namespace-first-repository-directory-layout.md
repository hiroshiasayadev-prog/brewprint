# 097: app namespace-first repository directory layout の採用
...
REQ-PRODUCT-001 / REQ-PRODUCT-002 により app namespace と domain namespace のモデルが定義された。しかし物理ディレクトリ構造はこれに追随しておらず、現状以下の問題がある。
...
REQ-PRODUCT-001 は「namespace-first physical layout migration work」を将来の prerequisite 対象として予告していた。REQ-PRODUCT-003 はこの物理レイアウトモデルの仕様化を要求する。
...
実際のファイル・ディレクトリ移動は本 ADR のスコープ外とし、WORK-PRODUCT-003 に委ねる。

## 理由

**namespace model との一致**: ADR-095 / REQ-PRODUCT-001 が定義した app namespace モデルを物理ディレクトリに反映することで、ディレクトリ構造を見るだけで「どの app の concern か」が分かるようになる。
...
- 実際の migration タイミング・手順・互換方針は WORK-PRODUCT-003 で決定する
- `records/` 内部のサブディレクトリ構造（`adr/`、`spec/`、`requirements/`、`work-items/`、`tasks/`、`investigations/`）は変更しない

## Evidence

- REQ-PRODUCT-001: App and domain namespace model
