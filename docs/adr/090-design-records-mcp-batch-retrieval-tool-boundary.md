# 090: Design Records MCP batch retrieval tool boundary

- **status**: accepted
- **date**: 2026-05-26
- **depends_on**: ADR-077, ADR-087
- **supersedes**:
- **migrated_to_spec**:
...
ADR-077 は Design Records MCP の read-only query 境界として `list_records` / `get_record` / `validate_records` を定め、`get_record` を record ID から metadata、headings、必要に応じて raw body を取得する単一取得 tool として採用した。

ADR-087 は Design Records MCP の record index を `decision` / `spec` / `investigation` の横断取得・検証へ拡張し、設計対話で複数種別の record を参照する導線を確立した。

M19 close 後の dogfooding では、関連する ADR、spec、investigation record の判断根拠を確認する際、候補を絞った後に複数 record の headings または body を確認するため、単一 `get_record` を繰り返す経路が実際に生じた。この観測は `REQ-MCP-002` および `TASK-MCP-002-01` に記録されている。
...
本ADRは ADR-077 / ADR-087 の既存 tool 境界を破棄せず、選択済み record の batch retrieval 経路を追加する形で refine する。
...
- `ADR-077` のような `decision` record
- `SPEC-design-records-mcp-tools` のような `spec` record
- `INV-DOCS-001` のような `investigation` record
...
具体的な作業項目と完了条件は `WORK-MCP-002` および対応する task file で追跡する。

### Requirement / work item への影響

`REQ-MCP-002` は capability 採用結果を反映し、`WORK-MCP-002` は spec 更新、implementation、tests、runtime verification へ進む。

## Evidence

- commit: tbd
- impl commit: tbd
- 参考: ADR-077, ADR-087, REQ-MCP-002, WORK-MCP-002, TASK-MCP-002-01, TASK-MCP-002-02, M19 close 後の Design Records MCP dogfooding
