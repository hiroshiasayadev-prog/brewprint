...
ADR-050 および `docs/doc-policy.md` により、責務は以下のように整理されている。
...
ADR-067 は、実例由来の設計判断を ADR として整理する過程で、以下の学びを生んだ。

- UC-002 の実例は enum model 導入の重要な evidence だった
- しかし、実例の YAML shape は migration によって変わりうる
- そのため、ADR-067 には「判断根拠としての UC-002 実例の扱い」を明記した
- 後続 task list は ADR から外し、M15 / UC-002 task file で追跡する方針にした

そのため、ADR-067 は authoring guide 初版で参照しうる初期 example の一つとして扱う。
...
ADR-067 の経験を標準化することで、以後の ADR で同じ混乱を繰り返しにくくなる。
...
- 利点: 文書追加が不要
- 欠点: ADR-067 と同じ混乱が今後も起きる。実例を仕様固定のように書く ADR、task list を抱え込む ADR が増える
...
ADR-050 / doc-policy.md の漸進移行方針に従い、触れたタイミングで必要に応じて改善する。
...
M15 Phase C では、ADR-067 以降の設計判断で authoring guide を参照する。
