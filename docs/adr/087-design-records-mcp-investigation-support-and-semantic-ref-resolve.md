# 087: Design Records MCP investigation support and semantic ref resolve

- **status**: accepted
- **date**: 2026-05-23
- **depends_on**: ADR-076, ADR-077, ADR-081, ADR-084, ADR-085, ADR-086
- **supersedes**:
- **migrated_to_spec**:
...
ADR-076 / ADR-077 は、Design Records MCP を ADR / spec の machine-readable metadata に対する read-only index / query / validation layer として導入し、`list_records` / `get_record` / `validate_records` を中心とする tool boundary を決定した。

その後、ADR-085 / ADR-086 により `docs/investigations/` が導入され、investigation artifact の責務、format、lifecycle が確定した。investigation は決定や現行仕様を所有しない一方、調査根拠、未確定点、選択肢、推奨案、後続 artifact 候補を保存する。ADR-086 は、investigation を MCP index / query / validate 対象にする可能性を認めたが、Design Records MCP に統合するか、別 MCP interface とするかは後続判断とした。
...
さらに、ADR-086 は初期運用として investigation の `source_refs` / `follow_up_candidates` / `follow_up_results` に human-readable な artifact ID または path を許容した。しかし semantic traceability は physical layout 変更と trace の安定性を分離する方針であり、調査根拠となる `source_refs` を physical path に依存させ続ける根拠はない。
...
本ADRは Design Records MCP の record kind 全体を閉じた列挙として確定するものではない。ADR-081 で将来拡張として予約された `kind: requirement` を含め、後続判断により他の artifact kind を追加する可能性を制限しない。

この決定は investigation を decision と同一視するものではない。ADR-085 / ADR-086 の責務境界は維持する。
...
```json
{
  ...
  "investigation": {
    ...
    "source_refs": ["ADR-086"],
    "follow_up_candidates": ["ADR-087"],
    "follow_up_results": ["ADR-087"]
  }
}
```
...
ADR-086 が初期運用の便宜として許容した path-based reference は、既存documentのcompatibility inputとして読み取る必要がある場合に限り許容してよい。ただし、canonical form ではなく、validator は noncanonical reference として診断できるものとする。
...
存在検査を必須にすると、候補と結果の区別が崩れ、ADR-086が分離した `follow_up_candidates` / `follow_up_results` の意味を壊す。
...
READMEは、ADR-086の初期運用として記載した path許容を本ADRに合わせてrefineする必要がある。
...
ADR-086 の investigation format / lifecycle と、investigation が決定を所有しないという境界は維持する。

本ADRは、ADR-086が後続判断に送ったMCP interface方針と、初期運用で暫定許容したreference記法のcanonicalityをrefineする。
ADR-086本文は起票時点のスナップショットとして遡及修正しない。
