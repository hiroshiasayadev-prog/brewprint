# 080: state machine semantic object と file-path-free scenario reference

- **status**: proposed
- **date**: 2026-05-13
- **depends_on**: ADR-030, ADR-032, ADR-035, ADR-078, ADR-079
...
ADR-079 では、transition ID policy のうち ADR-080 に依存しない制約を proposed として整理した。
...
ADR-079 では transition final anchor を未決として残した。
...
transition ID の最終形式、escaping、`local_id`、ObjectRef shape は ADR-079 の後続修正または transition ID spec 反映で確定する。
...
これは ADR-078 の MCP identity policy とは別レイヤーの問題だが、UI / MCP parity を考えると同じ方向で解決すべきである。
...
ADR-079 は、本ADRの結論を受けて transition ID final policy を更新する。
...
ADR-079 側では、guard inclusion、old file path based ID unsupported、visibility generated などの非 file-path 制約を維持しつつ、anchor / local_id を本ADRに合わせて確定する。
