...
ADR-069 では、TypeRef に anonymous inline struct を導入しないことを決定した。
...
ADR-011 では task file について「1ファイル = 1メインノード + 複数サブノード」を認め、サブノードを file-private として扱う方針を導入した。
ADR-058 では、この file-private 性を実装上も保証するため、private sub node は外部 QualifiedID 参照対象ではなく、file-local object として扱うことを補強した。
...
ADR-011 では `main: true` を task にのみ適用するとしていたが、本ADRでは model にも `main: true` を拡張する。
...
そのため、ADR-058 / MCP schema の private sub node と同じく、file-local synthetic ID を使う。
...
ADR-069 で決定した通り、brewprint は anonymous inline struct TypeRef を導入しない。
...
ADR-011 以降、brewprint は file の代表 node を `main: true` で明示し、それ以外を file-private helper として扱う設計を採っている。
...
しかし ADR-011 では、ファイル名推論ではなく `main: true` による明示を採用した。
...
- 欠点: ADR-069 の判断に反する。匿名 shape は設計意図が後から追いにくく、TypeRef variant / compatibility / lint の論点も増える
...
- 欠点: ADR-011 で避けたファイル名推論に戻る。representative model が YAML 内で明示されず、複数候補の検出も分かりづらくなる
...
ADR-069 で anonymous inline struct を導入しないことを決めたため、本ADRはその補完策となる。
