...
ADR-057でbrewprint v1.0.0-spec が凍結された（gitタグ `v1.0.0-spec` 発行済み）。
...
v1範囲のspec仕様で表現できない箇所はspec gapとして `TASKS-UC-002.md` に記録し、ADR-057で確定したRelease snapshots運用に従って次の `v{N}.0.0-spec` に向けた改訂候補として扱う。
...
- v1.0.0-spec のspec / ADR / Go実装ツリーの遡及修正（ADR-050 / ADR-057に準ずる）
- ADR-010の複数論点混在分割（v1後検討事項として継続。ADR-057 §6）
- DISCLAIMER.mdの起草（M14と独立。ユーザー起草）

self-hosting途中でv1範囲内のspec誤記レベルの修正が必要になった場合は、ADR-050 §7の漸進移行ルールに従って個別判断する。
...
ADR-054 / ADR-055 / ADR-056で定義され、`docs/spec/mcp/**` で仕様化されているtool群を、brewprint YAMLで表現する。
...
レイヤー境界はADR-047で確定しており、現行Go実装と対応する。
...
- **ADR起票トリガー**:
  - v1範囲内で即修正可能なspec誤記レベル: ADR-050 §7の漸進移行ルールに従い個別ADR起票またはspec修正
