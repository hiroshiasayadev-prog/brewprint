# TASK-MCP-016-01: Define required narrative section close policy
...
|---|---|---|---|---|
| WORK | `done` | `Goal`, `Boundary`, `Evidence` | `Impact Scope`, `Task flow`, `Task Candidates`, `Completion Condition` | Newly enforced by `REQ-MCP-017`; the accepted requirement explicitly requires at least this guard for `WORK status: done`. |
...
- Both categories should be `error` severity because warnings would not close the validation gap identified by `REQ-MCP-017`.
...
- Literal placeholder text such as `Pending` passes this non-empty guard. This is an accepted limitation because `REQ-MCP-017` explicitly excludes narrative quality or sufficiency judgment.

Catalog target correction for `TASK-MCP-016-02`:
...
- Confirmed that WORK/REQ narrative non-empty policy is newly enforced by `REQ-MCP-017` and should be cited as such, not misrepresented as pre-existing guide policy.
