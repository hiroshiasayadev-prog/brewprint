# TASK-MCP-009-02: Cleanup policy and canonical boundary decision
...
TASK-MCP-009-01 の inventory をもとに、authoring guidance canonical source と legacy docs の扱いを決める。

曖昧さが残る場合は、Opus / reviewer 向けの handoff または review result を evidence として残し、判断根拠を明確にする。

## Work

- TASK-MCP-009-01 の inventory を確認し、各 legacy file の残存価値と stale risk を評価する。
...
- TASK-MCP-009-03 が具体的に更新対象を選べる状態になっている。
- REQ-MCP-008 authoring transaction implementation、WORK-MCP-006 metadata validation strictness、accepted ADR supersession を不要に再オープンしていない。
- 判断に ambiguity が残る場合は、reviewer handoff または review result が evidence として残っている。

## Verification

- TASK-MCP-009-01 の inventory と cleanup policy decision の対応を Evidence に記録する。
...
- Accepted ADR supersession is not required for this cleanup decision. No WORK-MCP-006 metadata strictness decision and no REQ-MCP-008 authoring transaction implementation scope is reopened.

TASK-MCP-009-01 inventory input used:
...
| file | classification | decision / reason | TASK-MCP-009-03 handling |
|---|---|---|---|
...
| `docs/spec/design-records-mcp/tools.md` | keep non-canonical | This is MCP public tool contract. It defines `list_authoring_guides` / `get_authoring_guidance`, read-only behavior, guide ID lookup, and response shape. It is not an authoring guide cleanup target. | Keep. Update only if stale wording is found during TASK-MCP-009-03, especially any wording that implies guide source path is public contract. Do not thin or delete. |
| `docs/spec/design-records-mcp/schema.md` | keep non-canonical | This is MCP schema/source model contract. It defines authoring guidance as a read-only guidance source separate from Design Records record kinds and says guide source path is not public response contract. It is not an authoring guide cleanup target. | Keep. Update only if stale wording is found during TASK-MCP-009-03. Do not thin or delete. |

Handling of legacy-only material:

- Legacy-only material must not be silently lost. TASK-MCP-009-03 should review each unique section and either move it into the appropriate canonical guide or record that it was intentionally dropped because it is obsolete, too example-specific, or outside the canonical guide scope.
...
What TASK-MCP-009-03 should edit:
...
- `validate_records` for `kind=task`, `id_range=TASK-MCP-009-01..TASK-MCP-009-02` returned `ok: true` and `diagnostics: null`.
