# TASK-MCP-007-03: MCP tools spec update
...
`TASK-MCP-007-02` で決めた workflow artifact range filter contract を Design Records MCP tools spec に反映する。
...
- `REQ-MCP-007` の acceptance expectation と矛盾しない。
...
- The spec now reflects the selected contract from `TASK-MCP-007-02`.
- Implementation can proceed in `TASK-MCP-007-04` against this public contract.
