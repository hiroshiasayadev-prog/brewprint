# TASK-MCP-006-03: Workflow metadata strictness contract を spec / guidance に反映する
...
TASK-MCP-006-02 で確定した workflow artifact metadata validation strictness contract を、Design Records MCP の public spec と authoring guidance に反映する。
...
- README 系 wording cleanup は REQ-MCP-009 scope とし、本 task では扱わない。
- ADR-092 は起票時点の decision snapshot として扱い、現在仕様は spec 側に反映する。
- REQ-MCP-003 / WORK-MCP-003 の完了済み scope は再オープンしない。

## Work

1. TASK-MCP-006-02 の decision output を読む。
...
TASK-MCP-006-04 が実装可能な形で、public spec と authoring guidance が同期している。
...
- README 系は REQ-MCP-009 の cleanup scope として分離し、本 task では更新しない。
...
Implementation input for TASK-MCP-006-04:
...
- Replaced the README synchronization scope wording with explicit REQ-MCP-009 separation.
