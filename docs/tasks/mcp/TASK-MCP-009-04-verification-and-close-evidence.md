# TASK-MCP-009-04: Verification and close evidence
...
- **outputs**:
  ...
  - WORK-MCP-009 / REQ-MCP-009 close evidence if complete

## Goal

Metadata relation、stale reference、MCP authoring guidance retrieval behavior を確認し、close 可能なら WORK-MCP-009 / REQ-MCP-009 の status 同期まで行う。

## Work

- `validate_records` で `REQ-MCP-009`, `WORK-MCP-009`, and `TASK-MCP-009-01` through `TASK-MCP-009-04` を確認する。
...
- TASK-MCP-009-03 で implementation / tests を変更した場合は、対象テスト結果を確認する。
- Completion Condition を満たす場合は、WORK-MCP-009 と REQ-MCP-009 に close evidence を残し、status を同期する。
- Completion Condition を満たさない場合は、未解決事項と blocker を evidence として残す。

## Done condition

- `REQ-MCP-009`, `WORK-MCP-009`, and `TASK-MCP-009-01` through `TASK-MCP-009-04` の metadata relation validation result が記録されている。
- Stale reference check result が記録されている。
- MCP authoring guidance retrieval verification result が記録されている。
- WORK-MCP-009 / REQ-MCP-009 を close できるかが evidence に基づいて判断されている。
...
- `validate_records(kind=task, id_range=TASK-MCP-009-01..TASK-MCP-009-04)` returned `ok: true`, `diagnostics: null`.
...
- The closeout records are untracked in the current worktree, so they were checked separately with `git diff --no-index --check -- NUL <file>` for TASK-MCP-009-04, WORK-MCP-009, and REQ-MCP-009. Those commands returned exit code 1 because `--no-index` compares `NUL` to a real file, but output contained only line-ending normalization warnings and no whitespace errors.
...
- Completion conditions for TASK-MCP-009-04 are satisfied.
- WORK-MCP-009 completion conditions are satisfied by TASK-MCP-009-01 inventory, TASK-MCP-009-02 cleanup policy, TASK-MCP-009-03 cleanup/reference updates, and this verification.
- REQ-MCP-009 required outcomes are satisfied. Nearby completed MCP requirements use status `accepted`, so REQ-MCP-009 was closed as `accepted` rather than inventing a new requirement status.
- No unresolved blocker remains for REQ-MCP-009 / WORK-MCP-009.
