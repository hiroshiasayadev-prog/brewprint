# TASK-MCP-011-01: Reproduce authoring proposal dogfooding failures
...
- **outputs**:
  ...
  - Cause hypothesis and recommended split for TASK-MCP-011-02
...
- A recommended scope for `TASK-MCP-011-02` is provided.
...
- Finding 3: `domain: "mcp"` with an uppercase ID domain such as `REQ-MCP-992` is reproduced as a case-sensitive mismatch. Implementation entrypoint: `authoring.go` lines 653 and 664. Classification: spec gap.
- Finding 4: propose-time validation scope pollution is reproduced. `propose_record_create(REQ-MCP-990)` emitted unresolved `spec:*` diagnostics for `INV-DOCS-002` through `INV-DOCS-006`, while direct `validate_records(kind=investigation)` and direct full `validate_records` did not reproduce the same diagnostics. Implementation entrypoint: `authoring.go` lines 447 and 1023. Classification: implementation bug plus missing spec.
...
Recommended next step: proceed to `TASK-MCP-011-02` and specify the public MCP contract for `body` / `fields`, top-level `id` / `fields.id`, domain case normalization, and proposal affected-record validation scope.

Additional dogfooding evidence: attempting to create `TASK-MCP-011-02` through Design Records MCP produced proposal `pw_000008`, but `accept_proposed_write` returned `written:false` with `invalid_request: proposal validation has error diagnostics`. The proposal diagnostics included unrelated `INV-DOCS-002` through `INV-DOCS-006` unresolved `spec:*` references and unrelated `TASK-MCP-005-*` metadata errors. This confirms that proposal accept is also blocked by repository-wide or otherwise unrelated diagnostics, matching Finding 4 and strengthening `REQ-MCP-012`.
