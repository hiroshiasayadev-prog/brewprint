# TASK-MCP-022-04: Go implementation: update types.go and validation.go for new status vocabulary
...
`internal/designrecords/types.go` と `internal/designrecords/validation.go` を ADR-094 に従って更新し、新 status vocabulary (`not_started` / `in_progress` / `blocked` / `done`) を Go 実装に反映する。
