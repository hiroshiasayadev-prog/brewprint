# TASK-MCP-014-04: Runtime smoke close evidence and status synchronization
...
| server-side `REQ-MCP-new` plus fields plus body | pass; resolved to `REQ-MCP-016`; generated H1 and metadata used the resolved ID; caller body did not need the resolved ID |
...
| fields plus `body_cache_id` | pass; rejected with `invalid_request` diagnostic in REQ-MCP-014 scope |
...
- `go test ./...` passed.
- Design Records MCP `validate_records(kind=requirement, id_range=REQ-MCP-014..REQ-MCP-014)` passed with no diagnostics.
- Design Records MCP `validate_records(kind=work_item, id_range=WORK-MCP-014..WORK-MCP-014)` passed with no diagnostics.
- Design Records MCP `validate_records(kind=task, id_range=TASK-MCP-014-01..TASK-MCP-014-04)` passed with no diagnostics.

REQ-MCP-015 retry/cache expansion remains out of scope for this task. In particular, `fields + body_cache_id` remains invalid here.
