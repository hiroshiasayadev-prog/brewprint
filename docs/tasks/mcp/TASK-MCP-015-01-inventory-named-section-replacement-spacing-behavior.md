# TASK-MCP-015-01: Inventory named section replacement spacing behavior
...
Identify the current `propose_record_update` `named_section_replace` formatting behavior and reproduce the spacing loss described by `REQ-MCP-016`.
...
- **変更ファイル**: `internal/designrecords/authoring_test.go` の failing regression test 追加のみ。実装ファイルへの変更は `TASK-MCP-015-03` に委ねる。
