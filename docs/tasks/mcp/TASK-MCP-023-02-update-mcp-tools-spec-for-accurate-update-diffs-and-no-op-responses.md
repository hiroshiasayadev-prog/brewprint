# TASK-MCP-023-02: Update MCP tools spec for accurate update diffs and no-op responses
...
- Confirm whether ADR-093 needs a short compatibility note; leave ADR unchanged if the behavior is a contract clarification under the existing authoring transaction model.
...
- Review the updated spec against REQ-MCP-027 acceptance criteria.
...
Spec contract updated in `SPEC-design-records-mcp-tools` for REQ-MCP-027.
...
- ADR-093 required no change. This is a contract clarification and correction under the existing proposal-first authoring transaction model, not a new transaction model decision.

Implementation handoff:

- Proceed to `TASK-MCP-023-03`.
