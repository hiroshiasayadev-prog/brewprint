# TASK-MCP-002-03: Batch retrieval の public contract / spec を更新する
...
- TASK-MCP-002-04 が曖昧な contract 判断なしに実装へ進める。
...
- 2026-05-26: ADR-090 のレビュー指摘を受け、実装前に concrete contract を spec へ明文化する方針を採用した。
...
- Spec の更新内容は ADR-090 の責務境界、partial result、duplicate handling、raw body 非 truncate、public numeric size limit 非定義と整合する。
- Independent review で挙がった response shape、all-missing / empty `ids`、duplicate diagnostic payload、exact record ID と reference resolve の境界の四点は、実装前の public contract として明文化済みである。TASK-MCP-002-04 は追加の contract 判断なしに実装へ進める。
