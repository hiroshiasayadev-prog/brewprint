# TASK-MCP-008-01: Current write gap and authoring transaction scope review
...
- **outputs**:
  ...
  - ADR drafting inputs for TASK-MCP-008-02

## Goal

現行 Design Records MCP の read / navigation / validation / guidance capabilities と、REQ-MCP-008 が求める authoring transaction capability の差分を確認し、ADR 起草前に MVP scope と設計判断点を明確にする。

## Work

- `REQ-MCP-008` と `WORK-MCP-008` を確認し、要求された authoring transaction scope を整理する。
...
- Existing related requirements / work items such as `REQ-MCP-005`, `REQ-MCP-007`, `WORK-MCP-005`, and `WORK-MCP-007` を必要範囲で確認し、artifact-oriented access の既存方針と矛盾しないか整理する。
- ADR 化すべき判断点を分類する。
...
- REQ-MCP-008 の MVP scope が、ADR / spec / implementation に渡せる粒度で確認されている。
- ADR draft に含めるべき decision points と、ADR ではなく spec に委ねるべき schema details が分離されている。
- 後続 `TASK-MCP-008-02` が ADR draft に進める状態になっている。

## Verification

- Relevant requirement / work item / spec / current MCP tool contract の確認結果を Evidence に記録する。
- 可能であれば `validate_records` で `REQ-MCP-008`, `WORK-MCP-008`, and this task の metadata relation を確認する。
...
- Accepted ADR title list under `docs/adr/`, especially the current Design Records MCP lineage referenced by the specs: ADR-076, ADR-077, ADR-087, ADR-088, ADR-090, ADR-092
...
No current public tool supports any of the following REQ-MCP-008 authoring-write capabilities:
...
- REQ-MCP-005 / WORK-MCP-005 established artifact-independent authoring guidance retrieval by guide ID and explicitly kept guide source paths out of the public contract.
- REQ-MCP-007 / WORK-MCP-007 extended artifact-oriented range navigation for workflow artifacts.
- Those completed scopes support read/navigation/guidance ergonomics, but they did not introduce record authoring writes.

### REQ-MCP-008 MVP scope confirmation

The REQ-MCP-008 MVP scope is still valid and not already implemented.
...
### ADR decision points for `TASK-MCP-008-02`
...
- Design Records MCP validation:
  - `validate_records(kind: requirement, id_range: REQ-MCP-008..REQ-MCP-008)` returned `ok: true` with no diagnostics.
  - `validate_records(kind: work_item, id_range: WORK-MCP-008..WORK-MCP-008)` returned `ok: true` with no diagnostics.
  - `validate_records(kind: task, id_range: TASK-MCP-008-01..TASK-MCP-008-01)` returned `ok: true` with no diagnostics.
...
`TASK-MCP-008-01` satisfies its done condition.

The current Design Records MCP is internally consistent with its accepted/spec-backed read-only MVP contract. The gap confirmed here is not an implementation bug; it is the absence of a public artifact-oriented authoring transaction contract. `WORK-MCP-008` should proceed to `TASK-MCP-008-02` to draft an ADR for the transaction model before changing the tools spec or implementation.
