# TASK-MCP-006-02: Workflow metadata strictness contract を判断する
...
- **outputs**:
  ...
  - Diagnostic category decision for TASK-MCP-006-03 and TASK-MCP-006-04

## Goal

TASK-MCP-006-01 の evidence に基づき、Design Records MCP の workflow artifact metadata validation strictness を public contract として判断する。
...
- REQ-MCP-003 / WORK-MCP-003 の declared relation validation 完了 scope は再オープンしない。
...
1. TASK-MCP-006-01 の gap table を確認する。
...
- TASK-MCP-006-03 / TASK-MCP-006-04 に渡す implementation input。
...
## Implementation input for TASK-MCP-006-03 / TASK-MCP-006-04
...
- Leave `diagnostics:null` versus `[]` as follow-up unless TASK-MCP-006-03 finds the spec wording forces a local clarification.

## Evidence

- TASK-MCP-006-01 evidence review found that required workflow fields are documented but currently not generally validated.
- TASK-MCP-006-01 evidence review found that workflow `date` is currently required by docs but ignored by parser.
- TASK-MCP-006-01 evidence review found that missing list fields and empty list fields currently collapse to empty slices, so parser presence tracking is required for this decision.
