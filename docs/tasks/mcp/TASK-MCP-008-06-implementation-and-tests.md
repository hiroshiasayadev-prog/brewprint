# TASK-MCP-008-06: Implementation and tests
...
Accepted `ADR-093` and reviewed `SPEC-design-records-mcp-tools` / `SPEC-design-records-mcp-schema` に基づき、Design Records MCP authoring transaction MVP を実装し、targeted tests で public contract を検証する。
...
- REQ-MCP-010 spec domain tree / placement discovery support
...
- Read the current instructions and relevant artifacts before implementation:
  ...
  - `REQ-MCP-008`
  - `WORK-MCP-008`
  - `TASK-MCP-008-01` through `TASK-MCP-008-05`
...
- Runtime MCP call smoke tests for representative cases are either completed or explicitly deferred to `TASK-MCP-008-07`.
...
- `validate_records` for `ADR-093`
- `validate_records` for `REQ-MCP-008`
- `validate_records` for `WORK-MCP-008`
...
- `validate_records(kind: decision, id_range: ADR-093..ADR-093)` returned `ok: true`.
- `validate_records(kind: requirement, id_range: REQ-MCP-008..REQ-MCP-008)` returned `ok: true`.
- `validate_records(kind: work_item, id_range: WORK-MCP-008..WORK-MCP-008)` returned `ok: true`.
- `validate_records(kind: task, id_range: TASK-MCP-008-06..TASK-MCP-008-06)` returned `ok: true`.

### Runtime MCP smoke

Runtime stdio smoke tests are deferred to `TASK-MCP-008-07`.
...
- `docs/tasks/mcp/TASK-MCP-008-01` through `TASK-MCP-008-05`
- `docs/tasks/mcp/TASK-MCP-008-06-implementation-and-tests.md`
- `docs/tasks/mcp/TASK-MCP-009-01` through `TASK-MCP-009-04`
...
Only `TASK-MCP-008-06` was edited among the pre-existing docs.
...
- `TASK-MCP-008-07` should run runtime MCP stdio smoke tests and close evidence.
- `WORK-MCP-008` was not marked done.
- `TASK-MCP-008-07` was not created by this task.
