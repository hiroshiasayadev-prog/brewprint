# TASK-DATA-012-01: Review UC-002 enum, literal, and vocabulary cleanup candidates
...
- **depends_on**:
- **outputs**:
  - Candidate review table for WORK-DATA-012 owned UC-002 enum-like / literal / vocabulary notes
...
Review the UC-002 enum-like, literal, and usage-site vocabulary cleanup candidates owned by `WORK-DATA-012` and produce a bounded candidate classification before any YAML migration or fixture regeneration.
...
- Review the `WORK-DATA-012` candidate set: N-019, N-030, N-034, N-045, N-046, N-051, and residual vocabulary or literal notes from N-006, N-015, N-023, and N-029.
- Use `TASK-DATA-009-03`, `TASK-DATA-009-04`, `INV-DATA-002`, and the relevant UC-002 YAML files as input evidence.
...
- Every candidate owned by `WORK-DATA-012` has a recorded classification.
...
- Confirm the reviewed candidate list matches `WORK-DATA-012` and the `TASK-DATA-009-03` / `TASK-DATA-009-04` successor split.
- Confirm classifications do not reopen M15, `WORK-DATA-001`, `WORK-DATA-006`, `WORK-DATA-009`, or `WORK-DATA-010`.
- Confirm default/behavior candidates remain outside this task and point to `REQ-DATA-006` / `WORK-DATA-013` where applicable.
- Confirm selector/support-matrix candidates remain outside this task and point to `REQ-DATA-007` / `WORK-DATA-014` where applicable.
- Run Design Records MCP validation for the created task and `WORK-DATA-012` after creation.
...
- Design Records MCP records for `ADR-067`, `ADR-070`, `ADR-073`, `REQ-DATA-004`, `WORK-DATA-006`, `WORK-DATA-010`, `REQ-DATA-006`, `WORK-DATA-013`, `REQ-DATA-007`, `WORK-DATA-014`, `REQ-DATA-008`, `WORK-DATA-015`, `REQ-MCP-004`, and `WORK-MCP-004`.
...
| N-051 | `docs/uc/002-brewprint-self-hosting/yaml/mcp/model/string_list.yaml` | `string_list.element` | list element is `str` | Shared `string_list` cannot own one enum because element vocabulary changes by usage site. | usage-site vocabulary / note cleanup candidate | Keep `string_list` as the free-text/shared fallback. For usage sites with a known closed value set, replace the field type with `list<selected_enum>` or a usage-specific list model only after a next-task decision. | Object-dependent vocabularies and selector/support matrices remain under `REQ-DATA-007` / `WORK-DATA-014`. Default/behavior constraints remain under `REQ-DATA-006` / `WORK-DATA-013`. | high | YAML says element enum values are supplemented per usage note; inventory classifies N-051 as not covered by existing ADRs because the shared list has usage-site-dependent vocabularies. |

### Out-of-scope routing notes

- Default, omitted-value, fallback, unknown-value, and other behavior constraints remain outside `WORK-DATA-012` and route to `REQ-DATA-006` / `WORK-DATA-013`. Encountered examples include `get_source_response.fallback` adjacency and request-side direction/default behavior.
- Selector support matrices and object-dependent `kind` vocabularies remain outside `WORK-DATA-012` and route to `REQ-DATA-007` / `WORK-DATA-014`. N-030 should not absorb N-031-style `kind` matrix work.
- Recursive and untagged-union representation remains outside `WORK-DATA-012` and routes to `REQ-DATA-008` / `WORK-DATA-015`.
- Tagged union and discriminator payload support remains outside `WORK-DATA-012` and is already covered by `REQ-DATA-004` / closed `WORK-DATA-010`.
- MCP identity / semantic reference identity remains outside `WORK-DATA-012` and routes to `REQ-MCP-004` / `WORK-MCP-004`. N-045 owns only the public `Reference.kind` value set here.
- Closed work items `WORK-DATA-001`, `WORK-DATA-006`, `WORK-DATA-009`, and `WORK-DATA-010` were treated as closed evidence and were not reopened.
...
- Pre-edit Design Records MCP validation for `TASK-DATA-012-01`: passed.
- Pre-edit Design Records MCP validation for `WORK-DATA-012`: passed.
...
- Post-edit Design Records MCP validation for `TASK-DATA-012-01`: passed with `ok: true`, `diagnostics: null`.
- Post-edit Design Records MCP validation for `WORK-DATA-012`: passed with `ok: true`, `diagnostics: null`.
- `git diff -- docs/tasks/data/TASK-DATA-012-01-review-uc-002-enum-literal-and-vocabulary-cleanup-candidates.md docs/work-items/data/WORK-DATA-012-uc-002-enum-literal-vocabulary-cleanup.md`: normal tracked diff showed only the existing `WORK-DATA-012.tasks` relation addition for `TASK-DATA-012-01`; the task file is currently untracked, so its content is not shown by normal `git diff`.
