# TASK-DATA-013-02: Update MCP tool specs for request option and response behavior contracts
...
Update the MCP tool contract specs for the request option and response behavior decisions made in `TASK-DATA-013-01`.
...
- Relevant MCP tool specs explicitly describe the default, omitted-input, unknown-value, fallback, numeric range, and response grouping behavior selected by `TASK-DATA-013-01`.
...
- Review updated spec text against `TASK-DATA-013-01` Evidence.
- Check that updates do not reopen excluded WORK-DATA-013 scopes such as MCP identity, selector support matrix, tagged unions, recursive structures, or DAG TypeRef hints.
...
- Checked the updated text against `TASK-DATA-013-01` Evidence.
- Confirmed the changes stay within WORK-DATA-013 scope and do not reopen MCP identity, selector matrix, tagged union, recursive structure, or DAG TypeRef hint work.
