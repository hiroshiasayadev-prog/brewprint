# TASK-DATA-009-03: Decide remaining UC-002 notes retreat successor outcomes
...
- **outputs**:
  ...
  - Follow-up artifact creation input for TASK-DATA-009-04 or later

## Goal

Decide the successor outcome for each remaining UC-002 notes retreat bucket classified by `TASK-DATA-009-02`.
...
- Use the bucket classification from `TASK-DATA-009-02` as the input set.
...
- Provide input for a later `TASK-DATA-009-04` or later follow-up split / close task.
...
- Creation of `TASK-DATA-009-04` or later tasks.
...
- Each `TASK-DATA-009-02` bucket has exactly one primary successor outcome.
- Existing ownership, no-action / obsolete outcomes, and proposed future artifact types are explicit.
- Follow-up artifact creation input is available for `TASK-DATA-009-04` or later.
- No implementation, UC-002 cleanup, new requirement, new work item, or later task artifact is created.

## Verification

- Confirm only `TASK-DATA-009-03` and `WORK-DATA-009` documentation files are changed by this task.
- Confirm `TASK-DATA-009-03` status is `done`.
- Confirm `WORK-DATA-009.tasks` includes `TASK-DATA-009-01`, `TASK-DATA-009-02`, and `TASK-DATA-009-03`.
- Confirm `WORK-DATA-009` is not marked `done`.
- Run Design Records MCP validation for `TASK-DATA-009-03` and `WORK-DATA-009`.
...
### Input summary from TASK-DATA-009-02

`TASK-DATA-009-02` classified the remaining `still-unowned` input from `TASK-DATA-009-01` into these buckets:
...
- `MCP identity / semantic reference`: no primary remaining `still-unowned` bucket; primary identity candidates were already reconciled as covered by `REQ-MCP-004` / `WORK-MCP-004`.
...
- DAG asset TypeRef hint render support is covered by `REQ-DATA-005` / `WORK-DATA-007`.
- UC-002 duplicate task QID / unresolved flow task repair is covered and closed as stale by `WORK-DATA-008`.
- Tagged union / discriminator payload candidates are covered by `REQ-DATA-004` / `WORK-DATA-010`.
...
| `recursive / union structure` | N-009, N-044 | `new requirement needed` | These are type-system expressiveness gaps: an untagged union list and recursive `ObjectRef.parent`. `WORK-DATA-010` covers tagged/discriminated unions only and should not be expanded to silently include untagged or recursive type semantics. | n/a | New DATA expressiveness requirement plus work item for recursive and untagged-union representation, or an explicit successor to ADR-073 if later review chooses to broaden that decision. |
| `MCP identity / semantic reference` | No primary remaining `still-unowned` bucket. Previously reconciled identity candidates include N-032, N-035, N-038, N-039, N-043, N-047, and N-050; N-045 and N-048 residue stays in non-identity buckets. | `covered by existing work` | Primary identity and semantic-reference questions are already owned outside DATA cleanup by `REQ-MCP-004` / `WORK-MCP-004`. Creating duplicate DATA identity work would conflict with the prior split. | `REQ-MCP-004` / `WORK-MCP-004` | n/a |
...
- Residual N-048 map-shape question: no DATA follow-up is selected here. If `WORK-MCP-004` later needs public reference-index map schema, that MCP work owns the split.
...
### Explicit follow-up artifact candidates for TASK-DATA-009-04

`TASK-DATA-009-04` or later should create only the selected follow-up artifacts, or record an explicit close decision if the user chooses not to create them:

- New DATA work item under `REQ-DATA-002`: request-side / generic container cleanup for N-002, N-004, N-007, N-008, N-012, N-016, N-018, and `TF-QUERY-RESULT`.
- New DATA work item under `REQ-DATA-002`: remaining enum / literal / usage-site vocabulary cleanup for N-019, N-030, N-034, N-045, N-046, N-051, and residual N-006/N-015/N-023/N-029 vocabulary or literal notes.
...
- New DATA expressiveness requirement plus work item: recursive and untagged-union representation for N-009 and N-044, kept separate from `WORK-DATA-010` unless a later ADR-073 review explicitly broadens that scope.

`TASK-DATA-009-04` should record no-action close notes rather than create artifacts for N-049, residual N-048 map-shape questions, N-052 through N-056, and the empty `other still-unowned` bucket.

No follow-up DATA artifact should be created for primary MCP identity rows already covered by `REQ-MCP-004` / `WORK-MCP-004`, for N-052 through N-056, for N-049, for residual N-048 map-shape questions, or for the empty `other still-unowned` bucket.

### Verification note

This task performed successor outcome decision only.

No UC-002 YAML migration, fixture / golden regeneration, parser / renderer / validator / MCP implementation change, ADR-073 implementation, ADR-074 implementation, ADR-078 / ADR-079 / ADR-080 MCP identity implementation, UC-002 duplicate task QID / unresolved flow task fix, new requirement creation, new work item creation, or `TASK-DATA-009-04` creation was performed.

`WORK-DATA-009` was not marked `done`; it remains open because `T4` still needs to create the selected follow-up split artifacts or close with explicit no-action outcomes.
...
- `TASK-DATA-009-03` exists with status `done`.
- `WORK-DATA-009.tasks` includes `TASK-DATA-009-01`, `TASK-DATA-009-02`, and `TASK-DATA-009-03`.
- `WORK-DATA-009` status is `decision_pending`, not `done`.
