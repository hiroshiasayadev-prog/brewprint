# TASK-DATA-011-03: Migrate generic container any fields and add public model shapes to UC-002 YAML
...
- **outputs**:
  ...
  - TASK-DATA-011-04 へのインプット

## Goal

TASK-DATA-011-02 のスコープ決定に基づき、UC-002 YAML の `any` フィールドを `string_list` またはヘルパーモデル型に移行する。
...
- 新規 REQ / WORK / ADR 起票。
- WORK-DATA-011 の `done` 遷移（TASK-DATA-011-04 担当）。
