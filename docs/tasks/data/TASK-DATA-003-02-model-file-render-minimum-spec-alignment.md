...
- **depends_on**:
  - TASK-DATA-003-01
- **outputs**:
  - ADR-075 decision-basis confirmation without ADR text change
  - Model-file render minimum spec alignment
  - Implementation-entry notes for model-file render minimum

## Goal

Align the relevant specs for the model-file render minimum path under WORK-DATA-003, using ADR-075 as the decision basis without editing ADR-075 in this task.

This task turns the TASK-DATA-003-01 decision into concrete documentation: ADR-075 is usable as the model-file render decision basis, while tagged union rendering is excluded from the current WORK-DATA-003 execution scope and remains deferred to ADR-073 or a separate tagged-union work item.

## Work

- Review TASK-DATA-003-01 evidence.
- Use ADR-075 as the model-file render decision basis without changing ADR-075 text or status in this task.
...
- Tagged union rendering is excluded from WORK-DATA-003 current execution scope and deferred to a later tagged-union work item.
...
- WORK-DATA-002 task-file helper minimum reopening.
- REQ-DATA-003 params / returns signature exposure policy.

## Done Condition

- ADR-075 is recorded as the decision basis for this task without changing ADR-075 text or status.
...
- Implementation-entry notes for TASK-DATA-003-03 are recorded.
- No renderer implementation, YAML migration, fixture update, or test execution is performed.

## Verification

- Confirm TASK-DATA-003-01 decision is preserved.
- Confirm WORK-DATA-003 current execution scope does not force ADR-073 implementation.
- Confirm ADR-070 human-visible helper constraint is satisfied for model-file helpers.
- Confirm ADR-072 model catalog remains opt-in and not a substitute for model-file render.
- Confirm WORK-DATA-002 remains closed and not reopened.
...
- `docs/spec/views/model-file.md` records ADR-075 as decision basis without editing ADR-075 text or status.
