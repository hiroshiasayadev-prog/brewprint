...
- **outputs**:
  ...
  - stale coverage / TASKS-UC-002 update list for TASK-SELFHOST-001-03
...
- `coverage.md` / `TASKS-UC-002.md` に反映すべき stale 記述を TASK-SELFHOST-001-03 へ送る。
...
- TASK-SELFHOST-001-03 に送る stale docs 更新事項が列挙されている。
...
- 2026-05-31: `git show --stat 8617d9c` confirmed that the commit added TASK-SELFHOST-001-01 evidence, 11 UC-002 render files, and a small WORK-SELFHOST-001 update.
...
- TASK-SELFHOST-001-03 stale docs update list:
  ...
  - Update `docs/uc/002-brewprint-self-hosting/docs/coverage.md` §8 execution results with the TASK-SELFHOST-001-01 and TASK-SELFHOST-001-02 evidence.
  ...
  - Consider adding a short note that WORK-SELFHOST-001 remains `fixture_pending` until coverage / TASKS-UC-002 synchronization and close evidence are completed.
- WORK-SELFHOST-001 status:
  - Keep `fixture_pending`.
  - Do not mark `done` until TASK-SELFHOST-001-03 synchronizes coverage / TASKS-UC-002 and records close evidence.
...
- 2026-05-31: Design Records MCP `validate_records(kind="task")` returned `ok: false` because of unrelated untracked workflow artifacts outside this task scope:
  - `TASK-DATA-002-01`: `workflow_relation_mismatch` because `WORK-DATA-002.tasks` does not contain `TASK-DATA-002-01`.
  - `TASK-MCP-005-01`: invalid status `not_started`.
  - `TASK-MCP-005-01`: `workflow_relation_mismatch` because `WORK-MCP-005.tasks` does not contain `TASK-MCP-005-01`.
  - No diagnostic was reported for `TASK-SELFHOST-001-02` or `WORK-SELFHOST-001`.
