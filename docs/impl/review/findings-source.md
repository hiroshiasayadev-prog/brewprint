...
source層レビュー（Sonnet担当）と Claude による追加発見で出た findings 8件を、ADR-050 の spec-first 方針に基づいて整理。
...
| ID | ラベル | 概要 | 最終対応 |
|---|---|---|---|
| F-S-001 | spec-migrate | ADR-030 の `as:` 値一覧に `er_diagram` 未記載 | spec/file-types.md §3 に集約。ADR-030 漸進移行 |
| F-S-002 | spec-decision + ADR | Unsupported の扱い未文書化 | warning diagnostic 採用。ADR-051 起票、spec/file-types.md §4 + spec/diagnostics.md 反映 |
| F-S-003 | spec-migrate | ADR-002 の `master.yaml` が ADR-043 で覆されているが ADR-002 に注記なし | ADR-002 の partial supersede 行を更新、漸進移行 |
| F-S-004 | spec-migrate | render_index.yaml の loader 探索ルールが未文書化 | spec/project-layout.md §1〜§2 に記載 |
| F-S-005 | doc-only | ADR-002 / ADR-030 の Evidence の `impl commit: tbd` | **未対応**（後述）|
| F-S-006 | spec-decision + ADR | `File.Path` が abs/rel どちらか未規定 | プロジェクトルート相対 + slash 正規化を採用。ADR-052 起票、spec/file-types.md §5 反映 |
| F-S-007 | doc-only | wireframe は file-level view を持たないが ADR-030 に明示なし | spec/file-types.md §3 注記で対応 |
| F-S-008 | doc-only | ADR-030 は `as:` 有無で振り分けと書くが、実装は `nodes:` キーも判定 | spec/file-types.md §2 で分類アルゴリズムを正確に記述 |

## 各findingの詳細

### F-S-001 [spec-migrate]: `as:` 値一覧

**問題**: ADR-030 の `as:` 値表に `er_diagram`（ADR-039 で追加）が反映されていなかった。

**対応**:
- `docs/spec/file-types.md` §3 に `as:` 値一覧を集約。`api_table` / `sequence_diagram` / `er_diagram` を全て記載
- 各値の由来 ADR を表に明記
- ADR-030 漸進移行: 起票時点表を残しつつ、現行仕様は spec 参照と注記
...
**対応**:
- ADR-051 起票（accepted）: `unsupported_file` warning diagnostic を loader フェーズで発行
...
**問題**: ADR-002 が定めた `master.yaml` が ADR-043 で実質廃止されたが、ADR-002 に注記がなかった。

**対応**:
- ADR-002 の `partial supersede` 行に「ADR-043により `master.yaml` は廃止」を追加
- 「決定」セクション内の `master.yaml` 記述に「ADR-043 で廃止」の注記
...
**問題**: loader が「`yaml/` の親ディレクトリ直下の `render_index.yaml` を探す」挙動が ADR-043 から自然に読み取れない。
...
**問題**: ADR-002 / ADR-030 の Evidence の `impl commit: tbd` が放置されている。本来は実装commit hash で埋めるべき。
...
**対応**:
- ADR-052 起票（accepted）: プロジェクトルート相対 + slash 正規化を採用
...
**問題**: ADR-030 の `as:` 値表に wireframe が含まれないが、その理由が明示されていなかった。

**対応**:
- `docs/spec/file-types.md` §3 に注記: 「wireframe はファイルレベルの view を持たない。state ノード内に DSL として埋め込まれる形でのみ存在する（ADR-029, ADR-042）」

**ステータス**: 完了。

### F-S-008 [doc-only]: 分類判定アルゴリズム

**問題**: ADR-030 は「`as:` 有無で振り分け」と書くが、実装は `as:` 優先 → `nodes:` チェック → Unsupported の3段判定をしている。

**対応**:
- `docs/spec/file-types.md` §2 に分類アルゴリズムを4段階（ファイル名 → `as:` → `nodes:` → unsupported）で正確に記述
- ADR-030 漸進移行と合わせ、現行仕様は spec 参照とした
...
- **F-S-005**: ADR-002 / ADR-030 の `impl commit: tbd` の更新（ユーザー側で git log 提示が必要）
...
- **ネスト module 内 task の render 出力ファイル名**: ADR-045 §8 で「Go 実装 ADR で規定」と明記済み。実装 ADR 起票時に確定
