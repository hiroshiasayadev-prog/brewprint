...
- ADR-014: initializesフィールド設計
- ADR-027: モジュールネスト許容と名前解決ルール拡張
- ADR-030: YAMLファイル種別の自己宣言
- ADR-031: actorのグローバル定義
- ADR-047: Go semantic model / query layer boundary
- ADR-048: ResolvedProject index strategy
...
- actorはADR-031に従いglobal IDとして扱う
- symbol tableを作る
...
ADR-028に基づく正式なroute合成は、M5のAPI Table rendererで扱う。
...
- actorはADR-031によりproject globalである
- ADR-048のbase indexに `actorsByQID` が含まれる
